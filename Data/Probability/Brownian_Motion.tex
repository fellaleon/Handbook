...
\beast
\E\bb{\exp\bb{i\sum^{k}_{j=1} \xi_j\bb{B^{(n)}_{t_j} - B^{(n)}_{t_{j-1}}}}}  & = & \prod^k_{j=1} \E\bb{\exp\bb{i\xi_j\bb{B^{(n)}_{t_j} - B^{(n)}_{t_{j-1}}}}} \quad \text{(by Theorem \ref{thm:characteristic_function})}\\
...
Let $W = C(\R^{++},\R^d) = C([0,\infty),\R^d)$ (space of continuous functions, that is, $f:[0,\infty) \to \R^d$) be the space, endowed with the $\sigma$-algebra $\sW :=\sB\bb{C([0,\infty))}$. %product $\sigma$-algebra $\sW$ (or the Borel $\sigma$-algebra associated with the compact-open topology (wrt time)).

Let $(B_t)_{t \geq 0}$ be a Brownian motion, and let $\W$ be the law of $B$. that is, for any $A \in \sW$,
\be
\W(A) := \pro\bb{B \in A} = \pro\bb{\omega:(B_t(\omega))_{t \geq 0}\in A}.
\ee

It is also true that $\sW_t = \sigma(B_s: s \leq t)$ and $\sW = \sigma(B_t: t\geq 0)$ where $B_t: W\to\R, B(\omega) \mapsto B_t(\omega)$.

Then $\W$ is called Wiener measure\index{Wiener measure} and $\bb{W,\sW,\W}$ is called Wiener space\index{Wiener space}.
...
\beast
\W \bb{\phi(\omega) \in W:Y_{t_1}(\phi(\omega))\in A_1,\dots, Y_{t_n}(\phi(\omega))\in A_n}
& = & \W\bb{\phi(\omega) \in W:(\phi(\omega))(t_1) \in A_1,\dots, (\phi(\omega))(t_n)\in A_n}\\
& = & \W\bb{B(\omega)\in W: B_{t_1}(\omega)\in A_1,\dots B_{t_n}(\omega)\in A_n} \\
...
\beast
f\bb{B^{(T)}_{t_1},\dots, B^{(T)}_{t_m}} & = & f\bb{B_{t_1+T}-B_T,\dots, B_{t_m+T}-B_T} \\
& = & \lim_{n\to \infty} f\bb{B_{t_1+T_n}-B_{T_n},\dots, B_{t_m+T_n}-B_{T_n}} = \lim_{n\to \infty} f\bb{B^*_{t_1},\dots, B^*_{t_m}}
\eeast
...
\beast
\E\bb{\ind_A f\bb{B^{(T)}_{t_1},\dots, B^{(T)}_{t_m}}} & = & \lim_{n\to \infty}\E\bb{\ind_A f\bb{B^*_{t_1},\dots, B^*_{t_m}}} \\
& = & \pro\bb{A} \lim_{n\to \infty}  \E\bb{f\bb{B^{*}_{t_1},\dots,B^{*}_{t_m}}} = \pro\bb{A} \E\bb{f\bb{B^{(T)}_{t_1},\dots,B^{(T)}_{t_m}}}.
\eeast
...
\be
\pro\bb{\bra{T_y-T_x > t}\cap A} = \pro\bb{\bra{B^{(T_x)}_t < y-x}\cap A} = \pro\bb{B^{(T_x)}_t < y-x}\pro(A) = \pro\bb{T_{y}-T_x > t} \pro(A).
\ee
...
%\ee
%which is inverse gamma distribution $\Gamma^{-1}\bb{\frac 12, \frac{x^2}2}$ where $k=\frac 12$ and $\lm = \frac {x^2}2$ (see Definition \ref{def:inverse_gamma_rv}). Also, since $k= \frac 12$, we know that this hitting time $\E T = \infty$ a.s. as the expectation only exists for $k>1$.
%\end{proof}
...
& \stackrel{\text{a.s.}}{=} & \E\dabs{B_t-B_{s}}^2 + 2B_{s^+} \cdot\E\bb{B_t-B_s} + \dabs{B_{s^+}}^2 \quad (\text{Propositions \ref{pro:conditional_expectation_tower_independence}.(v), \ref{pro:conditional_expectation_basic_property}.(ii)})\\
...
\be
\bb{B_t\dabs{B_t}^2 - (d+2)tB_t}_{t \geq 0},\qquad \bb{\dabs{B_t}^4 - 2(d+2)t\dabs{B_t}^2 + d(d+2)t^2}_{t\geq 0}
\ee
...
\be
\E\dabs{B_t}^4 = \E\bb{2(d+2)t\dabs{B_t}^2} - d(d+2)t^2 = d(d+2)t^2.
\ee
\end{remark}
...
In particular,
\beast
\E\bb{B_t \dabs{B_t}^2|\sF_{s^+}} & = & \E\bb{(B_t-B_s + B_s)\bb{\left.\dabs{B_t-B_s}^2 + 2 (B_t-B_s)^T B_{s^+} + \dabs{B_{s^+}}^2}\right|\sF_{s^+}}\\
& \stackrel{\text{a.s.}}{=} & \E\bb{(B_t-B_s)\dabs{B_t-B_s}^2}  + B_s\E\bb{\dabs{B_t-B_s}^2}  + 2 \E\bb{(B_t-B_s)(B_t-B_s)^T} B_{s^+} \\
& & \qquad + 2B_{s^+}B_{s^+}^T \E(B_t-B_s)  + \dabs{B_{s^+}}^2\E(B_t-B_s) + \dabs{B_{s^+}}^2B_{s^+} \\
& = & B_s\E\bb{\dabs{B_t-B_s}^2}  + 2 \E\bb{(B_t-B_s)(B_t-B_s)^T} B_{s^+}  + \dabs{B_{s^+}}^2B_{s^+} \\
& = & d(t-s) B_{s^+} + 2(t-s)B_{s^+}  + B_{s^+}\dabs{B_{s^+}}^2= (d+2)(t-s) B_{s^+}  + B_{s^+}\dabs{B_{s^+}}^2.
\eeast

\beast
\E\bb{tB_t|\sF_{s^+}} = \E\bb{t(B_t-B_s + B_s|\sF_{s^+}} \stackrel{\text{a.s.}}{=}  tB_{s^+}.
\eeast

Therefore,
\beast
\E\bb{\left. B_t \dabs{B_t}^2 - (d+2)tB_t\right|\sF_{s^+}} & \stackrel{\text{a.s.}}{=} &  (d+2)(t-s) B_{s^+}  + B_{s^+}\dabs{B_{s^+}}^2 - (d+2)tB_{s^+} \\
& = & B_{s^+}\dabs{B_{s^+}}^2 - (d+2)s^+B_{s^+}
\eeast

For the fourth moment, we let $B_t^i$ be the $i$th element of vector $B_t$ and have
\beast
\E\bb{\left.\dabs{B_t}^4\right|\sF_{s^+}} & = & \E\bb{\left.\bb{\dabs{B_t}^2}^2\right|\sF_{s^+}} = \E\bb{\left.\bb{B_t^TB_t}^2\right|\sF_{s^+}} = \E\bb{\left.\bb{\sum^d_{i=1}\bb{B_t^i}^2 }^2\right|\sF_{s^+}} \\ %+ 2\sum_{1\leq i< j\leq d}B_t^iB_t^j
%& = & \E\bb{\left.\bb{\sum^d_{i=1}\bb{B_t^i}^2}^2 + 4\bb{\sum^d_{i=1}\bb{B_t^i}^2 \sum_{1\leq i< j\leq d}B_t^iB_t^j}+ 4 \bb{\sum_{1\leq i< j\leq d}B_t^iB_t^j}^2\right|\sF_{s^+}} \\
& = & \E\bb{\left.\sum^d_{i=1}\bb{B_t^i}^4 + 2\sum_{1\leq i<j\leq d}\bb{B_t^i}^2 \bb{B_t^j}^2\right|\sF_{s^+}} %+ 4\bb{\sum^d_{k=1}\bb{B_t^k}^2 \sum_{1\leq i< j\leq d}B_t^iB_t^j}+ 4 \bb{\sum_{1\leq i< j\leq d}B_t^iB_t^j}^2
\eeast

The first term is
\beast
& & \E\bb{\left.\sum^d_{i=1}\bb{B_t^i}^4\right|\sF_{s^+} } = \E\bb{\left. \sum^d_{i=1}\bb{B^i_t}^4\right|\sF_{s^+} }  = \E\bb{\left. \sum^d_{i=1}\bb{B^i_t-B_s^i + B_s^i}^4\right|\sF_{s^+} }  \\
& = & \E\bb{\left.\sum^d_{i=1}\bb{B^i_t-B^i_s}^4 + 4\bb{B^i_t-B^i_s}^3B_s^i + 6\bb{B^i_t-B^i_s}^2\bb{B_s^i}^2 + 4\bb{B^i_t-B^i_s}\bb{B_s^i}^3 + \bb{B_s^i}^4\right|\sF_{s^+}} \\
& \stackrel{\text{a.s.}}{=}  &  3d(t-s^+)^2 + 6(t-s^+)\sum^d_{i=1}\bb{B_{s^+}^i}^2 + \sum^d_{i=1}\bb{B_{s^+}^i}^4 \\
& = &  3d(t-s^+)^2 + 6(t-s^+)\dabs{B_{s^+}}^2 + \sum^d_{i=1}\bb{B_{s^+}^i}^4 .
\eeast

The second term is
\beast
& & 2\E\bb{\left.\sum_{1\leq i<j\leq d}\bb{B_t^i}^2 \bb{B_t^j}^2\right|\sF_{s^+}} = 2\E\bb{\left.\sum_{1\leq i<j\leq d}\bb{B_t^i-B_s^i+ B_s^i}^2 \bb{B_t^j-B_s^j+ B_s^j}^2\right|\sF_{s^+}} \\
& = & 2\E\bb{\left.\sum_{1\leq i<j\leq d}\bb{\bb{B_t^i-B_s^i}^2 + 2\bb{B_t^i-B_s^i}B_s^i+  \bb{B_s^i}^2} \bb{\bb{B_t^j-B_s^j}^2 + 2\bb{B_t^j-B_s^j}B_s^j+  \bb{B_s^j}^2}\right|\sF_{s^+}} \\
& \stackrel{\text{a.s.}}{=} & 2\bb{\sum_{1\leq i<j\leq d}(t-s)^2 + (t-s)\bb{ \bb{B_s^i}^2 + \bb{B_s^j}^2}+ \bb{B_s^i}^2\bb{B_s^j}^2 } \\
& = & d(d-1)(t-s^+)^2 + 2(t-s^+) \sum_{1\leq i<j\leq d}\bb{\bb{B_{s^+}^i}^2 +  \bb{B_s^j}^2} + 2\sum_{1\leq i<j\leq d}\bb{B_{s^+}^i}^2\bb{B_{s^+}^j}^2 \\
& = & d(d-1)(t-s^+)^2 + 2(t-s^+) (d-1)\dabs{B_{s^+}}^2 + 2\sum_{1\leq i<j\leq d}\bb{B_{s^+}^i}^2\bb{B_{s^+}^j}^2 .
\eeast

%The third term is
%\beast
%& & 4\E\bb{\left.\sum^d_{k=1}\bb{B_t^k}^2 \sum_{1\leq i< j\leq d}B_t^iB_t^j\right|\sF_{s^+}} \\
%& = & 4\E\bb{\left.\sum^d_{k=1}\bb{B_t^k-B_s^k + B_s^k}^2 \sum_{1\leq i< j\leq d}(B_t^i-B_s^i + B_s^i)(B_t^j-B_s^j + B_s^j)\right|\sF_{s^+}} \\
%& \stackrel{\text{a.s.}}{=} & 4\E\bb{\left.\sum^d_{k=1}\bb{B_s^k}^2 \sum_{1\leq i< j\leq d}(B_t^i-B_s^i + B_s^i)(B_t^j-B_s^j + B_s^j)\right|\sF_{s^+}}  \stackrel{\text{a.s.}}{=}  4\sum^d_{k=1}\bb{B_{s^+}^k}^2 \sum_{1\leq i< j\leq d}B_{s^+}^i B_{s^+}^j.
%\eeast
%
%The fourth term is
%\beast
%& & 4 \E\bb{\left.\bb{\sum_{1\leq i< j\leq d}B_t^iB_t^j}^2\right|\sF_{s^+}}  = 4 \E\bb{\left.\sum_{1\leq i< j\leq d}B_t^iB_t^j\sum_{1\leq m< n\leq d}B_t^mB_t^n\right|\sF_{s^+}} \\
%& = & 4 \E\bb{\left.\sum_{1\leq i< j\leq d}(B_t^i - B_s^i + B_s^i)(B_t^j - B_s^j + B_s^j)\sum_{1\leq m< n\leq d}(B_t^m - B_s^m + B_s^m)(B_t^n - B_s^n + B_s^n)\right|\sF_{s^+}} \\
%& \stackrel{\text{a.s.}}{=} & 4 \E\bb{\left.\sum_{1\leq i< j\leq d}(B_t^i - B_s^i)^2(B_t^j - B_s^j)^2 + \sum_{1\leq i,j<n\leq d}B_t^i B_s^j(B_t^n - B_s^n)^2 + \sum_{1\leq m< i,j\leq d}B_t^i B_s^j(B_t^m - B_s^m)^2 \right|\sF_{s^+}} \\
%& & \qquad + 4\E \bb{\left. \sum_{1\leq i< j\leq d}\sum_{1\leq m< n\leq d} B_s^i B_s^jB_s^m B_s^n\right|\sF_{s^+}} \\
...
\be
\E\bb{\left. t\dabs{B_t}^2\right|\sF_{s^+}} =  t \E\bb{\left. \dabs{B_t}^2\right|\sF_{s^+}} =  t\bb{ d(t-s^+) + \dabs{B_{s^+}}^2} = dt(t-s^+) + t\dabs{B_{s^+}}^2.
\ee

Thus,
\beast
& & \E\bb{\left. \dabs{B_t}^4 - 2(d+2)t\dabs{B_t}^2\right|\sF_{s^+}} \\
...
& = &  \dabs{B_{s^+}}^4 - 2(d+2)s^+\dabs{B_{s^+}}^2 - d(d+2)\bb{t^2-\bb{s^+}^2}.
\eeast
...
\be
f_t(y) =  \sqrt{\frac{2}{\pi\sigma^2 t}} \exp\bb{-\frac {y^2}{2\sigma^2 t}} ,\quad f_t(z) =  \sqrt{\frac{2}{\pi\sigma^2 t}} \exp\bb{-\frac {z^2}{2\sigma^2 t}} \quad \text{where }y>0,z<0.
\ee
...
\be
\pro\bb{S_t \leq y} = \pro\bb{\abs{B_t} \leq y} = \frac{2}{\sqrt{2\pi\sigma^2t}}\int^y_0 \exp\bb{-\frac {x^2}{2\sigma^2t}}dx =  \frac{2}{\sqrt{2\pi}}\int^{\frac{y}{\sqrt{\sigma^2t}}}_0 \exp\bb{-\frac {x^2}{2}}dx.
\ee

Taking the differentiation with respect to $y$, we have that
\be
f_t(y) = \sqrt{\frac{2}{\pi\sigma^2 t}} \exp\bb{-\frac {y^2}{2\sigma^2 t}}.
\ee
...
\be
\pro\bb{I_t < z, B_t\geq x} = \pro\bb{\sup_{0\leq s\leq t} W_s >-z, W_t \leq -x} = \pro\bb{W_t \geq -2z +x} = \pro\bb{B_t\leq 2z-x}
\ee

Thus,
\be
\pro\bb{I_t \geq z, B_t\geq x} = \pro(B_t\geq x) - \pro\bb{I_t < z, B_t\geq x} = \pro(B_t\geq x) - \pro\bb{B_t\leq 2z-x}
\ee
...
\be
f_t(x,y,z) = 4 \sum^\infty_{k=-\infty}  k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{x-2y+2k(y-z)}.
\ee
where
\be
\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{x^2}{2\sigma^2 t}}.
\ee

Also, for $a\geq 0$, $W_t = \max_{0\leq s\leq t}\abs{X_s}$, %\be%\pro\bb{Y_t \leq y,Z_t \geq z} = \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum_{k=1}^\infty \frac{2k\pi\sigma^4 \bb{\exp\bb{\frac{\mu y}{\sigma^2}} - (-1)^k \exp\bb{\frac{\mu z}{\sigma^2}}} }{k^2\pi^2\sigma^4  + \mu^2(y-z)^2}\exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}}\sin\frac{k\pi y}{y-z}.\ee
\be
f_{W_t}(a)= \frac 4{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=1}(-1)^{k+1}(2k-1)\exp\bb{-\frac{(2k-1)^2a^2}{2\sigma^2 t}},\qquad \E W_t = \sigma\sqrt{\frac{\pi t}{2}}.
\ee
\end{theorem}
...
\be
f_{X_t}(x,Y_t \leq y,Z_t \geq z) = \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x + 2kz - 2ky)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x -2kz + 2(k-1)y)^2}{2\sigma^2 t}}}.\qquad (*)
\ee

Then
\beast
& & f_{X_t,Z_t}(x,Y_t \leq y, z) \\
& = & - \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \frac{-2k(x-2k(y-z))}{\sigma^2 t}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} - \frac{2k(x-2y+2k(y-z))}{\sigma^2 t} \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}} \\
& = & \frac 1{\sqrt{2\pi \sigma^2 t}}   \sum^\infty_{k=-\infty} \frac{2k(x-2k(y-z))}{\sigma^2 t}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} + \frac{2k(x-2y+2k(y-z))}{\sigma^2 t} \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}}
\eeast

Thus,
\beast
f_{X_t,Y_t,Z_t}(x,y, z) & = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\frac{4k^2(x-2k(y-z))^2}{\sigma^4 t^2} - \frac{4k^2}{\sigma^2 t}}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} \\
& & + \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{ \frac{4k(k-1)}{\sigma^2 t} - \frac{4k(k-1)(x-2y+2k(y-z))^2}{\sigma^4 t^2} }\exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}} \\
& = & 4 \sum^\infty_{k=-\infty}  k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{(x-2y+2k(y-z)}.
\eeast
where $\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{x^2}{2\sigma^2 t}}$. From ($*$), we have
\be
\pro\bb{W_t \leq a} = \int^a_{-a}\frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x - 4ka)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x + 2(2k-1)a)^2}{2\sigma^2 t}}}dx.
\ee

Therefore, taking differentiation with respect to $a$,
\beast
f_{W_t}(a) & = & \frac 4{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=-\infty} \int^a_{-a}\frac{2k(x+4ka)}{2\sigma^2 t}\exp\bb{-\frac{(x + 4ka)^2}{2\sigma^2 t}} + \frac{(2k-1)(x+ 2(2k-1)a)}{2\sigma^2 t}\exp\bb{-\frac{(x + 2(2k-1)a)^2}{2\sigma^2 t}}dx\\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=-\infty} 2k \exp\bb{-\frac{(4k+1)^2a^2}{2\sigma^2 t}} - 2k \exp\bb{-\frac{(4k-1)^2a^2}{2\sigma^2 t}}  \\
& & \quad - \frac 2{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=-\infty} (2k-1)\exp\bb{-\frac{(4k-1)^2a^2}{2\sigma^2 t}} + (2k-1)\exp\bb{-\frac{(4k-3)^2a^2}{2\sigma^2 t}}\\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=-\infty} (4k+1) \exp\bb{-\frac{(4k+1)^2a^2}{2\sigma^2 t}} - (4k-1)\exp\bb{-\frac{(4k-1)^2a^2}{2\sigma^2 t}}  \\
& = & \frac 4{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=1} (-1)^{k+1} (2k-1)\exp\bb{-\frac{(2k-1)^2a^2}{2\sigma^2 t}} .
\eeast

Hence,
\beast
\E W_t & = & \int^\infty_{0}\frac 4{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=1} (-1)^{k+1} (2k-1)a \exp\bb{-\frac{(2k-1)^2a^2}{2\sigma^2 t}} da \\
& = & \frac 4{\sqrt{2\pi \sigma^2 t}}  \sum^\infty_{k=1} (-1)^{k+1} (2k-1)\int^\infty_{0} a \exp\bb{-\frac{(2k-1)^2a^2}{2\sigma^2 t}} da \\
...
\be
p_t(r) = \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \exp\bb{-\frac{k^2r^2}{2\sigma^2 t}}.
\ee

Accordingly, we have
\be
\pro\bb{R_t\leq r} = 1 + 4\sum^\infty_{k=1}(-1)^{k}k \erfc\bb{\frac{kr}{\sigma\sqrt{2 t}}} = 1 + 4\sum^\infty_{k=1}(-1)^{k}k \bb{1 - \erf\bb{\frac{kr}{\sigma\sqrt{2 t}}}}
\ee
...
\be
w_t(x) = \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{-\frac{x^2}{2\sigma^2 t}}.
\ee
...
\beast
\wh{w}_t(x) & = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x-2k(u+v))^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x+2u -2k(u+v))^2}{2\sigma^2 t}}}\\
& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}}.
\eeast
...
\beast
\frac{\partial F_t(S_t \leq v, I_t \geq -u)}{\partial v} & = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty}  \fp{}{v}\bb{\int^v_{-u} \bb{\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}} dx} \\
& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \int^v_{-u} \frac {2k(x-2ku - 2kv)}{\sigma^2 t}\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} dx \\
& & \qquad - \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \int^v_{-u} \frac {-2(k-1)(x+2ku + 2(k-1)v)}{\sigma^2 t}  \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}} dx
\eeast

This gives that
\beast
\frac{\partial F_t(S_t \leq v, I_t \geq -u)}{\partial v} & = & \left.\frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{k\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} + (k-1) \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}}\right|^{-u}_v \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} k\bb{\exp\bb{-\frac{((2k+1)u + 2kv)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(2ku + (2k-1)v)^2}{2\sigma^2 t}}} \\
& & \qquad + \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty}  (k-1) \bb{\exp\bb{-\frac{((2k-1)u + 2(k-1)v)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(2ku + (2k-1)v)^2}{2\sigma^2 t}}}\\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} 2k \exp\bb{-\frac{((2k+1)u + 2kv)^2}{2\sigma^2 t}} - (2k-1)\exp\bb{-\frac{(2ku + (2k-1)v)^2}{2\sigma^2 t}} \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} (-1)^k k\exp\bb{-\frac{((k+1)u + kv)^2}{2\sigma^2 t}} .
\eeast

Thus,
\beast
f_t(u,v) & = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \fp{}{u}\bb{(-1)^k k\exp\bb{-\frac{((k+1)u + kv)^2}{2\sigma^2 t}}} \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} (-1)^{k+1}k(k+1)\frac{(k+1)u + kv}{\sigma^2 t}  \exp\bb{-\frac{((k+1)u + kv)^2}{2\sigma^2 t}}
\eeast

Then, the transition density function of the range $R_t$ is
\beast
p_t(r) & = & \int^r_0 f_t(u,r-u) du \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} (-1)^{k+1}k(k+1) \int^r_0 \frac{u + kr}{\sigma^2 t}  \exp\bb{-\frac{( u + kr)^2}{2\sigma^2 t}} du \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} (-1)^{k+1}k(k+1) \left. \exp\bb{-\frac{(u + kr)^2}{2\sigma^2 t}} \right|^0_r \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} (-1)^{k+1}k(k+1) \bb{\exp\bb{-\frac{(kr)^2}{2\sigma^2 t}}  - \exp\bb{-\frac{((k+1)r)^2}{2\sigma^2 t}} }
\eeast

If k = 0, the term is 0, thus we have
\beast
p_t(r) & = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=1} (-1)^{k+1} k(k+1) \bb{\exp\bb{-\frac{(kr)^2}{2\sigma^2 t}}  - \exp\bb{-\frac{((k+1)r)^2}{2\sigma^2 t}}} \\
& & \quad + \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=1} (-1)^{k+1} k(k-1) \bb{\exp\bb{-\frac{(kr)^2}{2\sigma^2 t}} - \exp\bb{-\frac{((k-1)r)^2}{2\sigma^2 t}}}
\eeast

Therefore,
\beast
p_t(r) & = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=1} (-1)^{k+1} \bb{2k^2 \exp\bb{-\frac{(kr)^2}{2\sigma^2 t}} -  k(k+1) \exp\bb{-\frac{((k+1)r)^2}{2\sigma^2 t}} - k(k-1) \exp\bb{-\frac{((k-1)r)^2}{2\sigma^2 t}}} \\
& = & \frac 2{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=1} (-1)^{k+1} \bb{2k^2 \exp\bb{-\frac{(kr)^2}{2\sigma^2 t}} +  k(k-1) \exp\bb{-\frac{(kr)^2}{2\sigma^2 t}} + k(k+1) \exp\bb{-\frac{(kr)^2}{2\sigma^2 t}}} \\
& = & \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \exp\bb{-\frac{k^2r^2}{2\sigma^2 t}}.
\eeast

Thus, we can not switch the integrals such that
\beast
\pro\bb{R\leq r} & = & \int^r_0 \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} ds =  \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \int^r_0 \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} ds \\
& = & \sum^\infty_{k=1}(-1)^{k+1}k \frac{8}{\sqrt{\pi }}\int^r_0 \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} d\bb{\frac{ks}{\sigma\sqrt{2t}}} = 4\sum^\infty_{k=1}(-1)^{k+1}k \erf\bb{\frac{kr}{\sigma\sqrt{2 t}}}.
\eeast
...
\beast
\pro\bb{R > r} & = &  \int^\infty_r \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} ds = \frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \int^\infty_r \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} ds \\
& = & \sum^\infty_{k=1}(-1)^{k+1}k \frac{8}{\sqrt{\pi }}\int^\infty_r \exp\bb{-\frac{k^2s^2}{2\sigma^2 t}} d\bb{\frac{ks}{\sigma\sqrt{2t}}} = 4\sum^\infty_{k=1}(-1)^{k+1}k \erfc\bb{\frac{kr}{\sigma\sqrt{2 t}}}.
\eeast

This is based on the fact that $\sum^\infty_{k=1}(-1)^{k+1}k \erfc\bb{\frac{kr}{\sigma\sqrt{2 t}}}$ is convergent. Hence,
\be
\pro\bb{R\leq r} = 1 - \pro\bb{R >r} = 1 + \sum^\infty_{k=1}(-1)^{k}k \erfc\bb{\frac{kr}{\sigma\sqrt{2 t}}}
\ee
as required.
\end{proof}
...
%\be
%L(x) = 1 - 2\sum^\infty_{k=1} (-1)^{k-1} \exp\bb{-2k^2x^2} = \frac{\sqrt{2\pi}}{x} \sum^\infty_{k=1} \exp\bb{-\frac{(2k-1)^2\pi^2 }{8x^2}}.
%\ee
%Thus, it is easy
...
\be
\E R = \sqrt{\frac{8\sigma^2 t}{\pi}} \approx 1.5958 \sigma \sqrt{t},\qquad \E R^2 = (4\ln 2) \sigma^2 t,\qquad \var R = \bb{4\ln 2 - \frac{8}{\pi}}\sigma^2 t \approx 0.2181\sigma^2 t.
\ee

More generally, for any $p\in [1,\infty)$,
\be
\E R^p = \frac{4}{\sqrt{\pi}} \Gamma\bb{\frac {p+1}2}\bb{1-\frac{4}{2^p}}\zeta\bb{p-1}\bb{2\sigma^2 t}^{p/2}.
\ee
...
\be
\E R^3 = \frac {\bb{2\pi\sigma^2 t}^{3/2}}{3},\qquad \E R^5 = \frac {7\pi\bb{2\pi\sigma^2 t}^{5/2}}{90}.
\ee
\end{remark}
...
\be
L'(x) = 8\sum^\infty_{k=1}(-1)^{k-1} k^2 x \exp\bb{-2k^2x^2} \ \ra \ \frac{L'(x)}{x} = 8\sum^\infty_{k=1}(-1)^{k-1} k^2 \exp\bb{-2k^2x^2}.
\ee

%On the other hand, taking the differentiation of the second equation of Kolmogorov-Smirnov theorem, we have
%\be
%L'(x) = \frac{\sqrt{2\pi}}{x} \sum^\infty_{k=1} \frac{(2k-1)^2\pi^2}{4}x^{-3} \exp\bb{-\frac{(2k-1)^2\pi^2 }{8x^2}}.
%\ee

Thus, letting
\be
x = \frac{r}{2\sigma\sqrt{t}} \ \ra \ \frac{L'(x)}{x} = \sigma\sqrt{2\pi t} p_t(r) \ \ra \ p_t(r) =  \frac 1{\sigma\sqrt{2\pi t}} \frac {2\sigma\sqrt{t}}r L'\bb{ \frac{r}{2\sigma\sqrt{t}}} = \frac {2}{r\sqrt{2\pi }} L'\bb{ \frac{r}{2\sigma\sqrt{t}}}
\ee
...
\beast
\int^\infty_0 p_t(r)dr & = & \int^\infty_0 \frac {2}{r\sqrt{2\pi }} L'\bb{ \frac{r}{2\sigma\sqrt{t}}} dr = \frac {\sigma\sqrt{8t}}{\sqrt{\pi }}\int^\infty_0 r^{-1} d L\bb{ \frac{r}{2\sigma\sqrt{t}}}  \\
& = & \frac {\sigma\sqrt{8t}}{\sqrt{\pi }} \bb{r^{-1} L\bb{ \left.\frac{r}{2\sigma\sqrt{t}}}\right|^\infty_0 + \int^\infty_0  L\bb{ \frac{r}{2\sigma\sqrt{t}}}\frac 1{r^2}dr} = \frac {\sigma\sqrt{8t}}{\sqrt{\pi }} \int^\infty_0  L\bb{ \frac{r}{2\sigma\sqrt{t}}}\frac 1{r^2}dr \\
& = & \frac {\sigma\sqrt{8t}}{\sqrt{\pi }} \int^\infty_0\frac {\sqrt{2\pi} 2\sigma\sqrt{t}}r \sum^\infty_{k=1} \exp\bb{-\frac{(2k-1)^2\pi^2\sigma^2t }{2r^2}}\frac 1{r^2}dr =  8\sigma^2 t \int^\infty_0  \sum^\infty_{k=1} \exp\bb{-\frac{(2k-1)^2\pi^2\sigma^2t }{2r^2}}\frac 1{r^3}dr \\
& = & 8\sigma^2 t \sum^\infty_{k=1} \frac 1 {(2k-1)^2\pi^2\sigma^2t } \left.\exp\bb{-\frac{(2k-1)^2\pi^2\sigma^2t }{2r^2}}\right|^\infty_0 = 8 \sum^\infty_{k=1} \frac 1 {(2k-1)^2\pi^2} = 1
\eeast
by Corollary \ref{cor:sum_inverse_square_odd}. Similarly, we have that
\be
\E R = \int^\infty_0 r p_t(r)dr = \frac {\sigma\sqrt{8t}}{\sqrt{\pi }} \left.L\bb{ \frac{r}{2\sigma\sqrt{t}}}\right|^\infty_0 =  \frac {\sigma\sqrt{8t}}{\sqrt{\pi }} L\bb{ \frac{\infty}{2\sigma\sqrt{t}}} = \sqrt{\frac{8\sigma^2 t}{\pi}}.
\ee
...
\beast
\E R^2 & = & \int^\infty_0 r^2 p_t(r)dr = \frac{8}{\sigma\sqrt{2\pi t}} \sum^\infty_{k=1}(-1)^{k+1}k^2 \int^\infty_0 r^2  \exp\bb{-\frac{k^2r^2}{2\sigma^2 t}} dr \\
...
\beast
\E R^p & = & \int^\infty_0 r^p p_t(r)dr = \frac{8}{\sigma\sqrt{2\pi t}} \sum^\infty_{k=1}(-1)^{k+1}k^2 \int^\infty_0 r^p  \exp\bb{-\frac{k^2r^2}{2\sigma^2 t}} dr \\
& = & \frac{4}{\sigma\sqrt{2\pi t}} \sum^\infty_{k=1}(-1)^{k+1}k^2 \int^\infty_0 r^{(p-1)/2}  \exp\bb{-\frac{k^2r}{2\sigma^2 t}} dr \\
& = & \frac{4}{\sigma\sqrt{2\pi t}} \sum^\infty_{k=1}(-1)^{k+1}k^2 \frac{\Gamma\bb{\frac {p+1}2}}{\bb{\frac{k^2}{2\sigma^2 t}}^{(p+1)/2}} =\frac{4}{\sqrt{\pi }}  \Gamma\bb{\frac {p+1}2}  \bb{2\sigma^2 t}^{p/2} \sum^\infty_{k=1}(-1)^{k+1}k^{1-p}
...
\be
\zeta(p-1) - \sum^\infty_{k=1} (-1)^{k+1}k^{1-p} = 2\sum^\infty_{k=1} (2k)^{1-p} = 2^{2-p} \zeta(p-1)\ \ra \ \bb{1- \frac{4}{2^p}}\zeta(p-1) = \sum^\infty_{k=1} (-1)^{k+1}k^{1-p} .
\ee
...
%\be
%f(t) = \frac 1{\sqrt{2\pi \sigma^2 t^3}}\bb{\sum_{\substack{x=u,v\\ y=\pm 1}}\bb{x\exp\bb{-\frac{x^2}{2\sigma^2t}} + \sum^\infty_{k=1} y\bb{2k(u+v)+ xy}\exp\bb{-\frac{(2k(u+v)+xy)^2}{2\sigma^2 t}}}}.%\frac{8}{\sigma\sqrt{2\pi t}}\sum^\infty_{k=1}(-1)^{k+1}k^2 \exp\bb{-\frac{k^2r^2}{2\sigma^2 t}}.
%\ee
...
%\beast
%\pro\bb{T\geq t} & = & F_t(S_t \leq v, I_t \geq -u) \\
%& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty}  \int^v_{-u} \bb{\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}} dx.
%\eeast

%Then take the differentiation with respect to $t$, we have
%\beast
%-f(t) & = & -\frac 1{2\sqrt{2\pi \sigma^2 t^3}} \sum^\infty_{k=-\infty}  \int^v_{-u} \bb{\exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}} dx \\
% & & \qquad \frac 1{2\sqrt{2\pi \sigma^2 t^3}} \sum^\infty_{k=-\infty}  \int^v_{-u} \frac{(x-2ku - 2kv)^2}{\sigma^2 t} \exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} dx \\
% & & \qquad - \frac 1{2\sqrt{2\pi \sigma^2 t^3}} \sum^\infty_{k=-\infty}  \int^v_{-u} \frac{(x+2ku + 2(k-1)v)^2}{\sigma^2 t}\exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}} dx
%\eeast

%By integral by parts, we have
%\beast
%f(t) & = & \frac 1{\sqrt{8\pi \sigma^2 t^3}} \sum^\infty_{k=-\infty}\left.  (x-2ku - 2kv) \exp\bb{-\frac{(x-2ku - 2kv)^2}{2\sigma^2 t}} -(x+2ku + 2(k-1)v)\exp\bb{-\frac{(x+2ku + 2(k-1)v)^2}{2\sigma^2 t}}  \right|^v_{-u} \\
%& = & \frac 1{\sqrt{2\pi \sigma^2 t^3}} \sum^\infty_{k=-\infty}((2k+1)u + 2kv) \exp\bb{-\frac{((2k+1)u + 2kv)^2}{2\sigma^2 t}} - (2ku + (2k-1)v)\exp\bb{-\frac{(2ku + (2k-1)v)^2}{2\sigma^2 t}} \\
%& = & \frac 1{\sqrt{2\pi \sigma^2 t^3}}\bb{\sum_{\substack{x=u,v\\ y=\pm 1}}\bb{x\exp\bb{-\frac{x^2}{2\sigma^2t}} + \sum^\infty_{k=1} y\bb{2k(u+v)+ xy}\exp\bb{-\frac{(2k(u+v)+xy)^2}{2\sigma^2 t}}}}
%\eeast
...
%& = & \int^\infty_0 \frac {t^n}{\sqrt{2\pi \sigma^2 t^3}}\bb{\sum_{\substack{x=u,v\\ y=\pm 1}}\bb{x\exp\bb{-\frac{x^2}{2\sigma^2t}} + \sum^\infty_{k=1} y\bb{2k(u+v)+ xy}\exp\bb{-\frac{(2k(u+v)+xy)^2}{2\sigma^2 t}}}}\\
%& = & \frac{\Gamma\bb{\frac 12-n}\bb{2\sigma^2}^{1/2-n}}{\sqrt{2\pi \sigma^2}} \bb{\sum_{\substack{x=u,v\\ y=\pm 1}}\bb{{x}^{2n}+ \sum^\infty_{k=1} y \bb{2k(u+v)+ xy}^{2n}}}
%\eeast

%By Proposition \ref{pro:gamma_half}, $\Gamma\left(\frac{1}{2}-n\right) = {(-4)^n n! \over (2n)!} \sqrt{\pi}$, we have
%\be
%\E T^n = \frac{(-4)^n n!\bb{2\sigma^2}^{-n}}{(2n)!} \bb{\sum_{\substack{x=u,v\\ y=\pm 1}}\bb{x\exp\bb{-\frac{x^2}{2\sigma^2t}} + \sum^\infty_{k=1} y\bb{2k(u+v)+ xy}\exp\bb{-\frac{(2k(u+v)+xy)^2}{2\sigma^2 t}}}}\\
%\ee
%\end{proof}
...
Also,
\be
\pro\bb{T_x < \infty} = 1,\qquad \E T_x = \infty.
\ee
\end{proposition}
...
By Corollary \ref{cor:abs_sup_equal_brownian_motion}, we have%\footnote{see Rogers' book}
\be
\pro\bb{T_x\leq t} = \pro\bb{S_t \geq x} = \pro\bb{\abs{B_t} \geq x} = 2\int^\infty_x \frac{1}{\sqrt{2\pi \sigma^2 t}}e^{-\frac{y^2}{2\sigma^2 t}}dy = 2\int^\infty_{x/\sqrt{\sigma^2 t}} \frac{1}{\sqrt{2\pi }}e^{-\frac{y^2}{2}}dy.
\ee

Thus,
\be
\pro(T_x < \infty) = \lim_{t\ua\infty}\pro\bb{T_X\leq t} = 2\int^\infty_0 \frac{1}{\sqrt{2\pi }}e^{-\frac{y^2}{2}}dy = 1.
\ee

Then we take the differentiation,
\be
f_{T_x}(t) = -\frac{2}{\sqrt{2\pi}}e^{-\frac{x^2}{2\sigma^2t}}\bb{ -\frac 12 \frac {x}{\sigma\sqrt{t^3}}} = \frac{x e^{-\frac{x^2}{2\sigma^2t}}}{\sigma\sqrt{2\pi t^3}}
\ee
...
\be
\E\exp\bb{-r T_x} = \exp\bb{-\frac{x\sqrt{2r}}{\sigma}}.
\ee
%Moreover, the Laplace transform of $T_x =  \inf\bra{t \geq 0:B_t = x}$ for $x \in \R$ is given by
%\be
%\E\exp\bb{-rT_x} = \exp\bb{-\frac{\abs{x}\sqrt{2r}}{\sigma}},\qquad r\in\C.
%\ee
\end{proposition}
...
\be
\E X = \left.-\frac {d}{dr}\exp\bb{-\frac{x\sqrt{2r}}{\sigma}}\right|_{r=0} =- \infty.
\ee
...
\beast
\E\exp\bb{-r T_x} & = & \frac{x}{\sigma\sqrt{2\pi}}\int^\infty_0 e^{- r t} t^{-3/2}e^{-\frac{a^2}{2\sigma^2 t}}dt = \frac{x}{\sigma\sqrt{2\pi}}\int^\infty_0 t^{-3/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(\frac{\sigma t\sqrt{2r}}{x}+\frac{x}{\sigma t\sqrt{2r}}\right)\right\}dt \\
...
\int^\infty_0 t^{-3/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(t+\frac{1}{t}\right)\right\}dt  = \exp\bb{-\frac{x\sqrt{2r}}{\sigma}}\int^\infty_0 t^{-3/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(\sqrt{t}-\frac{1}{\sqrt{t}}\right)^2\right\}dt\quad (\dag)
\eeast

Defining $s:=1/t$, we have
\be%\label{equ:reflection_t_inverse}
\int^\infty_0 t^{-3/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(t+\frac{1}{t}\right)\right\}dt = \exp\bb{-\frac{x\sqrt{2r}}{\sigma}} \int^\infty_0 s^{-1/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(\sqrt{s}-\frac{1}{\sqrt{s}}\right)^2\right\}ds\quad (\dag\dag)
\ee

Then $\bb{(\dag)+(\dag\dag)}/2$ is
\beast
\int^\infty_0 t^{-3/2} \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(t+\frac{1}{t}\right)\right\}dt & = & \frac 12\exp\bb{-\frac{x\sqrt{2r}}{\sigma}} \int^\infty_0 \left(t^{-1/2}+ t^{-3/2}\right) \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(\sqrt{t}-\frac{1}{\sqrt{t}}\right)^2\right\}dt  \\
& = & \exp\bb{-\frac{x\sqrt{2r}}{\sigma}}\int^\infty_0 \exp\left\{-\frac{x\sqrt{2r}}{2\sigma}\left(\sqrt{t}-\frac{1}{\sqrt{t}}\right)^2\right\}d\left(\sqrt{t}-\frac{1}{\sqrt{t}}\right)  \\
& = & \exp\bb{-\frac{x\sqrt{2r}}{\sigma}} \sqrt{2\pi}\bb{\frac{x\sqrt{2r}}{\sigma}}^{-1/2}.
\eeast
...
%\beast
%\frac{x}{\sigma\sqrt{2\pi}}\int^\infty_0 e^{-r t} t^{-3/2}e^{-\frac{x^2}{2\sigma^2t}}dt & = & \int^\infty_0 \frac{x}{\sigma\sqrt{2\pi t^3}}\exp\bb{-r t - \frac{x^2}{2\sigma^2t}}dt \\
%& = & \int^\infty_0 \frac{x}{\sigma\sqrt{2\pi t^3}}\exp\bb{-\frac 1{2 t}\bb{\sqrt{2r} t - \frac{x}{\sigma}}^2 } \exp\bb{-\frac{x\sqrt{2r}}{\sigma}} dt \\
%& = & \exp\bb{-\frac{x\sqrt{2r}}{\sigma}} \int^\infty_0 \bb{\frac{x^2}{2\pi \sigma^2 t^3}}^{1/2}\exp\bb{-\frac {2r}{2t}\bb{t - \frac{x}{\sigma\sqrt{2r}}}^2 }  dt
%\eeast
%
%The term in the integral is the pdf of inverse Gaussian distribution $IG\bb{\frac{x}{\sigma\sqrt{2r}}, \frac{x^2}{\sigma^2}}$ where (see Definition \ref{def:inverse_gaussian_rv})
%\be
%X\sim IG\bb{\mu,\lm} \ \ra \ f(x) = \bb{\frac{\lambda}{2 \pi x^3}}^{1/2} \exp\bb{\frac{-\lambda (x-\mu)^2}{2 \mu^2 x}}.
%\ee

Thus,
\be
\E\exp\bb{-r T_x}  = \frac{x}{\sigma\sqrt{2\pi}} \left(\frac{x}{\sigma\sqrt{2r}}\right)^{-1/2} \exp\bb{-\frac{x\sqrt{2r}}{\sigma}}  \sqrt{2\pi}\bb{\frac{x\sqrt{2r}}{\sigma}}^{-1/2} = \exp\bb{-\frac{x\sqrt{2r}}{\sigma}}.
\ee
...
Now we can apply martingale theorem to prove Proposition \ref{pro:brownian_motion_hitting_time_single_barrier}. Recall that $T_x = \inf\bra{t:B_t = x}$ and $B$ is a Brownian motion with $B_t \sim \sN\bb{0,\sigma^2 t}$.

\begin{proof}[\bf Proof]%By Theorem \ref{thm:brownian_motion_martingale}, we have that $M_t=\exp\bb{ \theta \frac{ B_t}{\sigma}- \abs{\theta}^2t/2}$ is a martingale for $\theta := \sqrt{2r}$ where $r\in \R$. Since $M$ is bounded and therefore UI, we can apply optional stopping theorem (Theorem \ref{thm:optional_stopping_ui_continuous})
%\be
%1 = M_0 = \E\bb{M_{t \land T_x}}= \E\bb{\exp\left(\sqrt{2r} \frac{x}{\sigma}- r T_x\right)\ind_{\{T_x\leq t\}}} + \E\bb{\exp\left(\sqrt{2r}\frac{B_t}{\sigma}- rt\right)\ind_{\{T_x > t\}}}.
%\ee
%The second term here is bounded above by $\exp\bb{\sqrt{2r} \frac{x}{\sigma}}$ or $1$ (for two cases for the sign of $r$), so converges to zero as $t\to\infty$, and the first term converges to $\E\bb{\exp\left(\sqrt{2r} \frac{x}{\sigma}- rT_x\right)}$. Then we can get
%\be
%1 = \E\exp\bb{ \frac{x\sqrt{2r}}{\sigma}- r T_x} \ \ra\ \E\exp\bb{ - r T_x} = \exp\bb{ - \frac{x\sqrt{2r}}{\sigma}}
%\ee
%as required.

Without loss of generality, we can assume that $x\geq 0$ and $\lm = \sqrt{2r}$ with $r\in \C$. We know the fact that
\be
\bb{\exp\bb{\lm B_t/\sigma-\abs{\lm}^2t /2}}_{t\geq 0}
\ee
is a martingale by Theorem \ref{thm:brownian_motion_martingale}. Therefore,
\be
\bb{\exp\bb{\frac{\lm B_t^{T_x}}{\sigma}-\frac{\abs{\lm}^2}2(t\land T_x)}}_{t\geq 0}
\ee
...
\be
\E\bb{\exp\bb{\frac{\lm x}{\sigma}-\frac{\abs{\lm}^2}2 T_x}} = \E\bb{\exp\bb{\frac{\lm B_{T_x}}{\sigma}-\frac{\abs{\lm}^2}2 T_x}} = 1  \ \ra \ \exp\bb{-\frac{\lm x}{\sigma}} = \E\bb{\exp\bb{-\frac{\abs{\lm}^2 T_x}2}}.
\ee
...
\be
\E\bb{e^{-rT}} = \frac{\sinh(\sqrt{2r}x) + \sinh(\sqrt{2r}y)}{\sinh(\sqrt{2r}(x + y))}
\ee
...
\be
\sinh(\lm y) = M_0 = \E M_T = \E\bb{\sinh(\lm (B_T + y))e^{-\lm^2T/2}} = \E\bb{\sinh(\lm(x + y)e^{-\lm^2T/2}\ind_{\bra{T_x < T_{-y}}}}.
\ee

Thus,
\be
\E\bb{e^{-\lm^2T/2}\ind_{\bra{T_x <T_{-y}}}} = \frac{\sinh(\lm y)}{\sinh(\lm(x + y))} \ \ra \ \E\bb{e^{-\lm^2T/2}\ind_{\bra{T_x >T_{-y}}}} = \frac{\sinh(\lm x)}{\sinh(\lm (x + y))}
\ee
...
\be
\E\bb{e^{-rT}\ind_{\bra{T_x < T_{-y}}}} = \frac{\sinh(\sqrt{2r} y)}{\sinh\bb{\sqrt{2r}(x + y)}},\qquad \E\bb{e^{-rT}\ind_{\bra{T_x > T_{-y}}}} = \frac{\sinh(\sqrt{2r} x)}{\sinh\bb{\sqrt{2r} (x + y)}}.
\ee

Then by Proposition \ref{pro:laplace_finite_moment}%{pro:mgf_finite_moment}
\beast
& &\E \bb{T\ind_{\bra{T_x < T_{-y}}}} = -\left.\fp{\E\bb{e^{-rT}\ind_{\bra{T_x<T_{-y}}}}}{r}\right|_{r=0} \\
& = & \left.\frac{\sinh(\sqrt{2r} y) \frac{x+y}{\sqrt{2r}}\cosh\bb{\sqrt{2r}(x + y)}- \cosh(\sqrt{2r} y) \frac{y}{\sqrt{2r}}\sinh\bb{\sqrt{2r}(x + y)}}{\bb{\sinh\bb{\sqrt{2r}(x + y)}}^2}\right|_{r=0} \\
& = & \left.\frac{(x+y)\bb{y+ \frac 13ry^3+o(r)}\bb{1+ r(x+y)^2+o(r)}-y \bb{1+ry^2+o(r)}  \bb{(x+y)+ \frac 13r(x+y)^3+o(r) }}{\bb{2r(x+y)^2 + o(r)}^2}\right|_{r=0}\\
& = & \left.\frac{\frac 23 yr(x+y)\bb{(x+y)^2 - y^2} + o(r)}{\bb{\sqrt{2r}(x+y) + o(r)}^2}\right|_{r=0} = \frac{xy(x+2y)}{3(x+y)}.
\eeast

Similarly, we have
\be
\E \bb{T\ind_{\bra{T_x > T_{-y}}}} = \frac{xy(2x+y)}{3(x+y)}.
\ee

Therefore,
\beast
\E T & = & \E \bb{T\ind_{\bra{T_x < T_{-y}}}} + \E \bb{T\ind_{\bra{T_x > T_{-y}}}} \\
...
Thus,
\beast
\E \bb{TB_T^2 } & = & \E\bb{\E \bb{TB_T^2\ind_{\bra{T_x < T_{-y}}}} + \E \bb{\E\bb{TB_T^2\ind_{\bra{T_x > T_{-y}}}}}} \\
...
%and thus\footnote{}
\beast
\E T^2 & = & \frac 13\bb{6\E\bb{TB_T^2} - \bb{\frac{x^4y}{x+y}+ \frac{y^4x}{x+y}}} \\
& = & \frac 13\bb{2xy\bb{x^2+y^2 + xy}  - xy(x^2+y^2-xy)} = \frac 13 xy\bb{x^2 + y^2 + 3xy}.
\eeast

Hence,%Therefore,
\be
\var T = \frac 13 xy\bb{x^2 + y^2 + 3xy} - x^2y^2 = \frac 13 xy\bb{x^2+ y^2}.
\ee
\end{proof}
...
\be
\pro\bb{\int^1_0 \ind_{\bra{B_t > 0}}\leq x} = \frac 2{\pi}
\arcsin\sqrt{x}.
\ee
\end{theorem}
...
\be
\pro\bb{T\leq x} = \frac 2{\pi}\arcsin\sqrt{x}.
\ee
\end{theorem}
...
& = & \int_{[0,\infty)\times(0,\infty)} \frac 1C (y +z)\mu^+(dy)\mu^-(dz)\bb{f(y)\frac z{y + z} + f(-z)\frac y{y + z}}\\
...
\be
\E\bb{\inf\bra{t \geq 0 : B_t \in \{y,-z\}}} = yz,
\ee
so by a similar conditioning argument as above,
\beast
\E T_1 & = & \E\bb{\E\bb{T_1|Y_1,Z_1}} = \int_{[0,\infty)\times(0,\infty)} \frac 1C (y+ z)yz\mu^+(dy)\mu^-(dz)\\
...
\be
M_n := \max_{1\leq k\leq n}\bb{S_k- \mu k},\qquad \forall n\geq 1.
\ee
...
\be
\wt{S} := \bb{\frac{S_{Nt}}{\sqrt{\sigma^2N}}}_{0 \leq t \leq 1}
\ee
...
\be
\E(\Lambda(\wt{S}))\ \to\ \E\bb{\Lambda(B)}\quad\text{as }n\to\infty.
\ee
\end{theorem}
...
\be
\limsup_{N\to\infty} \pro\bb{\dabs{\wt{S}^{(N)} - B'}_\infty > \ve} = \limsup_{N\to\infty} \pro\bb{\sup_{0\leq t\leq 1}\abs{\wt{S}^{(N)}_t - B'_t} > \ve} \leq \pro(L_{2\delta,\ve})
\ee
...
\be
\limsup_{N\to\infty} \pro\bb{\dabs{\wt{S}^{(N)} - B'}_\infty > \ve} = \limsup_{N\to\infty} \pro\bb{\sup_{0\leq t\leq 1}\abs{\wt{S}^{(N)}_t - B'_t} > \ve} = 0.
\ee
...
\be
\E(\Lambda(\wt{S})) \to \E\bb{\Lambda(B)}
\ee
which means
\be
\E(h(\wt{S}_1)) \to \E\bb{h(B_1)} = \E\bb{h(X)}.
\ee
...
\be
\E\bsb{h\bb{\sup_{0\leq t\leq 1}\wt{S}_t}} = \E\bb{\Lambda (\wt{S})} \to \E\bb{\Lambda(B) } = \E\bsb{h\bb{\sup_{0\leq t\leq 1}B_t}}.
\ee
...
\be
\lim_{n\to\infty}\pro\bb{\max_{1\leq k\leq n}S_k \leq x\sigma\sqrt{n}} = \sqrt{\frac{2}{\pi}} \int^x_0 e^{-y^2/2}dy.
\ee

Similarly, redefine $\Lambda(f) := h(\inf_{0\leq t\leq 1}f(t))$. Then we can have
\be
\E\bb{h\bb{ \max_{0\leq k\leq n}\frac{S_k}{\sigma\sqrt{n}}}} \to \E\bb{h\bb{-\abs{\sN(0,1)}}}.
\ee

Thus, we have for $x\leq 0$
\be
\lim_{n\to\infty}\pro\bb{\min_{1\leq k\leq n}S_k\leq x\sigma\sqrt{n}} = \sqrt{\frac{2}{\pi}} \int^x_{-\infty} e^{-y^2/2}dy.
\ee

In fact, we can let $\alpha,\beta,\gamma\in \R$ be fixed, and define
\be
\Lambda(f) := h\bb{\alpha f(1) + \beta \sup_{0\leq t\leq 1}f(t)+ \gamma \inf_{0\leq t\leq 1}f(t)}.
\ee

Therefore, by Donsker's invariance principle,
\be
\E\bb{h\bb{\alpha \frac{S_n}{\sigma\sqrt{n}} + \beta \max_{1\leq k\leq n}\frac{S_k}{\sigma\sqrt{n}} + \gamma \min_{1\leq k\leq n}\frac{S_k}{\sigma\sqrt{n}}}} \to \E\bb{h\bb{\alpha B_1 + \beta \sup_{0\leq t\leq 1}B_t + \gamma\inf_{0\leq t\leq 1}B_t}}.
\ee
...
\be
\bb{\frac{S_n}{\sigma\sqrt{n}},\ \max_{1\leq k\leq n}\frac{S_k}{\sigma\sqrt{n}},\ \min_{1\leq k\leq n}\frac{S_k}{\sigma\sqrt{n}}} \to \bb{B_1,\  \sup_{0\leq t\leq 1}B_t,\ \inf_{0\leq t\leq 1}B_t}
\ee
...
\be
\E\bb{h\bb{\max_{1\leq k\leq n}\frac{\abs{S_k}}{\sigma\sqrt{n}}}} \to \E\bb{h\bb{\sup_{0\leq t\leq 1}\abs{B_t}}}
\ee
...
\be
\lim_{n\to\infty}\pro\bb{\max_{1\leq k\leq n}\abs{S_k} \leq x\sigma\sqrt{n}} = \frac 4{\pi}\sum^\infty_{k=1}\frac{(-1)^{k+1}}{2k-1} \exp\bb{-\frac{(2k-1)^2\pi^2}{8x^2}}.
\ee
...
\be
f(x) = \frac{4}{\sqrt{2\pi}}\sum^\infty_{k=1}(-1)^{k+1}(2k-1) \exp\bb{-\frac{(2k-1)^2 x^2 }{2}}.
\ee
\end{example}
...
\be
\frac 1n\sum^n_{k=1}f\bb{\frac{S_k}{\sigma\sqrt{n}}} \to \int^1_0 f(B_t)dt
\ee
...
\be
\lim_{n\to \infty}\pro\bb{\frac 1n\sum^n_{k=1}\ind_{\bra{S_k> 0}}\leq x} = \frac 2{\pi}\arcsin\sqrt{x}.
\ee
\end{proposition}
...
\beast
\E\bb{\exp\bb{\theta B_s|_{B_t}}} & \stackrel{\text{a.s.}}{=} & \E\bb{\left.\exp\bb{\theta B_s}\right| B_t} = \E\bb{\left.\exp\bb{\theta \bb{B_s - sB_t/t}}\exp\bb{\theta sB_t/t} \right|B_t} \quad \text{ by Proposition \ref{pro:conditional_probability_density_function}} \\
& \stackrel{\text{a.s.}}{=} & \exp\bb{\theta sB_t/t} \E\bb{\left.\exp\bb{\theta \bb{B_s - sB_t/t}}\right|B_t} \quad \text{by Proposition \ref{pro:conditional_expectation_tower_independence}.(ii)}\\
& \stackrel{\text{a.s.}}{=} & \exp\bb{\theta sB_t/t} \E\bb{\exp\bb{\theta \bb{B_s - sB_t/t}}} \quad \text{by Lemma \ref{lem:brownian_bridge_independent_of_brownian_motion} and Proposition \ref{pro:conditional_expectation_tower_independence}.(v)}\\
& = & \exp\bb{\theta sB_t/t} \E\bb{\exp\bb{\theta \bb{\frac{t-s}{t}B_s - \frac st B_{t-s}}}} \\
& = & \exp\bb{\theta sB_t/t} \E\bb{\exp\bb{\frac{t-s}{t}\theta B_s}}\E\bb{\exp\bb{-\frac st \theta B_{t-s}}}\quad \text{by Proposition \ref{pro:independent_mgf}}\\
& = & \exp\bb{\theta sB_t/t} \exp\bb{\frac{\theta^2 (t-s)^2 s}{2t^2}} \exp\bb{\frac {\theta^2 s^2(t-s) }{2t^2}} = \exp\bb{\frac{\theta sB_t}t} \exp\bb{\frac{\theta^2 s(t-s)}{2t}}.
\eeast
...
\be
\cov\bb{B_s|_{B_u},B_t|_{B_u}} = \rho \sigma_x \sigma_y = \sqrt{\frac{s(u-t)}{t(u-s)}}\sqrt{\frac{s(u-s)}{u}} \sqrt{\frac{t(u-t)}{u}} = \frac{s(u-t)}{u}
\ee
as required.
\end{proof}
...
\be
f_t(x) = \frac{1}{\sqrt{2\pi \sigma^2 t}}\exp\bb{-\frac{(x-\mu t)^2}{2\sigma^2 t}}.
\ee
...
\be
f_t(y) = 2\exp\bb{\frac{2\mu y}{\sigma^2}} \bb{\frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(y+\mu t)^2}{2\sigma^2t}} - \frac {\mu }{\sigma^2 }\bb{1 - N\bb{\frac{y+\mu t}{\sigma\sqrt{t}}}}},\quad y\geq 0
\ee%\be
%f_t(y) =  \sqrt{\frac{2}{\pi\sigma^2 t}} \exp\bb{-\frac {y^2}{2\sigma^2 t}} ,\quad f_t(z) =  \sqrt{\frac{2}{\pi\sigma^2 t}} \exp\bb{-\frac {z^2}{2\sigma^2 t}} \quad \text{where }y>0,z<0.
%\ee

\be
f_t(z) = 2\exp\bb{\frac{2\mu z}{\sigma^2}} \bb{\frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(z+\mu t)^2}{2\sigma^2t}} + \frac {\mu }{\sigma^2 }N\bb{\frac{z+\mu t}{\sigma\sqrt{t}}}},\quad z\leq 0
\ee
where
\be
N(x) = \frac 1{\sqrt{2\pi}}\int^x_{-\infty}\exp\bb{-\frac {s^2}2}ds.
\ee

Also, for $a\geq 0$,
\be
\pro\bb{Y_t \leq a} = N\bb{\frac{a-\mu t}{\sigma\sqrt{t} }} - \exp\bb{\frac{2\mu a}{\sigma^2}}N\bb{-\frac{a+\mu t}{\sigma\sqrt{t} }},
\ee

\be
\pro\bb{Z_t \geq -a} = N\bb{\frac{a+\mu t}{\sigma\sqrt{t} }} - \exp\bb{-\frac{2\mu a}{\sigma^2}}N\bb{-\frac{a-\mu t}{\sigma\sqrt{t} }}.
\ee

This implies that
\be
\pro\bb{\max_{t\geq 0}X_t \leq a} = \left\{\ba{ll}
\exp\bb{\frac{2\mu a}{\sigma^2}}\quad\quad  & \mu < 0 \\
0 & \mu \geq 0
\ea\right.
\ee
\end{proposition}
...
If $\mu = 0$, we have
\be
f_t(y) = \sqrt{\frac{2}{\pi\sigma^2 t}}\exp\bb{-\frac{y^2}{2\sigma^2t}},\qquad f_t(z) = \sqrt{ \frac{2}{\pi\sigma^2 t}}\exp\bb{-\frac{z^2}{2\sigma^2t}}
\ee
...
%\pro\bb{Y_t \leq y} = \pro\bb{\sigma S_t + \mu t \leq y} = \pro\bb{S_t \leq \frac{y-\mu t}{\sigma}} = \frac{2}{\sqrt{2\pi}}\int^{\frac{y-\mu t}{\sigma}}_0 \exp\bb{-\frac {x^2}2} dx
%\ee
...
we have
\beast
f_t(x,y) & = & \exp\bb{\frac{\mu}{\sigma} \frac x{\sigma} - \frac 12 \bb{\frac{\mu}{\sigma}}^2 t }\frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{1}{2\sigma^2t}(2y-x)^2} = \frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(2y-x)^2-2\mu t x + \mu^2 t^2}{2\sigma^2t}} \\
& = & \frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{\frac{2\mu y}{\sigma^2}}\exp\bb{-\frac{(2y-x+\mu t)^2}{2\sigma^2t}}.
\eeast

Thus, we have that the transition density of $X$ is
\beast
f_t(x) & = & \int^\infty_{x}  \frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(2y-x)^2-2\mu t x + \mu^2 t^2}{2\sigma^2t}} dy = \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \int^\infty_{2x}  \frac{(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(2y-x)^2}{2\sigma^2t}} d2y \\
& = & \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \int^\infty_{x}  \frac{y}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{y^2}{2\sigma^2t}} dy = \frac{1}{\sigma\sqrt{2\pi t}} \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \left.\exp\bb{-\frac{y^2}{2\sigma^2t}}\right|^x_\infty \\
& = &  \frac{1}{\sigma\sqrt{2\pi t}} \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \exp\bb{-\frac{x^2}{2\sigma^2t}} = \frac{1}{\sqrt{2\pi \sigma^2t}} \exp\bb{-\frac{(x-\mu t )^2}{2\sigma^2t}}
\eeast
which is consistent with the assumption. Furthermore, we have
\beast
f_t(y) & = & \int^{y}_{-\infty}  \frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{\frac{2\mu y}{\sigma^2}}\exp\bb{-\frac{(2y-x+\mu t)^2}{2\sigma^2t}} dx = 2\exp\bb{\frac{2\mu y}{\sigma^2}} \int_{y}^{\infty}  \frac{x}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(x+\mu t)^2}{2\sigma^2t}} dx \\
& = & 2\exp\bb{\frac{2\mu y}{\sigma^2}} \bb{\int_{y}^{\infty}  \frac{x+\mu t}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(x+\mu t)^2}{2\sigma^2t}} dx - \int_{y}^{\infty}  \frac{\mu t}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(x+\mu t)^2}{2\sigma^2t}} dx}\\
& = & 2\exp\bb{\frac{2\mu y}{\sigma^2}} \bb{\left.\frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(x+\mu t)^2}{2\sigma^2t}} \right|^{y}_{\infty} - \frac {\mu t}{\sigma^2 t}\int_{\frac{y+\mu t}{\sigma\sqrt{t}}}^{\infty}  \frac{1}{\sqrt{2\pi }}\exp\bb{-\frac{x^2}{2}} dx} \\
& = & 2\exp\bb{\frac{2\mu y}{\sigma^2}} \bb{\frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(y+\mu t)^2}{2\sigma^2t}} - \frac {\mu }{\sigma^2 }\bb{1 - N\bb{\frac{y+\mu t}{\sigma\sqrt{t}}}}}.
\eeast
...
For $a\geq 0$,
\beast
\pro(Y_t \leq a) & = & \int^a_0 2\exp\bb{\frac{2\mu y}{\sigma^2}} \bb{\frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(y+\mu t)^2}{2\sigma^2t}} - \frac {\mu }{\sigma^2 }\bb{1 - N\bb{\frac{y+\mu t}{\sigma\sqrt{t}}}}} dy \\
& = & 2\int^a_0 \frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(y-\mu t)^2}{2\sigma^2t}} dy  - \frac {2\mu }{\sigma^2 }\int^a_0 \exp\bb{\frac{2\mu y}{\sigma^2}} \frac 1{\sqrt{2\pi}}\int^\infty_{\frac {y+\mu t}{\sigma\sqrt{t}}} \exp\bb{-\frac {x^2}2} dx dy\\
& = & 2\int^a_0 \frac{1}{\sqrt{2\pi\sigma^2 t}}\exp\bb{-\frac{(y-\mu t)^2}{2\sigma^2t}} dy\\
 & & \qquad - \frac {2\mu }{\sigma^2 }\frac 1{\sqrt{2\pi}}\bb{\int^\infty_{\frac{a+\mu t}{\sigma\sqrt{t}}} \exp\bb{-\frac{x^2}2}\int^a_0 \exp\bb{\frac{2\mu y}{\sigma^2}} dy dx + \int^\frac{a+\mu t}{\sigma\sqrt{t}}_{\frac{\mu t}{\sigma\sqrt{t}}} \exp\bb{-\frac{x^2}2}\int^{x\sigma\sqrt{t}-\mu t}_0 \exp\bb{\frac{2\mu y}{\sigma^2}} dy dx}
\eeast

The second term is
\beast
& & \frac 1{\sqrt{2\pi}}\bb{\int^\infty_{\frac{a+\mu t}{\sigma\sqrt{t}}} \left.\exp\bb{-\frac{x^2}2}\exp\bb{\frac{2\mu y}{\sigma^2}} \right|^a_0 dx + \int^\frac{a+\mu t}{\sigma\sqrt{t}}_{\frac{\mu t}{\sigma\sqrt{t}}} \exp\bb{-\frac{x^2}2}\left. \exp\bb{\frac{2\mu y}{\sigma^2}} \right|^{x\sigma\sqrt{t}-\mu t}_0 dx} \\
& = & \bb{\exp\bb{\frac{2\mu a}{\sigma^2}} -1}\bb{1-N\bb{\frac{a+\mu t}{\sigma\sqrt{t}}}} + \frac 1{\sqrt{2\pi}}\bb{\int^{\frac{a+\mu t}{\sigma\sqrt{t}}}_{\frac{\mu t}{\sigma\sqrt{t}}} \exp \bb{-\frac{\bb{\sigma x \sqrt{t}- 2\mu t}^2}{2\sigma^2t}}- \exp\bb{-\frac{x^2}2} dx} \\
& = & \bb{\exp\bb{\frac{2\mu a}{\sigma^2}} -1}\bb{1-N\bb{\frac{a+\mu t}{\sigma\sqrt{t}}}} + \bb{N\bb{\frac{a-\mu t}{\sigma\sqrt{t}}}-N\bb{\frac{-\mu t}{\sigma\sqrt{t}}} - \bb{N\bb{\frac{a+\mu t}{\sigma\sqrt{t}}}-N\bb{\frac{\mu t}{\sigma\sqrt{t}}}} }
\eeast
...
Moreover, we have
\beast
f(x,Y_t\leq a) & = & \int^a_{x}  \frac{2(2y-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{(2y-x)^2-2\mu t x + \mu^2 t^2}{2\sigma^2t}} dy \\
& = & \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \int^{2a-x}_{x}  \frac{y}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{y^2}{2\sigma^2t}} dy = \frac{1}{\sigma\sqrt{2\pi t}} \exp\bb{\frac{2\mu t x - \mu^2 t^2}{2\sigma^2t}} \left.\exp\bb{-\frac{y^2}{2\sigma^2t}}\right|^x_{2a-x} \\
& = &  \frac{1}{\sqrt{2\pi \sigma^2t}} \exp\bb{-\frac{(x-\mu t )^2}{2\sigma^2t}} \bb{1- \exp\bb{\frac{2a(x-a)}{\sigma^2t}}}.
\eeast
...
\be
f_t(x,y,z) = 4 \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty}  k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{x-2y+2k(y-z)}.
\ee
where
\be
\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{x^2}{2\sigma^2 t}}.
...
Also,
\be
\pro\bb{Y_t \leq y,Z_t \geq z} = \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum_{k=1}^\infty \frac{2k\pi\sigma^4 \bb{\exp\bb{\frac{\mu y}{\sigma^2}} - (-1)^k \exp\bb{\frac{\mu z}{\sigma^2}}} }{k^2\pi^2\sigma^4  + \mu^2(y-z)^2}\exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}}\sin\frac{k\pi y}{y-z}.
\ee
\end{theorem}
...
If $\mu = 0$, then
\be
\pro\bb{Y_t \leq y,Z_t \geq z} = \sum_{k=1}^\infty \frac{2\bb{1 - (-1)^k }}{k\pi }\exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}}\sin\frac{k\pi y}{y-z}.
\ee
\end{remark}
...
\be
f_{X_t}(x,Y_t \leq y,Z_t \geq z) = \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x + 2kz - 2ky)^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x -2kz + 2(k-1)y)^2}{2\sigma^2 t}}}.
\ee
...
& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}}}.
\eeast

Then
\beast
& & f_{X_t,Z_t}(x,Y_t \leq y, z) \\
& = & - \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \times \\
& &  \sum^\infty_{k=-\infty} \frac{-2k(x-2k(y-z))}{\sigma^2 t}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} - \frac{2k(x-2y+2k(y-z))}{\sigma^2 t} \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}} \\
& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \times \\
& &  \sum^\infty_{k=-\infty} \frac{2k(x-2k(y-z))}{\sigma^2 t}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} + \frac{2k(x-2y+2k(y-z))}{\sigma^2 t} \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}}
\eeast
...
& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} \bb{\frac{4k^2(x-2k(y-z))^2}{\sigma^4 t^2} - \frac{4k^2}{\sigma^2 t}}\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} \\
& & + \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} \bb{ \frac{4k(k-1)}{\sigma^2 t} - \frac{4k(k-1)(x-2y+2k(y-z))^2}{\sigma^4 t^2} }\exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}} \\
& = & 4 \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty}  k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{(x-2y+2k(y-z)}.
\eeast
where $\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{x^2}{2\sigma^2 t}}$.

Integrating $f_{X_t}(x,Y_t \leq y,Z_t \geq z)$, we have
\beast
& & \pro\bb{Y_t \leq y,Z_t \geq z} \\
& = & \int^y_z \frac 1{\sqrt{2\pi \sigma^2 t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} \bb{\exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} - \exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}}} dx. %\\& = & \frac 1{\sqrt{2\pi \sigma^2 t}} \sum^\infty_{k=-\infty}  \int^y_z  \exp\bb{- \frac{2k(y-z)\mu t }{\sigma^2}} \exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} - \exp\bb{- \frac{(2y - 2k(y-z))\mu t}{\sigma^2}}\exp\bb{-\frac{(x-2y+2k(y-z))^2}{2\sigma^2 t}} dx \\
\eeast%by Fubini theorem (Theorem \ref{thm:fubini}).

Thus, for the first term, we have
\be
I = \sum^\infty_{k=-\infty} \exp\bb{-\frac{(x-2k(y-z))^2}{2\sigma^2 t}} = \exp\bb{-\frac{x^2}{2\sigma^2 t}} \sum^\infty_{k=-\infty} \exp\bb{-\frac{2k^2(y-z)^2}{\sigma^2 t}}\exp\bb{\frac{2kx(y-z))}{\sigma^2 t}}
\ee
...
\beast
I &= & \exp\bb{-\frac{x^2}{2\sigma^2 t}}  (-i \tau)^{-1/2} \exp\bb{-\frac{\pi}{\tau} i z^2 }  \sum_{k=-\infty}^\infty \exp \bb{-\pi i k^2 \frac{1}{\tau} + 2 \pi i k  \frac{z}{\tau}} \\
& = & \exp\bb{-\frac{x^2}{2\sigma^2 t}} \sqrt{\frac{\pi\sigma^2 t}{2(y-z)^2}} \exp\bb{\frac{x^2}{2\sigma^2 t}}  \sum_{k=-\infty}^\infty \exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2} - \frac{ \pi i k x}{y-z}}\\
& = & \sqrt{\frac{\pi\sigma^2 t}{2(y-z)^2}} \sum_{k=-\infty}^\infty \exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}} \cos \bb{\frac{\pi k x}{y-z}}
\eeast

Since we can have the similar result for the second term, we have
\beast
\pro\bb{Y_t \leq y,Z_t \geq z} = \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \int^y_z \frac{1}{y-z} \sum_{k=1}^\infty \exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}} \exp\bb{\frac{\mu x}{\sigma^2} }\bb{\cos \bb{\frac{\pi k x}{y-z}} - \cos \bb{\frac{\pi k (x-2y)}{y-z}}}dx
\eeast

Then by Proposition \ref{pro:integral_exponential_cosine}, we have that%and Fubini theorem (Theorem \ref{thm:fubini})
\beast
\int^y_z \exp\bb{\frac {\mu x}{\sigma^2}}\cos \bb{\frac{\pi k x}{y-z}} dx= \bb{\left. \frac {1}{\bb{\frac {\mu }{\sigma^2}}^2 + \bb{\frac{\pi k}{y-z}}^2}\bb{\frac{\pi k}{y-z} \exp\bb{\frac {\mu x}{\sigma^2}}\sin\bb{\frac{\pi k x}{y-z}} + \frac {\mu }{\sigma^2} \exp\bb{\frac {\mu x}{\sigma^2}} \cos\bb{\frac{\pi k x}{y-z}}}\right|^y_z},
\eeast

Also,
\beast
& & \int^y_z \exp\bb{\frac {\mu x}{\sigma^2}}\cos \bb{\frac{\pi k (x-2y)}{y-z}} dx = \exp\bb{\frac {2\mu y}{\sigma^2}} \int^{-y}_{z-2y} \exp\bb{\frac {\mu x}{\sigma^2}}\cos \bb{\frac{\pi k x}{y-z}} dx \\
& = & \exp\bb{\frac {2\mu y}{\sigma^2}}  \bb{\left. \frac {1}{\bb{\frac {\mu }{\sigma^2}}^2 + \bb{\frac{\pi k}{y-z}}^2}\bb{\frac{\pi k}{y-z} \exp\bb{\frac {\mu x}{\sigma^2}}\sin\bb{\frac{\pi k x}{y-z}} + \frac {\mu }{\sigma^2} \exp\bb{\frac {\mu x}{\sigma^2}} \cos\bb{\frac{\pi k x}{y-z}}}\right|^{-y}_{z-2y}}
\eeast

Thus,
\beast
& &\int^y_z \exp\bb{\frac {\mu x}{\sigma^2}}\bb{\cos \bb{\frac{\pi k x}{y-z}} -\cos \bb{\frac{\pi k (x-2y)}{y-z}}} dx \\
& = & \frac {\frac{2\pi k}{y-z}}{\bb{\frac {\mu }{\sigma^2}}^2 + \bb{\frac{\pi k}{y-z}}^2}\bb{\exp\bb{\frac {\mu y}{\sigma^2}}\sin\bb{\frac{\pi k y}{y-z}} - \exp\bb{\frac {\mu z}{\sigma^2}}\sin\bb{\frac{\pi k z}{y-z}}}\\
& = & \frac {2\pi k(y-z)\sigma^4}{\mu^2(y-z)^2  + k^2 \pi^2 \sigma^4}\bb{\exp\bb{\frac {\mu y}{\sigma^2}}\sin\bb{\frac{\pi k y}{y-z}} - \exp\bb{\frac {\mu z}{\sigma^2}}\sin\bb{\frac{\pi k z}{y-z}}}.
\eeast

Then
\beast
& & \pro\bb{Y_t \leq y,Z_t \geq z} \\
& = & \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum_{k=1}^\infty  \frac {2\pi k\sigma^4}{\mu^2(y-z)^2  + k^2 \pi^2 \sigma^4} \exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}} \bb{\exp\bb{\frac {\mu y}{\sigma^2}}\sin\bb{\frac{\pi k y}{y-z}} - \exp\bb{\frac {\mu z}{\sigma^2}}\sin\bb{\frac{\pi k z}{y-z}}} \\
& = & \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum_{k=1}^\infty  \frac {2\pi k\sigma^4\bb{\exp\bb{\frac {\mu y}{\sigma^2}} - (-1)^k \exp\bb{\frac {\mu z}{\sigma^2}}}}{\mu^2(y-z)^2  + k^2 \pi^2 \sigma^4} \exp \bb{- \frac{k^2\pi^2 \sigma^2 t}{2(y-z)^2}} \sin\bb{\frac{\pi k y}{y-z}}
\eeast
as required.
\end{proof}
%
...
%\be
%f_{X_t,Y_t,Z_t}(x,y,z) = 4 \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty}  k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{(x-2y+2k(y-z)}.
%\ee
%where $\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{-x^2}{2\sigma^2 t}}$. Thus, we calculate
%\beast
%& & \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \int^y_z \exp\bb{\frac{\mu x}{\sigma^2}}\phi\bb{x-a}dx \\
%& = & \frac 1{\sigma^5\sqrt{2\pi t^5}}\exp\bb{\frac{\mu a}{\sigma^2}}  \int^y_z \bb{(x-a)^2 - \sigma^2 t} \exp\bb{-\frac{(x-a-\mu t)^2}{2\sigma^2 t}}dx \\
%& = & \frac 1{\sigma^5\sqrt{2\pi t^5}}\exp\bb{\frac{\mu a}{\sigma^2}}  \int^y_z \bb{(x-a-\mu t)^2 + 2\mu t(x-a-\mu t) + \mu^2t^2 - \sigma^2 t} \exp\bb{-\frac{(x-a-\mu t)^2}{2\sigma^2 t}}dx \\
%& = & \frac 1{\sigma^5\sqrt{2\pi t^5}}\exp\bb{\frac{\mu a}{\sigma^2}}  \bb{\left.\sigma^2t (x-a+\mu t)\exp\bb{-\frac{(x-a-\mu t)^2}{2\sigma^2 t}}\right|^z_y + \mu^2t^2 \int^y_z\exp\bb{-\frac{(x-a-\mu t)^2}{2\sigma^2 t}}dx}
%\eeast
%
%Then we can have
%\be
%\exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \int^y_z \exp\bb{\frac{\mu x}{\sigma^2}}\phi\bb{x-a}dx = \frac 1{\sigma^4t^2} \bb{\varphi\bb{z-a} \exp\bb{\frac{\mu z}{\sigma^2}} - \varphi\bb{y-a}\exp\bb{\frac{\mu y}{\sigma^2}} }
%\ee
%where
%\beast
%\varphi(x) & = & \exp\bb{-\frac{\mu x}{\sigma^2}}\bb{\sqrt{\frac{\sigma^2t}{2\pi}} (x+\mu t)\exp\bb{-\frac{(x-\mu t)^2}{2\sigma^2 t}} - \mu^2t^2 N\bb{\frac{x-\mu t}{\sigma\sqrt{t}}}} \\
%& = & \sqrt{\frac{\sigma^2t}{2\pi}} (x+\mu t)\exp\bb{-\frac{x^2+ \mu^2 t^2}{2\sigma^2 t}} - \mu^2t^2 \exp\bb{-\frac{\mu x}{\sigma^2}} N\bb{\frac{x-\mu t}{\sigma\sqrt{t}}}.
%\eeast
%
%Thus,
%\beast
%f_{Y_t,Z_t}(y,z) & = & \frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k^2\bb{\varphi\bb{z-2k(y-z)} \exp\bb{\frac{\mu z}{\sigma^2}} - \varphi\bb{y-2k(y-z)}\exp\bb{\frac{\mu y}{\sigma^2}} } \\
%& & \qquad -\frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k(k-1)\bb{\varphi\bb{z-2y+2k(y-z)} \exp\bb{\frac{\mu z}{\sigma^2}} - \varphi\bb{y-2y+2k(y-z)}\exp\bb{\frac{\mu y}{\sigma^2}} }
%\eeast
%
%Let $r = y-z$ and $z= y-r$. We have
%\beast
%f_{Y_t,Z_t}(y,r-y) & = & \frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k^2\bb{\varphi\bb{y-(2k+1)r} \exp\bb{\frac{\mu (y-r)}{\sigma^2}} - \varphi\bb{y-2kr}\exp\bb{\frac{\mu y}{\sigma^2}} } \\
%& & \qquad -\frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k(k-1)\bb{\varphi\bb{-y+(2k-1)r} \exp\bb{\frac{\mu (y-r)}{\sigma^2}} - \varphi\bb{-y+2kr}\exp\bb{\frac{\mu y}{\sigma^2}} }
%\eeast
%
%Next, let $\vp(x) = \vp_1(x) + \vp_2(x) + \vp_3(x)$ where
%\beast
%\vp_1(x) = \sqrt{\frac{\sigma^2t}{2\pi}} x \exp\bb{-\frac{x^2+ \mu^2 t^2}{2\sigma^2 t}},\  \vp_2(x) = \sqrt{\frac{\sigma^2t}{2\pi}} \mu t\exp\bb{-\frac{x^2+ \mu^2 t^2}{2\sigma^2 t}},\  \vp_3(x) =  - \mu^2t^2 \exp\bb{-\frac{\mu x}{\sigma^2}} N\bb{\frac{x-\mu t}{\sigma\sqrt{t}}}.
%\eeast
%
...
%\be
%g_1(y) := \frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k(2k+1)\varphi_1\bb{y-(2k+1)r} \exp\bb{\frac{\mu (y-r)}{\sigma^2}} - k(2k-1)\varphi_1\bb{y-2kr}\exp\bb{\frac{\mu y}{\sigma^2}}
%\ee
%and
%\be
%g_2(y) := \frac 4{\sigma^4t^2} \sum_{k=-\infty}^\infty k \bb{\varphi_2\bb{y-2kr}\exp\bb{\frac{\mu y}{\sigma^2}} - \varphi_2\bb{y-(2k+1)r} \exp\bb{\frac{\mu (y-r)}{\sigma^2}}}.
%\ee
%
%Thus,
%\beast
%& & f_1(r) = \int^r_0 g_1(y)dy \\
%& = & \sum_{k=-\infty}^\infty \frac 4{\sqrt{2\pi \sigma^6 t^3}} \exp\bb{\frac{2kr \mu }{\sigma^2 }} k(2k+1) \int^r_0 \bb{y-(2k+1)r} \exp\bb{-\frac{\bb{y-(2k+1)r-\mu t}^2}{2\sigma^2 t}} dy\\
%& & \qquad - \sum_{k=-\infty}^\infty\frac 4{\sqrt{2\pi \sigma^6 t^3}} \exp\bb{\frac{2kr \mu }{\sigma^2 }} k(2k-1)\int^r_0 \bb{y-2kr}  \exp\bb{-\frac{\bb{y-2kr-\mu t}^2}{2\sigma^2 t}} dy \\
%& = & \sum_{k=-\infty}^\infty \frac {4\exp\bb{\frac{2kr \mu }{\sigma^2 }}}{\sqrt{2\pi \sigma^2 t}}   k(2k+1) \bb{\left.\exp\bb{-\frac{\bb{y-(2k+1)r-\mu t}^2}{2\sigma^2 t}}\right|^0_r  + \mu \sigma^2t^2 \int^r_0 \exp\bb{-\frac{\bb{y-(2k+1)r-\mu t}^2}{2\sigma^2 t}} dy}\\
%& &  - \sum_{k=-\infty}^\infty \frac {4\exp\bb{\frac{2kr \mu }{\sigma^2 }}}{\sqrt{2\pi \sigma^2 t}}  k(2k-1)\bb{\left. \exp\bb{-\frac{\bb{y-2kr-\mu t}^2}{2\sigma^2 t}} \right|^0_r + \mu \sigma^2t^2 \int^r_0  \exp\bb{-\frac{\bb{y-2kr-\mu t}^2}{2\sigma^2 t}} dy}
%\eeast
%
%\end{proof}
...
\be
f_t(x,R_t <r) = \frac 1{\sigma\sqrt{2\pi t}} \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} \bb{2k+1 - \frac {2k}{\sigma^2 t}\bb{r - \abs{x}}\bb{\abs{x} + 2kr}} \exp\bb{-\frac{\bb{\abs{x} + 2kr}^2}{2\sigma^2 t}}.
\ee
\end{lemma}
...
where $\phi(x) = \frac{x^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{x^2}{2\sigma^2 t}}$. If $x>0$, we have that\footnote{theorem needed.}

\be
f_t(x,r) = \int^r_x f_t(x,y,r-y)dy.
\ee

Ignoring $4 \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}}$, we calculate the integral as it is

\beast
& & \sum^\infty_{k=-\infty}\int^r_x k^2 \phi\bb{x-2k(y-z)} - k(k-1)\phi\bb{x-2y+2k(y-z)} dy \\
& = & \sum^\infty_{k=-\infty} \int^r_x k^2 \frac{\bb{x-2kr}^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x-2kr}^2}{2\sigma^2 t}}  - k(k-1)\frac{\bb{x-2y+2kr}^2 - \sigma^2 t}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x-2y+2kr}^2}{2\sigma^2 t}} dy \\
& = & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x-2kr}^2(r-x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x-2kr}^2}{2\sigma^2 t}} - \frac{k^2(r-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x-2kr}^2}{2\sigma^2 t}} \\
& & \qquad \qquad - \left.  \frac{k(k-1)\bb{x-2y+2kr} }{2\sigma^3\sqrt{2\pi t^3}} \exp\bb{-\frac{\bb{x-2y+2kr}^2}{2\sigma^2 t}} \right|^r_x.
\eeast

Changing the sign of $k$, we have the summation is
\beast
& & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x+2kr}^2(r-x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} - \frac{k^2(r-x)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} \\
& & \qquad \qquad + \frac{k(k-1)\bb{-x+2kr} }{2\sigma^3\sqrt{2\pi t^3}} \exp\bb{-\frac{\bb{-x+2kr}^2}{2\sigma^2 t}} - \frac{k(k-1)\bb{x+2(k-1)r} }{2\sigma^3\sqrt{2\pi t^3}} \exp\bb{-\frac{\bb{x+2(k-1)r}^2}{2\sigma^2 t}}\\
&= & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x+2kr}^2(r-x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} - \frac{k^2(r-x)+ k(k+1)(x+2kr)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} \\
&= & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x+2kr}^2(r-x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} - \frac{k\bb{x + 2k^2r + 3kr}}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}}.
\eeast
...
& & \qquad \qquad + \frac{k(k-1)\bb{x+2kr} }{2\sigma^3\sqrt{2\pi t^3}} \exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} - \frac{k(k-1)\bb{-x+2(k-1)r} }{2\sigma^3\sqrt{2\pi t^3}} \exp\bb{-\frac{\bb{-x+2(k-1)r}^2}{2\sigma^2 t}}\\
&= & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x+2kr}^2(r+x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} - \frac{k^2(r+x)- k(k-1)(x+2kr)}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x+2kr}^2}{2\sigma^2 t}} \\
&= & \sum^\infty_{k=-\infty}   \frac{k^2\bb{x-2kr}^2(r+x)}{\sigma^5\sqrt{2\pi t^5}}\exp\bb{-\frac{\bb{x-2kr}^2}{2\sigma^2 t}} - \frac{k\bb{-x  + 2k^2 r + 3kr}}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{x-2kr}^2}{2\sigma^2 t}}.
\eeast

Thus, we have that
\beast
f_t(x,r) = 4 \exp\bb{\frac{\mu x}{\sigma^2} - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty}  \bb{ \frac{k^2\bb{\abs{x}+2kr}^2(r-\abs{x})}{\sigma^5\sqrt{2\pi t^5}} - \frac{k\bb{\abs{x} + 2k^2r + 3kr}}{\sigma^3\sqrt{2\pi t^3}}}\exp\bb{-\frac{\bb{\abs{x}+2kr}^2}{2\sigma^2 t}}.
\eeast

To get $f_t\bb{x,R<r} = \int^r_{\abs{x}} f_t(x,s) ds$, we calculate the integral

\beast
& & \int^r_{\abs{x}} \frac{k^2\bb{\abs{x}+2ks}^2(s-\abs{x})}{\sigma^5\sqrt{2\pi t^5}} \exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}} ds\\
& = & \frac k{2\sigma^3\sqrt{2\pi t^3}} \bb{\left.\bb{\abs{x}+2ks}(s-\abs{x}) \exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}} \right|^{\abs{x}}_r+ \int^r_{\abs{x}}  \bb{2k(s-\abs{x}) + \bb{\abs{x}+2ks}} \exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}}ds }.
\eeast

Thus, add it to integral $\int^r_{\abs{x}} - \frac{k\bb{\abs{x} + 2k^2s + 3ks}}{\sigma^3\sqrt{2\pi t^3}}\exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}}  ds$, we get
\beast
& & \sum^\infty_{k=-\infty} \frac k{2\sigma^3\sqrt{2\pi t^3}} \bb{\bb{\abs{x}+2kr}(r-\abs{x}) \exp\bb{-\frac{\bb{\abs{x}+2kr}^2}{2\sigma^2 t}} - \int^r_{\abs{x}} (2k+1)\bb{\abs{x}+2ks}\exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}} ds} \\
& = & \sum^\infty_{k=-\infty} \frac k{2\sigma^3\sqrt{2\pi t^3}} \bb{\abs{x}+2kr}(r-\abs{x}) \exp\bb{-\frac{\bb{\abs{x}+2kr}^2}{2\sigma^2 t}} + \left.\frac {2k+1}{4\sigma\sqrt{2\pi t}} \exp\bb{-\frac{\bb{\abs{x}+2ks}^2}{2\sigma^2 t}} \right|^r_{\abs{x}} \\
& = & \sum^\infty_{k=-\infty} \frac k{2\sigma^3\sqrt{2\pi t^3}} \bb{\abs{x}+2kr}(r-\abs{x}) \exp\bb{-\frac{\bb{\abs{x}+2kr}^2}{2\sigma^2 t}} + \frac {2k+1}{4\sigma\sqrt{2\pi t}} \exp\bb{-\frac{\bb{\abs{x}+2kr}^2}{2\sigma^2 t}}.
\eeast
...
\beast
\pro\bb{R_t \leq r}  & = & \sum^\infty_{k=1}\frac{4k^2\pi^2\sigma^8}{\bb{k^2\pi^2\sigma^4 + \mu^2r^2}^2} \exp\bb{-\frac{\bb{k^2\pi^2\sigma^4 + \mu^2r^2}t}{2\sigma^2r^2}}\\
& & \qquad\qquad \times \bb{\bb{1-(-1)^k\cosh\bb{\frac{\mu r}{\sigma^2}}}\bb{1+\frac{k^2\pi^2 \sigma^2 t}{r^2} - \frac{4\mu^2r^2}{k^2\pi^2\sigma^4 + \mu^2r^2}} - (-1)^k\frac{\mu r}{\sigma^2} \sinh\bb{\frac{\mu r}{\sigma^2}}}.
\eeast
\end{theorem}
...
\be
\pro\bb{R_t\leq r} = 1 + 4\sum^\infty_{k=1}(-1)^{k}k \erfc\bb{\frac{kr}{\sigma\sqrt{2 t}}} = 4\sum^\infty_{k=1}\frac{1-(-1)^k}{k^2\pi^2} \exp\bb{-\frac{k^2\pi^2 \sigma^2 t}{2r^2}}\bb{1+\frac{k^2\pi^2\sigma^2 t}{r^2} }
\ee
...
\beast%\frac 1{\sigma\sqrt{2\pi t}} \sum_{k=-\infty}^{\infty} \frac{1}{2y} \sqrt{2\pi} \exp\bb{-\frac{\pi^2 k^2}{2y^2}} \exp\bb{\frac{\pi i kx} y} \\ %
\frac 1{\sigma\sqrt{2\pi t}}  \sum^\infty_{n = -\infty} \exp\bb{-\frac{(x+2ny)^2}{2\sigma^2 t}} & = &  \frac 1{\sigma\sqrt{2\pi t}} \sum_{k=-\infty}^{\infty} \frac{1}{2y} \sqrt{2\pi\sigma^2 t} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}} \exp\bb{\frac{\pi i kx} y} \\
& = & \frac{1}{2y}  \sum_{k=-\infty}^{\infty} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}} \exp\bb{\frac{\pi i kx} y} \qquad (*).
\eeast

Taking the derivatives of $x$ and $y$ respectively, we have

\beast
& & \frac 1{\sigma\sqrt{2\pi t}}  \sum^\infty_{n = -\infty} \frac{x+ 2ny}{\sigma^2 t} \exp\bb{-\frac{(x+2ny)^2}{2\sigma^2 t}} =  -\frac{1}{2y}  \sum_{k=-\infty}^{\infty} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}}  \exp\bb{\frac{\pi i kx} y} \frac{\pi i k} y \qquad (\dag)\\
& & \frac 1{\sigma\sqrt{2\pi t}}  \sum^\infty_{n = -\infty} \frac{2n(x+ 2ny)}{\sigma^2 t} \exp\bb{-\frac{(x+2ny)^2}{2\sigma^2 t}} =  \frac 12 \sum_{k=-\infty}^{\infty} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}}  \exp\bb{\frac{\pi i kx} y}\bb{\frac{1}{y^2} - \frac{2k^2\pi^2\sigma^2 t}{2y^4} + \frac{ik\pi x}{y^3}}\qquad (\dag\dag)
\eeast

Therefore, $\frac {\sigma^2t}y (*) + \bb{1-\frac xy}(\dag)$ is

\beast
\frac 1{\sigma\sqrt{2\pi t}}  \sum^\infty_{n = -\infty} (2n+1)\exp\bb{-\frac{(x+2ny)^2}{2\sigma^2 t}}  =  \frac{1}{2}  \sum_{k=-\infty}^{\infty} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}}  \exp\bb{\frac{\pi i kx} y} \bb{- \frac{\pi i k \sigma^2t }{y^3} +  \frac {y - x}{y^2}}\qquad (**)
\eeast

Then, $(x-y)*(\dag\dag) + (**)$ is

\beast
& & \frac 1{\sigma\sqrt{2\pi t}}  \sum^\infty_{n = -\infty} \bb{(2n+1) - \frac{2n}{\sigma^2 t} (y-x)(x+ 2ny)}\exp\bb{-\frac{(x+2ny)^2}{2\sigma^2 t}} \\
& = &  \frac 12 \sum_{k=-\infty}^{\infty} \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2y^2}}  \exp\bb{\frac{\pi i kx} y} \bb{- \frac{\pi i k \sigma^2t }{y^3} + (x-y)\bb{\frac{ik\pi x}{y^3}- \frac{k^2\pi^2\sigma^2 t}{y^4}}}.
\eeast
...
\be
\pro\bb{R\leq r} = \frac 1{2} \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum^\infty_{k=-\infty} I_k \exp\bb{-\frac{\pi^2 k^2 \sigma^2 t}{2r^2}}
\ee

where
\be
I_k = \int^r_{-r}\exp\bb{\frac{\mu x}{\sigma^2}}  \exp\bb{\frac{\pi i k\abs{x}} r} \bb{- \frac{\pi i k \sigma^2t }{r^3} + (\abs{x}-r)\bb{\frac{ik\pi \abs{x}}{r^3}- \frac{k^2\pi^2\sigma^2 t}{r^4}}}dx%& = & \int^r_{-r}\exp\bb{\frac{\mu x}{\sigma^2}}  \exp\bb{\frac{\pi i k\abs{x}} r} \bb{\frac{ik\pi }{r^3}\abs{x}^2- \frac{k^2\pi^2\sigma^2 t+ ik\pi r^2 }{r^4}\abs{x} +  \frac{\bb{k^2\pi^2 -\pi i k }\sigma^2t }{r^3}}dx.
\ee

Let
\beast
C_1(\mu) & = &  \int^r_0 \exp\bb{\frac{\mu x}{\sigma^2} + \frac{\pi i kx} r} dx = \frac{\sigma^2 r}{\mu r + i\pi k\sigma^2}\bb{(-1)^k\exp\bb{\frac{\mu r}{\sigma^2}} - 1}\\
C_2(\mu) & = &  \int^r_0 (x-r) \exp\bb{\frac{\mu x}{\sigma^2} + \frac{\pi i kx} r} dx = \frac{\sigma^2 r^2}{\mu r + i\pi k\sigma^2} + \frac{\sigma^4 r^2}{\bb{\mu r + i\pi k\sigma^2}^2}\bb{1- (-1)^k\exp\bb{\frac{\mu r}{\sigma^2}} }\\
C_3(\mu) & = &  \int^r_0 x(x-r) \exp\bb{\frac{\mu x}{\sigma^2} + \frac{\pi i kx} r} dx \\
& = &  -\frac{\sigma^4 r^3}{\bb{\mu r + i\pi k\sigma^2}^2}\bb{1+(-1)^k\exp\bb{\frac{\mu r}{\sigma^2}}} + \frac{2\sigma^6 r^3}{\bb{\mu r + i\pi k\sigma^2}^3}\bb{(-1)^k\exp\bb{\frac{\mu r}{\sigma^2}}-1 }.
\eeast

Thus,
\be
I_k = - \frac{\pi i k \sigma^2t }{r^3}\bb{C_1(\mu) + C_1(-\mu)} - \frac{k^2\pi^2\sigma^2 t}{r^4}\bb{C_2(\mu) + C_2(-\mu)} + \frac{ik\pi }{r^3}\bb{C_3(\mu) + C_3(-\mu)}
\ee

Accordingly,
\beast
C_1(\mu) + C_1(-\mu) & = & \frac{\sigma^2 r}{\mu r + i\pi k\sigma^2}\bb{(-1)^k\exp\bb{\frac{\mu r}{\sigma^2}} - 1} - \frac{\sigma^2 r}{\mu r - i\pi k\sigma^2}\bb{(-1)^k\exp\bb{\frac{-\mu r}{\sigma^2}} - 1} \\
& = & \frac{2\sigma^2 r (-1)^k}{\mu^2 r^2 + \pi^2 k^2\sigma^4}\bb{\mu r \sinh\bb{\frac{\mu r}{\sigma^2}} - i\pi k\sigma^2 \cosh\bb{\frac{\mu r}{\sigma^2}}} + \frac{2i\pi k \sigma^4r}{\mu^2 r^2 + \pi^2 k^2\sigma^4}.
\eeast

\beast
& & C_2(\mu) + C_2(-\mu) \\
& = & \frac{\sigma^2 r^2}{\mu r + i\pi k\sigma^2} + \frac{\sigma^4 r^2}{\bb{\mu r + i\pi k\sigma^2}^2}\bb{1- (-1)^k\exp\bb{\frac{\mu r}{\sigma^2}} } - \frac{\sigma^2 r^2}{\mu r - i\pi k\sigma^2} + \frac{\sigma^4 r^2}{\bb{\mu r - i\pi k\sigma^2}^2}\bb{1- (-1)^k\exp\bb{\frac{-\mu r}{\sigma^2}} } \\
& = & - \frac{2i\pi k \sigma^4r^2}{\mu^2 r^2 + \pi^2 k^2\sigma^4} + \frac{2\bb{\mu^2r^2 - \pi^2 k^2\sigma^4}\sigma^4r^2}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} - (-1)^k \frac{2\sigma^4r^2\bb{\cosh\bb{\frac{\mu r}{\sigma^2}}\bb{\mu^2r^2 - \pi^2 k^2\sigma^4} - 2i\pi k\sigma^2\mu r\sinh\bb{\frac{\mu r}{\sigma^2}}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2}
\eeast

\beast
& & C_3(\mu) + C_3(-\mu) \\
& = & - \frac{2\sigma^4r^3\bb{\mu^2r^2 - \pi^2 k^2\sigma^4}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} - (-1)^k \frac{2\sigma^4r^3\bb{\cosh\bb{\frac{\mu r}{\sigma^2}}\bb{\mu^2r^2 - \pi^2 k^2\sigma^4} - 2i\pi k\sigma^2\mu r\sinh\bb{\frac{\mu r}{\sigma^2}}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} \\
& &  + \frac{4\sigma^6r^3\bb{3i\pi k\sigma^2\mu^2r^2 - i\pi^3 k^3\sigma^6}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} +  (-1)^k \frac{4\sigma^6r^3\bb{\bb{\mu^3r^3 - 3\mu r k^2 \pi^2 \sigma^4}\sinh\bb{\frac{\mu r}{\sigma^2}} + i\bb{k^3\pi^3 \sigma^6 - 3\mu^2r^2\pi k\sigma^2}\cosh\bb{\frac{\mu r}{\sigma^2}}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3}
\eeast

%Since we have
%\be
%\int^r_0 \exp(ax) dx = \frac 1a \exp(ar) - \frac 1a,\quad \int^r_0 x \exp(ax) dx = \bb{\frac ra - \frac 1{a^2}}\exp(ar) + \frac 1{a^2},
%\ee
%\be
%\int^r_0 x^2 \exp(ax) dx = \bb{\frac {r^2}a  - \frac {2r}{a^2} + \frac 2{a^3} }\exp(ar) - \frac 2{a^3}.
%\ee
%
%Let $a = \frac{\mu }{\sigma^2} + \frac{\pi i k }r$ and $b = -\frac{\mu }{\sigma^2} + \frac{\pi i k }r $. We write $I_k = A_k+  B_k + C_k$ where
%\beast
%A_k & = & \frac{ik\pi }{r^3} \int^r_{-r} \exp\bb{\frac{\mu x}{\sigma^2}}  \exp\bb{\frac{\pi i k\abs{x}} r} \abs{x}^2 dx \\
%& = & \frac{ik\pi }{r^3} \int^r_0\bb{\exp\bb{\bb{\frac{\mu }{\sigma^2} + \frac{\pi i k }r}x } x^2 - \exp\bb{\bb{\frac{-\mu }{\sigma^2} + \frac{\pi i k }r}x} x^2} dx\\
%& = & \frac{ik\pi }{r^3} \bb{\bb{\frac {r^2}a  - \frac {2r}{a^2} + \frac 2{a^3} }\exp(ar) - \frac 2{a^3} - \bb{\frac {r^2}b  - \frac {2r}{b^2} + \frac 2{b^3} }\exp(br) + \frac 2{b^3}}
%\eeast
%
%\beast
%B_k & = & - \frac{k^2\pi^2\sigma^2 t+ ik\pi r^2 }{r^4} \int^r_{-r} \exp\bb{\frac{\mu x}{\sigma^2}}  \exp\bb{\frac{\pi i k\abs{x}} r} \abs{x}^2 dx \\
%& = & - \frac{k^2\pi^2\sigma^2 t+ ik\pi r^2 }{r^4} \int^r_0\bb{\exp\bb{\bb{\frac{\mu }{\sigma^2} + \frac{\pi i k }r}x } x + \exp\bb{\bb{\frac{-\mu }{\sigma^2} + \frac{\pi i k }r}x} x} dx\\
%& = & - \frac{k^2\pi^2\sigma^2 t+ ik\pi r^2 }{r^4} \bb{ \bb{\frac ra - \frac 1{a^2}}\exp(ar) + \frac 1{a^2} + \bb{\frac rb - \frac 1{b^2}}\exp(br) + \frac 1{b^2} }
%\eeast
%
%\beast
%C_k & = &  \frac{\bb{k^2\pi^2 -\pi i k }\sigma^2t }{r^3} \int^r_{-r} \exp\bb{\frac{\mu x}{\sigma^2}}  \exp\bb{\frac{\pi i k\abs{x}} r}  dx \\
%& = &  \frac{\bb{k^2\pi^2 -\pi i k }\sigma^2t }{r^3} \int^r_0\bb{\exp\bb{\bb{\frac{\mu }{\sigma^2} + \frac{\pi i k }r}x } - \exp\bb{\bb{\frac{-\mu }{\sigma^2} + \frac{\pi i k }r}x} } dx\\
%& = &  \frac{\bb{k^2\pi^2 -\pi i k }\sigma^2t }{r^3} \bb{ \frac 1a \exp(ar) - \frac 1a - \frac 1b \exp(br) + \frac 1b }
%\eeast

%Checking the terms with denominator $\mu^2 r^2 + \pi^2 k^2\sigma^4$, we have that their sum is
%\be
%- \frac{\pi i k \sigma^2t }{r^3}\bb{\frac{2\sigma r (-1)^k}{\mu^2 r^2 + \pi^2 k^2\sigma^4}\bb{\mu r \sinh\bb{\frac{\mu r}{\sigma^2}} - 2i\pi k\sigma^2 \cosh\bb{\frac{\mu r}{\sigma^2}}} + \frac{2i\pi k \sigma^3r}{\mu^2 r^2 + \pi^2 k^2\sigma^4}} + \frac{k^2\pi^2\sigma^2 t}{r^4} \frac{2i\pi k \sigma^3r^2}{\mu^2 r^2 + \pi^2 k^2\sigma^4}
%\ee
...
\be
C_1(\mu) + C_1(-\mu) \ \ra \ C'_1(\mu) = \frac{2i\pi k \sigma^4r}{\mu^2 r^2 + \pi^2 k^2\sigma^4}\bb{1 - (-1)^k\cosh\bb{\frac{\mu r}{\sigma^2}}}.
\ee

Similarly, we have
\be
C'_2(\mu) =   \frac{2\sigma^4r^2\bb{\mu^2r^2 - \pi^2 k^2\sigma^4}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} \bb{1-(-1)^k \cosh\bb{\frac{\mu r}{\sigma^2}}} ,
\ee

\be
C'_3(\mu) =   (-1)^k \frac{4i\sigma^6r^4 \pi k\mu \sinh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} + \frac{4i\sigma^6r^3\bb{3\pi k\sigma^2\mu^2r^2 - \pi^3 k^3\sigma^6}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} +  (-1)^k \frac{4i\sigma^6r^3\bb{k^3\pi^3 \sigma^6 - 3\mu^2r^2\pi k\sigma^2}\cosh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3}
\ee

Thus,
\beast
I_k & = &  \frac{2\pi^2 k^2 \sigma^6t}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}r^2}\bb{1 - (-1)^k\cosh\bb{\frac{\mu r}{\sigma^2}}}  - \frac{2k^2\pi^2\sigma^6 t\bb{\mu^2r^2 - \pi^2 k^2\sigma^4}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2r^2} \bb{1-(-1)^k \cosh\bb{\frac{\mu r}{\sigma^2}}}\\
& & \quad -(-1)^k \frac{4\sigma^6 \pi^2 k^2\mu r\sinh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} - \frac{4\sigma^6k\pi\bb{3\pi k\sigma^2\mu^2r^2 - \pi^3 k^3\sigma^6}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} - (-1)^k \frac{4\sigma^6k\pi \bb{k^3\pi^3 \sigma^6 - 3\mu^2r^2\pi k\sigma^2}\cosh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} \\
& = & \frac{4k^4\pi^4\sigma^{10} t }{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2r^2} \bb{1-(-1)^k \cosh\bb{\frac{\mu r}{\sigma^2}}} -(-1)^k \frac{4\sigma^6 \pi^2 k^2\mu r\sinh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} \\
& & \quad  \frac{4\sigma^8k^2\pi^2 \bb{k^2\pi^2 \sigma^4 + \mu^2 r^2 - 4\mu^2r^2}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} \bb{1- (-1)^k \cosh\bb{\frac{\mu r}{\sigma^2}}}.
\eeast

%Then we can eliminate the imaginary part and get
%\be
%C''_1(\mu) = \frac{2i\pi k \sigma^3r}{\mu^2 r^2 + \pi^2 k^2\sigma^4}\bb{1 - (-1)^k\cosh\bb{\frac{\mu r}{\sigma^2}}},\quad C''_2(\mu) =  \frac{2\sigma^2r^2\bb{\mu^2r^2 - \pi^2 k^2\sigma^4}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} \bb{1-(-1)^k \cosh\bb{\frac{\mu r}{\sigma^2}}} ,
%\ee
%
%\be
%C''_3(\mu) = (-1)^k \frac{2\sigma^2r^3\bb{ 2i\pi k\sigma^2\mu r\sinh\bb{\frac{\mu r}{\sigma^2}}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^2} + \frac{4\sigma^3r^3\bb{3i\pi k\sigma^2\mu^2r^2 - i\pi^3 k^3\sigma^6}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3} +  (-1)^k \frac{4\sigma^3r^3i\bb{k^3\pi^3 \sigma^6 - 3\mu^2r^2\pi k\sigma^2}\cosh\bb{\frac{\mu r}{\sigma^2}}}{\bb{\mu^2 r^2 + \pi^2 k^2\sigma^4}^3}
%\ee
...
\be
f_{T_a}(t) = \frac a{\sigma\sqrt{2\pi t^3}} \exp\bb{-\frac{(a-\mu t)^2}{2\sigma^2t }}.
\ee
\end{proposition}
...
\be
\pro\bb{T_a > t} = \pro\bb{\max_{0\leq s\leq t}X_s < a} = N\bb{\frac{a-\mu t}{\sigma\sqrt{t} }} - \exp\bb{\frac{2\mu a}{\sigma^2}}N\bb{-\frac{a+\mu t}{\sigma\sqrt{t} }}.
\ee

Then we take the differentiation,
\beast
f_{T_a}(t) & = & -\frac 1{\sqrt{2\pi}} \frac{-\mu \sqrt{t} - \frac 1{2\sqrt{t}}(a-\mu t)}{\sigma t} \exp\bb{-\frac{(a-\mu t)^2}{2\sigma^2t }} + \frac 1{\sqrt{2\pi}}\exp\bb{\frac{2\mu a}{\sigma^2}} \frac{-\mu \sqrt{t} + \frac 1{2\sqrt{t}}(a+\mu t)}{\sigma t} \exp\bb{-\frac{(a+\mu t)^2}{2\sigma^2t }} \\
& = &  \frac 1{\sigma\sqrt{2\pi t^3}} \bb{\frac {a+\mu t}2 + \frac {a-\mu t}2}\exp\bb{-\frac{(a-\mu t)^2}{2\sigma^2t }} =  \frac a{\sigma\sqrt{2\pi t^3}} \exp\bb{-\frac{(a-\mu t)^2}{2\sigma^2t }}.
\eeast
...
\beast
f(t) & = & \frac{\pi\sigma^2}{(a+b)^2}\exp\bb{- \frac{\mu^2t}{2\sigma^2}} \sum_{n=1}^\infty n \exp \bb{- \frac{n^2\pi^2 \sigma^2 t}{2(a+b)^2}} \bb{\exp\bb{\frac {\mu b}{\sigma^2}}\sin\bb{\frac{n\pi b}{a+b}} + \exp\bb{-\frac {\mu a}{\sigma^2}}\sin\bb{\frac{n \pi a}{a+b}}}\\
& = & \frac{\pi\sigma^2}{(a+b)^2} \exp\bb{-\frac{\mu^2t}{2\sigma^2}} \sum^\infty_{n=1} (-1)^{n-1} n \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\exp\bb{-\frac {\mu a}{\sigma^2}}\sin\frac{n\pi b}{a+b} + \exp\bb{\frac {\mu b}{\sigma^2}}\sin \frac{n\pi a}{a+b}}.
\eeast
\end{theorem}
...
\beast
& & \pro\bb{T \geq t} = \pro\bb{Y_t\leq b,Z_t \geq -a}\\
& = & \exp\bb{ - \frac{\mu^2t}{2\sigma^2}} \sum_{n=1}^\infty  \frac {2\pi n\sigma^4}{\mu^2(a+b)^2  + n^2 \pi^2 \sigma^4} \exp \bb{- \frac{n^2\pi^2 \sigma^2 t}{2(a+b)^2}} \bb{\exp\bb{\frac {\mu b}{\sigma^2}}\sin\bb{\frac{\pi n b}{a+b}} + \exp\bb{\frac {-\mu a}{\sigma^2}}\sin\bb{\frac{\pi n a}{a+b}}}.
\eeast

Then taking the differentiation of $t$, we have
\beast
f_T(t) & = & -\frac{\pi\sigma^2}{(a+b)^2}\exp\bb{- \frac{\mu^2t}{2\sigma^2}} \sum_{n=1}^\infty  (-n) \exp \bb{- \frac{n^2\pi^2 \sigma^2 t}{2(a+b)^2}} \bb{\exp\bb{\frac {\mu b}{\sigma^2}}\sin\bb{\frac{\pi n b}{a+b}} + \exp\bb{\frac {-\mu a}{\sigma^2}}\sin\bb{\frac{\pi n a}{a+b}}}\\
& = & \frac{\pi\sigma^2}{(a+b)^2}\exp\bb{- \frac{\mu^2t}{2\sigma^2}} \sum_{n=1}^\infty n \exp \bb{- \frac{n^2\pi^2 \sigma^2 t}{2(a+b)^2}} \bb{\exp\bb{\frac {\mu b}{\sigma^2}}\sin\bb{\frac{n\pi b}{a+b}} + \exp\bb{\frac {-\mu a}{\sigma^2}}\sin\bb{\frac{n \pi a}{a+b}}}
\eeast
...
\beast
f(t) & = & \frac{\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (-1)^{n-1} n \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\sin\frac{n\pi b}{a+b} + \sin \bb{\frac{n\pi a}{a+b}}} \\
& = & \frac{\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (-1)^{n-1} n\exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}} %\\ & = & \frac{2\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (2n-1) \exp\bb{-\frac{(2n-1)^2\pi^2 \sigma^2t}{2(a+b)^2}}\sin\frac{(2n-1)\pi b}{a+b}
\eeast

Since the fourier series ($L = a+b$, see Corollary \ref{cor:fourier_series_linear})
\be
x = 2(a+b)\sum^\infty_{n=1} \frac {(-1)^{n-1}}{n\pi}\sin \bb{\frac{n\pi x}{a+b}}
\ee
it is easy to check that
\beast
\pro(T<\infty) & = & \int^\infty_0 \frac{\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (-1)^{n-1} n\exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}}dt\\
& = & 2 \sum^\infty_{n=1}  \frac {(-1)^{n-1}}{n\pi} \bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}} = \frac{a}{a+b} + \frac{b}{a+b} = 1.
\eeast

Thus, by Corollary \ref{cor:fourier_series_cube},
\be
\sum^\infty_{n=1} \frac{(-1)^{n-1} }{n^3\pi^3}\sin \bb{\frac{n\pi x}{a+b}} = \frac{x\bb{(a+b)^2 - x^2}}{12(a+b)^3}.
\ee
we have that
\beast
\E T & = & \int^\infty_0 \frac{\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (-1)^{n-1} nt \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}}dt\\
& = & 2 \sum^\infty_{n=1} (-1)^{n-1} \frac 1{n\pi} \bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}}\int^\infty_0 \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}} dt\\
& = & \frac{4(a+b)^2}{\sigma^2} \sum^\infty_{n=1}  \frac {(-1)^{n-1}}{n^3\pi^3} \bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}} = \frac{4(a+b)^2}{\sigma^2}\bb{\frac{a\bb{(a+b)^2 - a^2}}{12(a+b)^3}+ \frac{b\bb{(a+b)^2 - b^2}}{12(a+b)^3}} = \frac{ab}{\sigma^2}.
\eeast

Thus, by Corollary \ref{cor:fourier_series_power_5},
\be
\sum^\infty_{n=1} \frac{(-1)^{n-1} }{n^5\pi^5}\sin \bb{\frac{n\pi x}{a+b}} = \frac{x\bb{3x^4 - 10x^2(a+b)^2 + 7(a+b)^4}}{720(a+b)^5}.
\ee
we have that
\beast
\E T^2 & = & \int^\infty_0 \frac{\pi\sigma^2}{(a+b)^2} \sum^\infty_{n=1} (-1)^{n-1} nt^2 \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}}\bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}}dt\\
& = & 2 \sum^\infty_{n=1} (-1)^{n-1} \frac 1{n\pi} \bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}}2\int^\infty_0 t\exp\bb{-\frac{n^2\pi^2 \sigma^2t}{2(a+b)^2}} dt\\
& = & \frac{16(a+b)^4}{\sigma^4} \sum^\infty_{n=1}  \frac {(-1)^{n-1}}{n^5\pi^5} \bb{\sin\frac{n\pi a}{a+b}+\sin\frac{n\pi b}{a+b}} \\
& = & \frac{16(a+b)^4}{\sigma^4} \bb{\frac{a\bb{3a^4 - 10a^2(a+b)^2 + 7(a+b)^4}}{720(a+b)^5} + \frac{b\bb{3b^4 - 10b^2(a+b)^2 + 7(a+b)^4}}{720(a+b)^5}} = \frac{ab(a^2 + b^2 +3ab)}{3\sigma^4}.
\eeast
...
\be
f(t) = \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1} (-1)^{n-1}n \exp\bb{-\frac{n^2\pi^2 \sigma^2t}{8a^2}}\sin\frac{n\pi }{2} = \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) \exp\bb{-\frac{(2n-1)^2\pi^2 \sigma^2 t}{8a^2}}.
\ee

Then
\beast
\pro( T<\infty) & = & \int^\infty_0 \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) \exp\bb{-\frac{(2n-1)^2\pi^2\sigma^2 t}{8a^2}}dt \\
& = & \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) \frac {8a^2 }{(2n-1)^2\pi^2\sigma^2} \left.t\exp\bb{-\frac{(2n-1)^2\pi^2 \sigma^2t}{8a^2}}\right|^0_\infty \\
...
\beast
\E T & = & \int^\infty_0 \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) t \exp\bb{-\frac{(2n-1)^2\pi^2\sigma^2 t}{8a^2}}dt \\
& = & \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) \frac {8a^2 }{(2n-1)^2\pi^2\sigma^2} \bb{\left.t\exp\bb{-\frac{(2n-1)^2\pi^2 \sigma^2t}{8a^2}}\right|^0_\infty + \int^\infty_0 \exp\bb{-\frac{(2n-1)^2\pi^2 t}{8a^2}}dt}\\
...
\beast
\E T^2 & = & \int^\infty_0 \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) t^2 \exp\bb{-\frac{(2n-1)^2\pi^2\sigma^2 t}{8a^2}}dt \\
& = & \frac{\pi\sigma^2}{2a^2} \sum^\infty_{n=1}(-1)^{n-1} (2n-1) \frac {8a^2 }{(2n-1)^2\pi^2\sigma^2} \bb{\left.t^2\exp\bb{-\frac{(2n-1)^2\pi^2 \sigma^2t}{8a^2}}\right|^0_\infty + \int^\infty_0 2t\exp\bb{-\frac{(2n-1)^2\pi^2 t}{8a^2}}dt}\\
...
\be
\E\bb{e^{-rT}} = \frac{e^{-\frac{\mu a}{\sigma^2}}\sinh\bb{\frac{b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}+ e^{\frac{\mu b}{\sigma^2}}\sinh\bb{\frac{a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}{\sinh\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}.
\ee

Accordingly, we have
\be
\pro\bb{T_{-a} < T_b} = \frac{1- e^{-\frac{2\mu b}{\sigma^2}}}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} },\qquad \pro\bb{T_{-a} > T_b} =  \frac{e^{\frac{2\mu a}{\sigma^2}} - 1}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} },
\ee
and
\be
\E T = \frac{ a\bb{e^{-\frac{2\mu b}{\sigma^2}} -1}  +  b\bb{e^{\frac{2\mu a}{\sigma^2}}-1}}{\mu\bb{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }}.
\ee

In particular,
\be
\E\bb{T \ind_{\bra{T_{b}<T_{-a}}}} \frac{e^{\frac{\mu b}{\sigma^2}}\bb{b\sinh\bb{\frac{a\mu}{\sigma^2}}\cosh\bb{\frac{(a+b)\mu}{\sigma^2}}  - a\sinh\bb{\frac{b\mu}{\sigma^2}}}}{\mu \sinh^2\bb{\frac{(a+b)\mu}{\sigma^2}}},
\ee
\be
\E\bb{T \ind_{\bra{T_{b} > T_{-a}}}} = \frac{e^{-\frac{\mu a}{\sigma^2}}\bb{a\sinh\bb{\frac{b\mu}{\sigma^2}}\cosh\bb{\frac{(a+b)\mu}{\sigma^2}}  - b\sinh\bb{\frac{a\mu}{\sigma^2}}}}{\mu \sinh^2\bb{\frac{(a+b)\mu}{\sigma^2}}}.
\ee
\end{theorem}
...
\beast
\exp\bb{-\bb{\frac{\mu}{\sigma^2}\pm \frac 1{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}}X_t -\frac{\lm^2 t}{2}}
& = & \exp\bb{-\frac{\mu}{\sigma^2}\bb{\mu t + \sigma B_t}\pm \frac 1{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}\bb{\mu t + \sigma B_t} -\frac{\lm^2 t}{2}} \\
& = & \exp\bb{-\bb{\frac{\mu}{\sigma} \pm \sqrt{\frac{\mu^2}{\sigma^2} + \lm^2}}B_t- \frac 12 \bb{\frac{\mu}{\sigma} \pm \sqrt{\frac{\mu^2}{\sigma^2} + \lm^2}}^2t}
\eeast
is a martingale. Thus, it is obvious that
\be
M_t = \exp\bb{-\frac{\mu X_t}{\sigma^2}-\frac{\lm^2 t}{2}} \sinh\bb{ \frac {X_t+a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}}
\ee
...
\beast
\sinh\bb{ \frac {a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}} = M_0 = \E M_T = \E\bb{\exp\bb{-\frac{\mu X_T}{\sigma^2}-\frac{\lm^2 T}{2}} \sinh\bb{ \frac {X_T+a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}}\ind_{\bra{T_{b}<T_{-a}}}}
\eeast

Hence,
\be
\E\bb{\exp\bb{-\frac{\lm^2 T}{2}} \ind_{\bra{T_{b}<T_{-a}}}} = \frac{e^{\frac{\mu b}{\sigma^2}}\sinh\bb{ \frac {a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}}}{\sinh\bb{ \frac {a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2}+\lm^2}}}
\ee
...
\beast
\E T & = & \left.- \fp{\E\bb{e^{-rT}}}{r}\right|_{r=0} \\
& = & -\left.\frac{\bb{\frac{be^{-\frac{\mu a}{\sigma^2}}}{\sigma\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} \cosh\bb{\frac{b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}+ \frac{ae^{\frac{\mu b}{\sigma^2}}}{\sigma \sqrt{\frac{\mu^2}{\sigma^2} + 2r}} \cosh\bb{\frac{a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}\sinh\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} }{\sinh^2\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}} \right|_{r=0} \\
& & + \left.\frac{\bb{e^{-\frac{\mu a}{\sigma^2}}\sinh\bb{\frac{b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}+ e^{\frac{\mu b}{\sigma^2}}\sinh\bb{\frac{a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}\cosh\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} \frac{a+b}{\sigma \sqrt{\frac{\mu^2}{\sigma^2} + 2r}} }{\sinh^2\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}} \right|_{r=0} \\
& = & \frac{\frac a{4\mu}\bb{1-e^{\frac{2\mu b}{\sigma^2}}+e^{-\frac{2\mu a}{\sigma^2}}-e^{-\frac{2\mu(a+b)}{\sigma^2}}}+\frac b{4\mu}\bb{e^{\frac{2\mu(a+b)}{\sigma^2}}-e^{\frac{2\mu b}{\sigma^2}}+e^{-\frac{2\mu a }{\sigma^2}}-1}}{\frac 14\bb{e^{\frac{2\mu(a+b)}{\sigma^2}} + e^{-\frac{2\mu(a+b)}{\sigma^2}} -2}}
\eeast

That is
\be
\E T = \frac{ a\bb{e^{-\frac{2\mu b}{\sigma^2}} -1} \bb{e^{\frac{2\mu b}{\sigma^2}}-e^{-\frac{2\mu a}{\sigma^2}}} +  b\bb{e^{\frac{2\mu a}{\sigma^2}}-1}\bb{e^{\frac{2\mu b}{\sigma^2}}-e^{-\frac{2\mu a}{\sigma^2}}}}{\mu\bb{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }\bb{e^{\frac{2\mu b}{\sigma^2}} - e^{-\frac{2\mu a}{\sigma^2}}}} = \frac{ a\bb{e^{-\frac{2\mu b}{\sigma^2}} -1}  +  b\bb{e^{\frac{2\mu a}{\sigma^2}}-1}}{\mu\bb{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }}.
\ee

In particular, by Proposition \ref{pro:laplace_finite_moment} %{pro:mgf_finite_moment}
we have
\beast
& & \E\bb{T \ind_{\bra{T_{b}<T_{-a}}}} \\
& = &  -\left.\frac {d \bb{\E\bb{\exp\bb{-r T\ind_{\bra{T_{b}<T_{-a}}}} }}}{dr}\right|_{r=0} = -\left.\frac {d \bb{\E\bb{\exp\bb{-r T} }\ind_{\bra{T_{b}<T_{-a}}}}}{dr}\right|_{r=0}\\
& = & \left.\frac{\frac{e^{\frac{\mu b}{\sigma^2}}}{\sigma \sqrt{\frac{\mu^2}{\sigma^2} + 2r}} \bb{-a\cosh\bb{\frac{a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}\sinh\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} + (a+b)\sinh\bb{\frac{a}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}\cosh\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}  }}{\sinh^2\bb{\frac{a+b}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}} \right|_{r=0} \\
& = & \frac{e^{\frac{\mu b}{\sigma^2}}\bb{b\sinh\bb{\frac{a\mu}{\sigma^2}}\cosh\bb{\frac{(a+b)\mu}{\sigma^2}}  - a\sinh\bb{\frac{b\mu}{\sigma^2}}}}{\mu \sinh^2\bb{\frac{(a+b)\mu}{\sigma^2}}}.
\eeast
...
Therefore,
\be
p = \frac{1}{a+b} \bb{b - \frac{ a\bb{e^{-\frac{2\mu b}{\sigma^2}} -1}  +  b\bb{e^{\frac{2\mu a}{\sigma^2}}-1}}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} } } = \frac{1- e^{-\frac{2\mu b}{\sigma^2}}}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }.
\ee

Similarly,
\be
\pro\bb{T_{-a} > T_b} =  \frac{e^{\frac{2\mu a}{\sigma^2}} - 1}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }.
\ee
\end{proof}
...
\be
0 = L_0 = \E L_t^T = \E\bb{\bb{X_t^T -\mu T\land t}^2 - \sigma^2 T\land t} = \E\bb{\bb{X_t^T}^2 - 2\mu T\land t X_t^T + \mu^2 (T\land t)^2 - \sigma^2T\land t}
\ee
Therefore we can apply dominated convergence theorem and monotone convergence theorem since $X_t$ is bounded and get
\beast
0 & = & \E\bb{X_T^2 - 2\mu T X_T + \mu^2 T^2 - \sigma^2T} \\
& = & pa^2 + (1-p)b^2 - 2\mu \E \bb{T X_T} + \mu^2 \E T^2 - \sigma^2 \E T \\
& = & pa^2 + (1-p)b^2 - 2\mu \E \bb{-a\E\bb{T\ind_{T_{-a}<T_b} } + b\E\bb{T\ind_{T_{-a} >T_b}}} + \mu^2 \E T^2 - \sigma^2 \E T
\eeast

Hence,
\be
\var T = \frac 1{\mu^2}\bb{2\mu \E \bb{-a\E\bb{T\ind_{T_{-a}<T_b} } + b\E\bb{T\ind_{T_{-a} >T_b}}} + \sigma^2 \E T - pa^2 - (1-p)b^2} - \bb{\E T}^2%\\ & = & \frac 1{\mu^2}\lob 2\bb{ \frac{be^{\frac{\mu b}{\sigma^2}}\bb{b\sinh\bb{\frac{a\mu}{\sigma^2}}\cosh\bb{\frac{(a+b)\mu}{\sigma^2}}  - a\sinh\bb{\frac{b\mu}{\sigma^2}}}}{\sinh^2\bb{\frac{(a+b)\mu}{\sigma^2}}} -\frac{ae^{-\frac{\mu a}{\sigma^2}}\bb{a\sinh\bb{\frac{b\mu}{\sigma^2}}\cosh\bb{\frac{(a+b)\mu}{\sigma^2}}  - b\sinh\bb{\frac{a\mu}{\sigma^2}}}}{\sinh^2\bb{\frac{(a+b)\mu}{\sigma^2}}} }\right.\\ & & \left.\qquad\qquad\quad + \sigma^2 \frac{ a\bb{e^{-\frac{2\mu b}{\sigma^2}} -1}  +  b\bb{e^{\frac{2\mu a}{\sigma^2}}-1}}{\mu\bb{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} }} - \frac{a^2\bb{1- e^{-\frac{2\mu b}{\sigma^2}}}}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} } - \frac{b^2\bb{e^{\frac{2\mu a}{\sigma^2}} - 1}}{e^{\frac{2\mu a}{\sigma^2}} - e^{-\frac{2\mu b}{\sigma^2}} } \rob\\
\ee

\item [(ii)] If $a=b=\Delta$, we have that
\be
\E\bb{e^{-rT}} = \frac{2\cosh\bb{\frac{\mu \Delta}{\sigma^2}} \sinh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}{\sinh\bb{\frac{2\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}} = \frac{\cosh\bb{\frac{\mu \Delta}{\sigma^2}} }{\cosh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}.
\ee

Therefore, by Proposition \ref{pro:laplace_finite_moment}, %{pro:mgf_finite_moment},
\beast
\E T & = & \left.- \fp{\E\bb{e^{-rT}}}{r}\right|_{r=0} = \left.\frac{\cosh\bb{\frac{\mu \Delta}{\sigma^2}}\sinh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}\frac{\Delta}{\sigma\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} }{\cosh^2\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}\right|_{r=0} \\
& = & \frac{\Delta}{\abs{\mu}}\tanh\bb{\frac{\abs{\mu}\Delta}{\sigma^2}} = \frac{\Delta}{\mu}\tanh\bb{\frac{\mu\Delta}{\sigma^2}}\approx  \frac{\Delta^2}{\sigma^2}\bb{1-\frac{\mu^2 \Delta^2}{3\sigma^4}}.
\eeast

Accordingly, by Proposition \ref{pro:laplace_finite_moment} again,
\beast
\E T^2 & = & \left. \frac{\partial^2\E\bb{e^{-rT}}}{\partial r^2}\right|_{r=0} = \left.-\fp{}{r}\bb{\frac{\cosh\bb{\frac{\mu \Delta}{\sigma^2}}\sinh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}\frac{\Delta}{\sigma\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} }{\cosh^2\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}}\right|_{r=0} \\
& = & \left.-\frac{\cosh\bb{\frac{\mu \Delta}{\sigma^2}}\frac{\Delta}{\sigma\bb{\frac{\mu^2}{\sigma^2} + 2r}} \cosh^2\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} \bb{\frac{\Delta}{\sigma}\cosh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} - \sinh\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}\frac{1}{\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} } }{\cosh^4\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}\right|_{r=0} \\
& & \qquad + \left. \frac{2\cosh\bb{\frac{\mu \Delta}{\sigma^2}}\frac{\Delta^2}{\sigma^2\bb{\frac{\mu^2}{\sigma^2} + 2r}} \sinh^2\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}} }{\cosh^3\bb{\frac{\Delta}{\sigma}\sqrt{\frac{\mu^2}{\sigma^2} + 2r}}}\right|_{r=0} \\
& = & 2\frac{\Delta^2}{\mu^2}\tanh^2\bb{\frac{\mu\Delta}{\sigma^2}} - \frac{\Delta^2}{\mu^2} + \frac{\Delta\sigma^2}{\mu^3} \tanh\bb{\frac{\mu\Delta}{\sigma^2}}.
\eeast

Therefore,
\beast
\var T =\E T^2 - \bb{\E T}^2 & = & \frac{\Delta^2}{\mu^2}\bb{\frac{\sinh^2\bb{\frac{\mu\Delta}{\sigma^2}}}{\cosh^2\bb{\frac{\mu\Delta}{\sigma^2}}} -1 + \frac{\sigma^2}{\mu\Delta}\tanh\bb{\frac{\mu\Delta}{\sigma^2}}} \\%- \frac{\Delta^2}{\mu^2}\tanh^2\bb{\frac{\mu\Delta}{\sigma^2}}\\
& = & \frac{\Delta^2}{\mu^2\cosh^2\bb{\frac{\mu\Delta}{\sigma^2}} }\bb{\frac{\sigma^2}{2\mu\Delta}\sinh\bb{\frac{2\mu\Delta}{\sigma^2}} -1}.
\eeast
...
& = & \frac{\Delta^2 }{\mu^2} \bb{1 - \frac 13 x^2 + \frac 2{15}x^4 + o(x^4)} - \frac {\Delta^2}{\mu^2} \bb{1 - \frac 12 x^2 + \frac 5{24}x^4 + o(x^4)}^2 \\
& = & \frac{\Delta^2 }{\mu^2} \bb{1 - \frac 13 x^2 + \frac 2{15}x^4 + o(x^4)} - \frac {\Delta^2}{\mu^2} \bb{1 - x^2 + \frac 5{12}x^4 + \frac 14x^4 + o(x^4)} \\
& = & \frac{\Delta^2 }{\mu^2} \bb{\frac 23 x^2 - \frac 8{15}x^4 + o(x^4)} \approx \frac{2\Delta^4 }{3\sigma^4}\bb{1 - \frac{4\mu^2\Delta^2}{5\sigma^4}}.
\eeast

Therefore, let $A :=\E T$, $B :=\var T$, $y: = \frac{\Delta^2}{\sigma^2}$ and $x^2:= \frac{\mu^2\Delta^2}{\sigma^4}$, we have
\be
A = y\bb{1-\frac 13 x^2},\quad B = \frac 23y^2 \bb{1- \frac 45 x^2},\quad 8x^2 = 24 - \frac {24A}y = 10 - \frac{15B}{y^2} \ \ra \ 14y^2 - 24Ay + 15B = 0.
\ee
...
Then
\be
x^2 = 3 - \frac{3A}y = 3 - \frac {42A\bb{12A - \sqrt{144 A^2 - 210B}}}{210B} = \frac{15B - 12A^2 + A\sqrt{144 A^2 - 210B}}{5B}.
\ee

Thus,
\be
\sigma^2 =   \frac {\Delta^2 A\bb{12A + \sqrt{144 A^2 - 210B}}}{15B},\quad \mu^2 = \frac{196\Delta^2\bb{15B - 12A^2 + A\sqrt{144 A^2 - 210B}}}{5B\bb{12A + \sqrt{144 A^2 - 210B}}^2}
\ee
\een
\end{remark}
...
\be
\frac{\sinh\bb{ax}}{\sinh\bb{(a+b)x}} = \sum^\infty_{n=1}\frac{2n\pi }{n^2\pi^2 + (a+b)^2x^2 }\sin\bb{\frac {n\pi b}{a+b}} = \sum^\infty_{n=1}\frac{2n\pi (-1)^{n-1}}{n^2\pi^2 + (a+b)^2x^2 }\sin\bb{\frac {n\pi a}{a+b}}.
\ee
\end{corollary}
...
\beast
\pro\bb{B_{t_1}>0,B_{t_2} < 0} & = & \pro\bb{B_{t_1}>0,B_{t_2} - B_{t_1} < -B_{t_1}} \\
& = & \E\bb{\ind_{\bra{B_{t_1}>0}}\ind_{\bra{B_{t_2-t_1} < -B_{t_1}}}} = \E\bb{\ind_{\bra{B_{t_1}>0}}\E\bb{\left.\ind_{\bra{B_{t_2-t_1} < -B_{t_1}}}\right|B_{t_1}}}\\
...
\be
\pro\bb{\frac{Y^2}{X^2+Y^2} \leq \alpha} = \frac 2{\pi}\arcsin\sqrt{\alpha}.
\ee
...
\be
\pro\bb{\forall t\in [a,b],B_t \neq 0} = \pro\bb{\sup_{0\leq s\leq b-a} \wt{B}_s <\abs{B_a}} = \pro\bb{\sqrt{b-a}\abs{Y}\leq \sqrt{a}\abs{X}}.
\ee
...
\beast
\pro\bb{\frac{Y^2}{X^2+Y^2} \leq \alpha} & = & \frac 1{2\pi}\int^\infty_{-\infty}\int^\infty_{-\infty} \ind_{\bra{\frac{y^2}{x^2+y^2}\leq \alpha}}e^{-x^2/2}e^{-y^2/2}dxdy = \frac 1{2\pi}\int^{2\pi}_0 \ind_{\bra{\sin^2\theta<\alpha}} \int^\infty_0 e^{-r^2/2}rdr d\theta \\
...
\beast
\pro\bb{\forall t\in [a,b],B_t \neq 0} & = & \pro\bb{\sup_{0\leq s\leq b-a} \wt{B_s}  < -B_a , B_a < 0}+ \pro\bb{\inf_{0\leq s\leq b-a} \wt{B_s}  >  -B_a ,B_a \geq 0}\\
& = & \pro\bb{\sup_{0\leq s\leq b-a} \wt{B_s}  < \abs{B_a},  B_a < 0} + \pro\bb{\sup_{0\leq s\leq b-a} -\wt{B_s}  < \abs{B_a} ,B_a \geq 0} \\
& = & \pro\bb{\sup_{0\leq s\leq b-a} \wt{B_s}  < \abs{B_a},  B_a < 0} + \pro\bb{\sup_{0\leq s\leq b-a} \wt{B_s}  < \abs{B_a} ,B_a \geq 0} \\
& = & \pro\bb{\sup_{0\leq s\leq b-a} \wt{B_s}  < \abs{B_a}}.
\eeast
...
\be
\pro\bb{\forall t\in [a,b],B_t \neq 0} = \pro\bb{\sqrt{b-a}\abs{Y}  \leq \sqrt{a}\abs{X}}.
\ee
