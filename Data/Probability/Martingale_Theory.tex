...
\ben
\item [(i)] $X$ is a martingale\index{martingale!discrete} with respect to $((\sF_n)_{n\geq 0},\pro)$ if $\E\brb{X_n | \sF_m} = X_m$ a.s. for all $n \geq m$.
\item [(ii)] $X$ is a supermartingale\index{supermartingale!discrete} with respect to $((\sF_n)_{n\geq 0},\pro)$ if $\E\brb{X_n | \sF_m}\leq X_m$ a.s. for all $n\geq m$.
\item [(iii)] $X$ is a submartingale\index{submartingale!discrete} with respect to $((\sF_n)_{n\geq 0},\pro)$ if $\E\brb{X_n | \sF_m} \geq X_m$ a.s. for all $n\geq m$.
\een
...
%\ben
%\item [(i)] martingale\index{martingale} with respect to $((\sF_t)_{t\in I},\pro)$ if $\E\brb{X_t | \sF_s} = X_s$ a.s. for all $s \leq t \in I$;
%\item [(ii)] super-martingale\index{super-martingale} with respect to $((\sF_t)_{t\in I},\pro)$ if $\E\brb{X_t | \sF_s}\leq X_s$ a.s. for all $s \leq t \in I$;
%\item [(iii)] sub-martingale\index{sub-martingale} with respect to $((\sF_t)_{t\in I},\pro)$ if $\E\brb{X_t | \sF_s} \geq X_s$ a.s. for all $s \leq t \in I$.
%\een

%In particular, if $X$ is a
%\ben
%\item [(i)] martingale, then $\E\brb{X_t} = \E\brb{X_s}$ for all $s, t\in I$;
%\item [(ii)] super-martingale, then $\E\brb{X_t} \leq \E\brb{X_s}$ for all $s \leq t \in I$;
%\item [(iii)] sub-martingale, then $\E\brb{X_t} \geq \E\brb{X_s}$ for all $s \leq t \in I$.
%\een
%\end{definition}
...
\be
\E\brb{S_{n+1}|\sF_n} = S_n +\E\brb{X_{n+1}} = S_n +\E X_1.
\ee
...
%\be
%\E\brb{X_{n+1}|\sF_n} = X_n \quad \text{a.s. for all }n.
%\ee
...
\be
\E\brb{(C \cdot X)_{n+1}| \sF_n} = \E\brb{(C \cdot X)_n + C_{n+1}(X_{n+1} - X_n) |\sF_n} = (C \cdot X)_n + C_{n+1} \E\brb{X_{n+1} - X_n | \sF_n} = (C \cdot X)_n\quad \text{a.s.}
\ee
...
\be
[M,N]_n := \sum^n_{k=1}\Delta M_k\Delta N_k,\qquad [M_n] := \sum^n_{k=1}\brb{\Delta M_k}^2.
\ee
...
First, we have
\be
M_nN_n - M_0N_0 = \sum^n_{k=1} M_{k-1}\brb{N_k - N_{k-1}} + \sum^n_{k=1} N_{k-1}\brb{M_k - M_{k-1}} + \sum^n_{k=1} \Delta M_k \Delta N_k.
\ee

From Proposition \ref{pro:stochastic_integral_discrete_martingale}\footnote{different version in Williams\cite{Williams_1991}.$P_{97}$}, we can have that \be \sum^n_{k=1} M_{k-1}\brb{N_k - N_{k-1}}\quad \text{and}\quad
\sum^n_{k=1} N_{k-1}\brb{M_k - M_{k-1}}\quad \text{martingales.} \ee
...
Thus, we have
\be
\E \brb{M} = \E \abs{X_0} + \E\brb{\sum_{m=0}^{T \land n-1}\abs{X_{m+1}-X_m}} \leq \E \abs{X_0} + M \cdot\mathbb{E}(T-1)<\infty.
\ee
...
1. The probability generating function is \be \phi(z) = \E \brb{z^T|S_0=1} = \sum^\infty_{k=0}\pro\brb{T=k|S_0=1}z^k. \ee
...
independent random variables is simply the product of the probability generating functions. Thus, we have \be \phi(z) = \frac{1}{2}\E \brb{z^{T+1}|S_0=0}+\frac{1}{2}\E \brb{z^{T+1}|S_0=2} = \frac{z}{2}\E
\brb{z^T|S_0=0}+\frac{z}{2}\E \brb{z^T|S_0=2} \ee since the random walk will moved from 1 to either 0 or 2. For starting point 0, $T$ is 0. Also, we have $\E \brb{z^T|S_0=2}=\phi(z)^2$. Thus, \be \phi(z) =
...
have \be \pro(T=2k-1|S_0=1) = \binom{1/2}{k}(-1)^{k+1} = \frac{\frac 12 \cdot \brb{-\frac 12} \cdot \brb{-\frac 32} \cdots\brb{\frac{3-k}2}}{k!}(-1)^{k+1} = \frac{1}{2k-1}\binom{2k}{k}2^{-2k}.\ee
...
\text{as $k\to \infty$} \ee which somehow proves the statement that $\pro\brb{T<\infty} = 1$. For expected stopping time \be \E \brb{T|S_0=1} \sim \sum^\infty_{k=1}\frac{2k-1}{(2k-1)\sqrt{\pi k}} =
...
\be
\E\brb{(C \cdot X)_K} \leq \E\brb{(C \cdot X)_0} = 0 \quad\ra\quad \E\brb{\ind_A(X_T - X_S)}\leq 0\quad \ra\quad \E\brb{\ind_A X_T}\leq  \E\brb{\ind_A X_S} \text{ for all }A \in \sF_S. \quad (*)
\ee
...
\be
\E\brb{\ind_A X_T} = \E\brb{\ind_A Y} \geq \E\brb{\ind_A X_S} + \frac 1n \pro(A)
\ee
...
\be
\E X_T = \E\brb{\lim_{n\to \infty} X_{T\land n}} = \E\brb{\liminf_n X_{T\land n}} \leq \liminf_n \E\brb{X_{T\land n}} \leq \liminf_n \E X_0 = \E X_0,
\ee
as required.
\end{proof}
...
\beast
\E T & \leq & (a+b)2^{-(a+b)} + 2(a+b) 2^{-(a+b)}\brb{1-2^{-(a+b)}} + 3(a+b)2^{-(a+b)}\brb{1-2^{-(a+b)}}^2 + \dots \\
& = & (a+b)2^{-(a+b)} \sum^\infty_{n=1} n \brb{1-2^{-(a+b)}}^{n-1} = (a+b)2^{-(a+b)} \brb{\sum^\infty_{n=1} \rho^{n}}' \quad (\rho := 1-2^{-(a+b)})\\
& = & (a+b)2^{-(a+b)} \brb{\frac{\rho}{1-\rho}}' = (a+b)2^{-(a+b)} \frac 1{(1-\rho)^2} = (a+b)2^{-(a+b)}  2^{2(a+b)} = (a + b)2^{a+b}.
\eeast
...
We also have \be \E S_T = -a\pro\brb{T_{-a} < T_b} + b\pro\brb{T_b < T_{-a}},\quad \pro\brb{T_{-a} < T_b} + \pro\brb{T_b < T_{-a}} = 1, \ee and hence we deduce that \be \pro\brb{T_{-a} < T_b} = \frac b{a + b}. \ee
...
\beast
\E \brb{M_{n+1}|\sF_n} & = & \E\brb{S_{n+1}^2-n-1|\sF_n} = \E \brb{(S_n+X_{n+1})^2-n-1|\sF_n} \\
& = & \E \brb{S_n^2+2S_nX_{n+1}+X_{n+1}^2-n-1|\sF_n} \stackrel{\text{a.s.}}{=} S_n^2-n+\E \brb{2S_nX_{n+1}+X_{n+1}^2-1|\sF_n} \\
...
property we have \be \E \brb{S_{n\land T}^2-n\land T}=\E \brb{S_{0\land T}^2-0\land T}=0  \ \ra \ \E \brb{n\land\tau} = \E \brb{S_{n\land\tau}^2} .\ee
...
\brb{S_T^2}=(-a)^2 \pro\brb{T_{-a} < T_b} +b^2 \pro\brb{T_b < T_{-a}} = \frac{a^2 b}{a+b} + \frac{ab^2}{a+b} = ab. \ee
...
integrability, for any $m\leq n$ \beast \E\brb{M_n|\sF_m} & = & \E\brb{S_n - (p-q)n} = \E\brb{S_m - (p-q)m + \sum^n_{k = m+1}X_k - (p-q)(n-m)|\sF_m} \\
& \stackrel{\text{a.s.}}{=} & S_m - (p-q)m + \E\brb{ \sum^n_{k = m+1}X_k} - (p-q)(n-m) = M_m. \eeast

Thus, applying optional stopping theorem, we have \be 0 = M_0 = \E M_n = \E \brb{S_T - (p-q)T} \ee which implies \beast (p-q)\E T & = & \E S_T = -a \frac{(q/p)^a - (q/p)^{a+b}}{1 - (q/p)^{a+b}} + b \frac{1 - (q/p)^a }{1 -
(q/p)^{a+b}} \\
\E T & = & \frac{b \brb{1 - (q/p)^a} - a \brb{(q/p)^a - (q/p)^{a+b}}  }{(p-q)\brb{1 - (q/p)^{a+b}}}.  \eeast
\end{example}
...
\be
(b - a)\E\brb{N_n(X, [a, b])} \leq \E\brb{(X_n - a)^-}.
\ee
\end{lemma}
...
\be
\E\brb{N_n(X, [a, b])} \leq (b-a)^{-1}\E\brb{(X_n - a)^-} \leq (b-a)^{-1}\brb{ \E\abs{X_n}+ a} <\infty
\ee
%for some constant $M$,
for all $n$. By the monotone convergence theorem (Theorem \ref{thm:monotone_convergence_probability}) since $N_n(X,[a,b])\ua N(X,[a,b])$ as $n\to \infty$, we get \be \E\brb{N_n(X, [a, b])} \ua \E\brb{N(X, [a, b])} \leq
(b-a)^{-1} \brb{\sup_n \E\abs{X_n}+ a} <\infty, \ee by the assumption on $X$ being bounded in $\sL^1(\Omega,\sF_\infty,\pro)$. Therefore, we get that $N(X,[a,b]) < \infty$ a.s. for every $a < b \in \Q$. Hence, \beast
\pro\brb{\bigcap_{a<b\in \Q}\bra{N(X,[a,b])< \infty}} & = & 1- \pro\brb{\bigcup_{a<b\in \Q}\bra{N(X,[a,b])= \infty}} \\
& \geq & 1 - \sum_{a<b\in \Q}\pro\brb{\bra{N(X,[a,b])= \infty}} =  1- 0 = 1.
\eeast
...
Then by Fatou's lemma (Lemma \ref{lem:fatou_function}) and the assumption on $X$ being bounded in $\sL^1(\Omega,\sF_\infty,\pro)$ we get \be \E\abs{X_\infty} = \E \brb{\liminf_n \abs{X}} \leq \liminf_n \E\abs{X_n} \leq
...
\be
E\brb{X_0} \geq \liminf_{n\to \infty} \E\brb{X_{T\land n}} \geq \E\brb{\liminf_{n\to \infty} X_{T\land n}} \ua  \E\brb{X_T}.
\ee
\end{proof}
...
\be
\lm \pro\brb{X_n^* \geq \lm} \leq \E\brb{X_n\ind_{\bra{X_n^* \geq \lm}}} \leq \E X_n.
\ee
\end{theorem}
...
\be
\lm \pro\brb{\max \abs{X_n} \geq \lm} \leq \E\brb{\abs{X_n}\ind_{\bra{\max\abs{X_n} \geq \lm}}} \leq \E\abs{X_n}.
\ee
\end{remark}
...
\be
\E(X_n) \geq \E(X_{T\land n}) = \E\brb{X_T \ind_{\bra{T\leq n}}} + \E\brb{X_n \ind_{\bra{T>n}}} \geq \lm \pro(T\leq n) + \E\brb{X_n\ind_{\bra{T>n}}}.
\ee

It is clear that $\bra{T\leq n} = \bra{X_n^* \geq \lm}$. Hence we get
\be
\lm \pro(X_n^* \geq \lm) \leq \E \brb{X_n \ind_{\bra{T\leq n}}} = \E \brb{X_n \ind_{\bra{X_n^*\geq \lm}}} \leq \E(X_n).
\ee
\end{proof}
...
\beast
\dabs{X_n^* \land k}_p^p & = & \E\brb{(X_n^*\land k)^p} = \E\brb{\int^{X_n^* \land k}_0 px^{p-1}dx} = \E \brb{\int^k_0 p x^{p-1}\ind_{\bra{X_n^* \geq x}}dx} \\
& = & \int^k_0 p x^{p-1}\pro(X_n^* \geq x)dx\qquad (\text{Fubini theorem (Theorem \ref{thm:fubini})})\\
& \leq &  p \int^k_0 x^{p-2} \E\brb{X_n \ind_{\bra{X_n^* \geq x}}}d x \qquad (\text{Doob's maximal inequality (Theorem \ref{thm:doob_maximal_inequality_discrete})})\\
& \leq & \frac p{p -1} \E\brb{\abs{X_n} \int^k_0 (p -1)x^{p-2} \ind_{\bra{X_n^*\geq x}}d x}\qquad (\text{Fubini theorem (Theorem \ref{thm:fubini})})\\
& = & \frac p{p -1} \E\brb{\abs{X_n} \int^{X_n^* \land k}_0 (p -1)x^{p-2} d x} = \frac p{p -1} \E\brb{\abs{X_n}(X_n^*\land k)^{p-1}} \\
& \leq & \frac p{p -1} \E\brb{\abs{X_n}^p}^{1/p} \E\brb{(X_n^*\land k)^{(p-1)q}}^{1/q}\qquad (\text{H\"older's inequality (Theorem \ref{thm:holder_inequality_expectation})})\\
...
\be
\pro\brb{\max_{0\leq k\leq n} X_k \geq \lm} \leq \frac{\E X_n^+}{\lm}.
\ee
\end{theorem}
...
Then
\beast
\pro\brb{B} & = & \sum_{k=1}^n \E\brb{\ind_{B_k}} \leq \sum_{k=1}^n \E\brb{\frac 1{\lm} X_k \ind_{B_k}} \leq \frac 1{\lm} \sum_{k=1}^n \E\brb{\E\brb{X_n|\sF_k}\ind_{B_k}}\qquad \text{ $X⁢$ is a submartingale}\\
& = & \frac 1{\lm} \sum_{k=1}^n\E\brb{\E\brb{X_n \ind_{B_k}|\sF_k}}\qquad \text{ $B_k$ is $\sF_k$-measurable}\\
& = & \frac 1{\lm} \sum_{k=1}^n\E\brb{X_n \ind_{B_k}} = \frac 1{\lm} \E\brb{X_n \ind_B}  \leq \frac 1{\lm}\E\brb{X_n^+ \ind_B} \leq \frac 1{\lm}\E\brb{X_n^+},
\eeast
as required.
\end{proof}
...
\be
\pro\brb{\max_{0\leq k\leq n}\abs{X_k -m} \geq \lm} \leq \frac{\var\brb{X_n}}{\lm^2}.
\ee
\end{corollary}
...
When $X_n = \E\brb{Z | \sF_n}$ a.s. for all $n \geq 0$ for some random variable $Z\in \sL^1(\Omega,\sF,\pro)$ then $(X_n)_{n \geq 0}$ is a martingale (by Proposition \ref{pro:conditional_expectation_tower_independence} (tower property)), and if $Z \in \sL^p(\Omega ,\sF,\pro)$ then we say that $X$ is closed in $\sL^p$\index{closed in $\sL^p$}.
\end{definition}
...
\be
X_n = \E\brb{Z | \sF_n}\ \text{ a.s. for all }n.
\ee
...
\be
f(\E(X_n)) \leq \E(f(X_n)) \ \ra \ \brb{\E\abs{X_n}}^p \leq \E\brb{\abs{X_n}^p} \ \ra \ \dabs{X_n}_1 \leq \dabs{X_n}_p
\ee
...
\beast
\E\brb{\abs{X_\infty}^p} & = & \E \brb{\liminf_n \abs{X_n}^p} \qquad(\text{Theorem \ref{thm:non_negative_measurable_property}})\\
& \leq & \liminf_n\E \brb{ \abs{X_n}^p} \qquad  (\text{Fatou's lemma (Lemma \ref{lem:fatou_function})}) \\
...
\be
\E \brb{\abs{X_n - X_\infty}^p} \to 0 \ \ra \ X_n \stackrel{\sL^p}{\to} X_\infty.
\ee
...
\be
\dabs{X_n - \E(X_\infty|\sF_n)}_p = \dabs{\E\brb{X_m - X_\infty|\sF_n}}_p \leq \dabs{X_m - X_\infty}_p \to 0 \text{ as }m\to \infty
\ee
...
If $p = \infty$,
\beast
\dabs{X_n - \E(X_\infty|\sF_n)}_\infty & = & \inf\brb{\lm:\abs{X_n - \E(X_\infty|\sF_n)}\leq \lm \text{ a.s.}} = \inf\brb{\lm:\abs{\E (X_m - X_\infty|\sF_n)}\leq \lm \text{ a.s.}}\\
 & \leq & \inf\brb{\lm:\E (\abs{X_m - X_\infty}|\sF_n)\leq \lm \text{ a.s.}}\quad (\text{conditional Jensen's inequality (Theorem \ref{thm:jensen_inequality_conditional_expectation})})\\
...
\be
\abs{X_n}^p = \brb{\E\brb{\abs{Z}|\sF_n}}^p\leq \E\brb{\abs{Z}^p|\sF_n}\  \ra \ \E\brb{\abs{X_n}^p} \leq \E\brb{\abs{Z}^p} \ \ra \ \sup_{n\geq 0} \E\brb{\abs{X_n}^p} \leq \E\brb{\abs{Z}^p} <\infty.
\ee
...
\be
\dabs{X_n}_\infty = \inf\bra{\lm: \abs{\E\brb{Z|\sF_n}}\leq \lm \text{ a.s.}} \leq \inf\bra{\lm: \E\brb{\abs{Z}|\sF_n}\leq \lm, \text{ a.s.}}
\ee
...
%\be
%F_\infty = \bigvee_{n\geq 0} \sF_n = \sigma\brb{\bigcup_{n\geq 0} \sF_n}.
%\ee
...
\be
X_n \to X_\infty \text{ as }n \to \infty \text{ a.s. and in }\sL^p(\Omega,\sF,\pro)\quad \text{with}\quad X_\infty = \E\brb{Z|\sF_\infty} \text{ a.s.}
\ee
...
\be
\E[Z\ind_A] = \E\brb{\E\brb{Z|\sF_N}\ind_{A}} = \E\brb{X_N\ind_A} \to \E\brb{X_\infty \ind_A} \text{ as }N \to \infty.
\ee

So this shows that for all $A \in \bigcup_{n\geq 0}\sF_n$ we have
\be
\E\brb{X_\infty \ind_A} = \E\brb{\E\brb{Z|\sF_\infty}\ind_A}.
\ee
...
\be
\sL^p(\Omega,\sF_\infty,\pro)\ \to\ \bra{\sL^p\text{-bounded martingales}} : Z \to (\E\brb{Z | \sF_n})_{n \geq 0} \ \text{ is a bijection.}
\ee
\end{remark}
...
\be
\sup_{n\geq 0} \E\brb{|X_n|\ind_{\bra{\abs{X_n}>K}}} \to 0 \ \text{ as } \ K \to \infty.
\ee
\end{definition}
...
\be
\E\abs{X_n - \E(X_\infty|\sF_n)} = \E\abs{\E\brb{X_m - X_\infty|\sF_n}} \leq \E\abs{X_m - X_\infty} \to 0 \text{ as }m\to \infty
\ee
...
%As the proof of Theorem \ref{thm:martingale_bounded_lp_as_lp_closed_discrete}, we have $X_n = \E\brb{X_{n+k}|\sF_n}$, and let $k \to \infty$.
...
\be
\sL^1(\Omega,\sF_\infty,\pro)\ \to \ \{\text{UI martingales}\}: Z \to \brb{\E\brb{Z |\sF_n}}_{n \geq 0}\ \text{ is a bijection.}
\ee
\end{remark}
...
\be
\abs{X_n} = \abs{\E \brb{X_\infty|\sF_n}} \leq \E \brb{\abs{X_\infty}|\sF_n},
\ee
...
\be
X_T = X_\infty \ind_{\bra{T=\infty}} + \sum_{n\geq 0} X_n\ind_{\bra{T=n}} = X_\infty \ind_{\bra{T=\infty}} + \sum_{n\geq 0} \E\brb{X_\infty|\sF_n}\ind_{\bra{T=n}}
\ee
by uniform integrability. Therefore
\be
\abs{X_T} \leq \abs{X_\infty}\ind_{\bra{T=\infty}} + \sum_{n\geq0} \E\brb{\abs{X_\infty}\ind_{\bra{T=n}}|\sF_n}\ \ra\ \E\abs{X_T} \leq \sum_{n\in \Z^+ \cup \bra{\infty}} \E\brb{\abs{X_\infty}\ind_{\bra{T=n}}} = \E\abs{X_\infty} < \infty.
\ee

Thus, $X_T$ is integrable ($X_T\in  \sL^1(\Omega,\sF,\pro)$). Let $A\in \sF_T$. Then since $X_n = \E \brb{X_\infty|\sF_n}$
\beast
\E\brb{X_T\ind_A} & = &  \sum_{n\in \Z^+ \cup \bra{\infty}} \E\brb{X_n \ind_{\bra{T=n}}\ind_A} = \sum_{n\in \Z^+ \cup \bra{\infty}} \E\brb{\E \brb{X_\infty|\sF_n} \ind_{\bra{T=n}}\ind_A} \\
& = & \sum_{n\in \Z^+ \cup \bra{\infty}}\E\brb{X_\infty \ind_{\bra{T=n}}\ind_A}  = \E\brb{X_\infty\ind_A}.
\eeast

Also, clearly $X_T$ is $\sF_T$-measurable, and hence
\be
\E\brb{X_\infty|\sF_T} = X_T\text{ a.s.}
\ee
...
\be
\E\brb{X_T|\sF_S} = \E\brb{\E\brb{X_\infty|\sF_T}|\sF_S} = \E\brb{X_\infty|\sF_S} = X_S \text{ a.s.}
\ee
...
\be
\E\brb{X_0|\sF_n} = \E \brb{\E\brb{X_n|\sF_0}|\sF_n} = X_n \text{ a.s.}.
\ee
...
\be
(b - a)\E\brb{N_{-n}(X,[a, b])} = (b - a)\E\brb{N_{n}(Y,[a, b])} \leq \E\brb{(Y_n - a)^-} = \E\brb{(X_0 - a)^-}.
\ee
...
\be
(b - a)\E\brb{N_{-n}(X,[a, b])} \ua (b - a)\E\brb{N(X,[a, b])} \leq \E\abs{X_0} + a < \infty
\ee
...
\be
\abs{\E\brb{X_0|\sF_n}}^p \leq \E \brb{\abs{X_0}^p|\sF_n} \text{ a.s..} %\E\brb{X_0|\sF_n}
\ee
...
\be
\E\brb{\abs{X_n}^p} = \E \brb{\abs{\E\brb{X_0|\sF_n}}^p} \leq \E\brb{\E \brb{\abs{X_0}^p|\sF_n} } = \E\brb{ \abs{X_0}^p} < \infty
\ee
...
\be
\E\abs{X_{-\infty}}^p = \E \brb{\liminf_n \abs{X_n}^p} \leq \liminf_n \E \brb{\abs{X_n}^p} < \infty.
\ee
...
\be
\abs{X_n - X_{-\infty}}^p = \abs{\E\brb{X_0|\sF_n} - X_{-\infty}}^p = \abs{\E\brb{X_0- X_{-\infty}|\sF_n}}^p \leq \E\brb{\abs{X_0- X_{-\infty}}^p|\sF_n}
\ee
...
\be
\sup_{n\leq 0} \E\brb{\abs{X_n - X_{-\infty}}^p\ind_{\bra{\abs{X_n - X_{-\infty}}^p > K}}} \leq \sup_{n\leq 0} \E\brb{\abs{Z_n}\ind_{\bra{\abs{Z_n}>K}}} \to 0 \text{ as }K\to \infty.
\ee
...
\be
\E\brb{X_n\ind_A} = \E\brb{\E\brb{X_0|\sF_n}\ind_A} = \E\brb{\E\brb{X_0\ind_A|\sF_n}} = \E\brb{X_0\ind_A}
\ee

Letting $n \to -\infty$ in the above equality and using the $\sL^1(\Omega,\sF,\pro)$ convergence of $X_n$ to $X_{-\infty}$,
\be
\E\brb{X_0\ind_A} = \E\brb{X_n\ind_A} \to \E\brb{X_{-\infty}\ind_A}.
\ee
...
\be
\E\brb{M_{m+1}|\sF_m} = \E \brb{\left.\frac{S_{-m-1}}{-m-1}\right|\sG_{-m}}.
\ee
...
\be
\E\brb{\left. \frac{S_{n-1}}{n-1}\right|\sG_n} = \E\brb{\left. \frac{S_n - X_n}{n-1}\right|\sG_n} = \frac {S_n}{n-1} - \E\brb{\left. \frac{X_n}{n-1}\right|\sG_n} \text{ a.s.}
\ee
...
\be
\E\brb{X_1|\sG_n} + \dots + \E\brb{X_n|\sG_n} = \E\brb{S_n|\sG_n} = S_n \text{ a.s.} \ \ra \ \E\brb{X_n|\sG_n} = S_n/n \text{ a.s.}
\ee


Finally putting everything together we get
\be
\E\brb{M_{-n+1}|\sF_{-n}} =  \E\brb{\left. \frac{S_{n-1}}{n-1}\right|\sG_n} = \frac {S_n}{n-1} - \frac{S_n}{n(n-1)} = \frac{S_n}{n} = M_{-n} \text{ a.s.}
\ee

Also $M_{-1} = X_1$ is integrable, so $M$ is a backwards martingale. Then by backwards martingale convergence theorem (Theorem \ref{thm:backwards_martingale_as_lp_closed_discrete}, $X_1\in \sL^1(\Omega,\sF,\pro)$), $M_{-n} = \frac{S_n}{n}$ converges a.s. and in $\sL^1(\Omega,\sF,\pro)$ to a random variable, $M_{-\infty}\in \sL^1(\Omega,\sF,\pro)$. Obviously, martingale property and dominated convergence theorem (Theorem \ref{thm:dominated_convergence_probability}) give that %$\lim \E\brb{M_{-n}} = \E\brb{M_{-\infty}}$. By
\be
\mu = \E\brb{X_1} = \E\brb{\frac {X_1}1} = \E\brb{M_{-1}} = \E \brb{M_{-n}} \to \E\brb{M_{-\infty}}, \quad (*)
\ee
...
\be
\E\brb{X_{n+1}|\sF_n} = \E\brb{Y_{n+1}X_n | \sF_n} = X_n \E\brb{Y_{n+1}| \sF_n} = X_n \E\brb{Y_{n+1}} = X_n,
\ee
...
\be
\E\brb{M^2_n} = \E\brb{\frac{X_n}{\prod^n_{i=1} a^2_i}} = \frac 1{\prod^n_{i=1} a^2_i} \leq \frac 1{\prod_{i\geq0} a^2_i} <\infty.
\ee

Therefore $M$ is bounded in $\sL^2(\Omega,\sF,\pro)$. Doob's inequality (Theorem \ref{thm:doob_lp_inequality_discrete}) gives that
\be
\E\brb{(M^*_n)^2} \leq \brb{\frac 2{2-1}}^2 \E\brb{M^2_n} \leq \frac 4{\prod_{i\geq 1} a^2_i} < \infty,
\ee
...
\be
\E \brb{\sup_{1\leq i\leq n}\abs{X_i}} \leq \E \brb{\sup_{1\leq i\leq n}\abs{M_n}^2} = \E \brb{\brb{\sup_{1\leq i\leq n}\abs{M_n}}^2} = \E\brb{(M^*_n)^2}  < \infty.
\ee
...
%Therefore $\sup_{n\geq 0} \E\brb{(M^*_n)^2} <\infty$. Then $M^*_\infty = \sup_{n\geq0} \frac 1{\prod^n_{i=1} a_i} \sqrt{X_n}$, which implies that
%\be
...
\be
1 = \E\brb{X_n}\to \E\brb{X_\infty}
\ee
by dominated convergence theorem (Theorem \ref{thm:dominated_convergence_probability}).

(ii) $\ra$ (iii): If $\pro(X_\infty >0) = 0$, $X_\infty = 0$ a.s.. Thus, $\E(X_\infty)= 0$ by Theorem \ref{thm:lebesgue_integrable_function_property}. Contradiction. %If $\E\brb{X_\infty} > 0$ then $\pro(X_\infty > 0) > 0$ since $X_\infty \geq 0$.
\end{proof}
...
%\E^\pro\brb{\E^{\pro}\brb{X_{n+1}|\sF_n}\ind_A} & = & \E^\pro\brb{\E^{\pro}\brb{X_{n+1}\ind_A|\sF_n}}  = \E^{\pro}\brb{X_{n+1}\ind_A} \\
%& = & \Q(A) = \E^{\pro_n}\brb{X_{n+1}|\sF_n}
%\eeast
%\beast
%\E^{\pro}\brb{X_{n+1}\ind_A|\sF_n} & = & \E^\pro\brb{\E^{\pro}\brb{X_{n+1}\ind_A|\sF_n}|\sF_{n+1}}  = \E^\pro\brb{\E^{\pro}\brb{1|\sF_n}X_{n+1}\ind_A|\sF_{n+1}}  \\
%& = & \E^\pro\brb{X_{n+1}\ind_A|\sF_{n+1}} = \Q(A|\sF_{n+1}) \stackrel{\text{a.e.}}{=}  \Q(A|\sF_n) = \E^{\pro}\brb{X_n\ind_A|\sF_n}
%\eeast
...
\be
\Q(A) = \E^\pro\brb{X_m\ind_A} = \E^\pro\brb{X_n\ind_A} \to \E^\pro\brb{X_\infty \ind_A}
\ee
...
\be
\Q(A) = \E^\pro\brb{X\ind_A} = \E^\pro\brb{\E^\pro\brb{X |\sF_n}\ind_A}
\ee
...
\be
\Q(B_n \text{ i.o.}) = \Q\brb{\limsup B_n} \geq \limsup \Q(B_n) \geq \ve > 0.%\Q\brb{\bigcup_{k\geq n} B_k} \geq \Q(B_n) \geq \ve.
\ee
...
\be
\Q(A) = \brb{\frac{\Q(A)}{\pro(A)}} \pro(A) = \E^\pro\brb{\frac{\Q(A)}{\pro(A)} \ind_A} = \E^\pro\brb{\ind_A \sum_{B\in \sA_n} \frac{\Q(B)}{\pro(B)}\ind_B}
\ee

%Indeed, if $A= A^\ve$ for some $\ve \in \{0, 1\}^n$ then
%\be
%\Q_n(A) = \frac{\Q_n(A)}{\pro_n(A)} P_n(A) = \E^{\pro_n} \brb{\frac{\Q(A)}{\pro(A)} \ind_A} = \E^{\pro_n}\brb{\ind_A \sum_{\ve\in \{0,1\}^n} \frac{\Q(A^\ve)}{\pro(A^\ve)} \ind_{A^\ve}}.
%\ee

Therefore $\Q(A) = \E\brb{X_n\ind_A}$ for all $A\in \sF_n$ ($*$).% By linearity this holds for all $A \in \sF_n$.
...
\be
\sup_{n\geq 0} \E^\pro\brb{X_n\ind_{\bra{X_n>\lm}}} \to 0\text{ as }\lm \to \infty.
\ee
...
\be
\pro(X_n > \lm) \leq \frac {\E\brb{X_n}}{\lm}  = \frac {\Q(\Omega)}{\lm} = \delta \ \ra \ \Q(X_n > \lm) = \E^\pro\brb{X_n\ind_{\bra{X_n>\lm}}} < \ve.
\ee
...
\ben
\item [(i)] $X$ is a martingale\index{martingale!continuous} with respect to $((\sF_t)_{t\geq 0},\pro)$ if $\E\brb{X_t | \sF_s} = X_s$ a.s. for all $t \geq s$.
\item [(ii)] $X$ is a supermartingale\index{supermartingale!continuous} with respect to $((\sF_t)_{t\geq 0},\pro)$ if $\E\brb{X_t | \sF_s}\leq X_s$ a.s. for all $t\geq s$.
\item [(iii)] $X$ is a submartingale\index{submartingale!continuous} with respect to $((\sF_t)_{t\geq 0},\pro)$ if $\E\brb{X_t | \sF_s} \geq X_s$ a.s. for all $t\geq s$.
\een
...
\be
\abs{X_s} = \abs{\E \brb{X_t|\sF_s}} \leq \E\brb{\abs{X_t}|\sF_s} \text{ a.s.}\ \ra \ \abs{X} \text{ is a submartingale.}
\ee

Also, $X = X^+ - X^-$ and $\abs{X} = X^+ + X^-$, by Proposition \ref{pro:conditional_expectation_basic_property}.(iii),(iv)
\beast
X_s^+ & = & \frac 12 \brb{\abs{X_s} + X_s} \leq \frac 12 \brb{\E\brb{\abs{X_t}|\sF_s} + \E \brb{X_t|\sF_s}} = \E \brb{X_t^+|\sF_s} \text{ a.s.}.\\
X_s^- & = & \frac 12 \brb{\abs{X_s} - X_s} \leq \frac 12 \brb{\E\brb{\abs{X_t}|\sF_s} - \E \brb{X_t|\sF_s}} = \E \brb{X_t^+|\sF_s} \text{ a.s.}.
\eeast
...
\be
\E\brb{\left.X^2_t -X^2_s \right|\sF_s} = \E\brb{\left.(X_t -X_s)^2\right|\sF_s}, \quad\text{a.s.}.
\ee
\end{lemma}
...
\be
\E\brb{\left(X_t -X_s)^2\right|\sF_s} \stackrel{\text{a.s.}}{=} \E\brb{\left.X^2_t \right|\sF_s} - 2X_s\E(X_t|\sF_s) + X^2_s \stackrel{\text{a.s.}}{=} \E\brb{\left.X^2_t \right|\sF_s} - 2X^2_s + X^2_s \stackrel{\text{a.s.}}{=} \E(X^2_t -X^2_s |\sF_s).
\ee
...
\be
\E\brb{\brb{\sum^{n-1}_{k=0} (M_{t_{k+1}} -M_{t_k})^2}^2}\quad \text{is bounded.}
\ee
\end{lemma}
...
First note that
\be
\E\brb{\brb{\sum^{n-1}_{k=0}(M_{t_{k+1}} -M_{t_k} )^2}^2} = \sum^{n-1}_{k=0} \E\brb{(M_{t_{k+1}} -M_{t_k} )^4} + 2\sum^{n-1}_{k=0} \E\brb{(M_{t_{k+1}} -M_{t_k})^2\sum^{n-1}_{j=k+1} (M_{t_{j+1}} -M_{t_j} )^2}.\quad\quad (*)\nonumber
\ee

For each fixed $k$ we have (by property of martingale and Lemma \ref{lem:sl2_martingale_trick}),
\beast
\E\brb{(M_{t_{k+1}} -M_{t_k} )^2\sum^{n-1}_{j=k+1} (M_{t_{j+1}} -M_{t_j} )^2} & = & \E\brb{(M_{t_{k+1}} -M_{t_k} )^2\E\brb{\left.\sum^{n-1}_{j=k+1} (M_{t_{j+1}} -M_{t_j} )^2\right|\sF_{t_{k+1}} }}\\
& = & \E\brb{(M_{t_{k+1}} -M_{t_k} )^2\E\brb{\left.\sum^{n-1}_{j=k+1} (M^2_{t_{j+1}} -M^2_{t_j})\right|\sF_{t_{k+1}} }}\\
& = & \E\brb{(M_{t_{k+1}} -M_{t_k} )^2\E\brb{\left. M^2_{t_n} -M^2_{t_{k+1}} \right|\sF_{t_{k+1}} }}\\
& = & \E\brb{(M_{t_{k+1}} -M_{t_k} )^2\brb{M^2_{t_n} -M^2_{t_{k+1}} }}
\eeast

After inserting this in ($*$) we get the estimate
\beast
\E\brb{\sum^{n-1}_{k=0}(M_{t_{k+1}} -M_{t_k} )^2} & \leq & \E\brb{\sup_{0\leq j<n}\abs{M_{t_{j+1}} -M_{t_j}}^2 \sum^{n-1}_{k=0} (M_{t_{k+1}} -M_{t_k} )^2 + 2 \sup_{0\leq j<n}\abs{M_{t_l} -M_{t_j} }^2 \sum^{n-1}_{k=0} (M_{t_{k+1}} -M_{t_k} )^2}\\
& = & \E\brb{\brb{\sup_{0\leq j<n}\abs{M_{t_{j+1}} -M_{t_j}}^2 + 2 \sup_{0\leq j<n}\abs{M_{t_l} -M_{t_j} }^2} \sum^{n-1}_{k=0} (M_{t_{k+1}} -M_{t_k} )^2}.
\eeast
...
\beast
\E\brb{\brb{\sum^{n-1}_{k=0} (M_{t_{k+1}}-M_{t_k})^2}^2} & \leq & \E\brb{\brb{4C^2 + 2\cdot 4C^2} \sum^{n-1}_{k=0} (M_{t_{k+1}} -M_{t_k} )^2} = 12C^2\E\brb{\sum^{n-1}_{k=0}(M_{t_{k+1}}-M_{t_k})^2}\\
& = & 12C^2\E\brb{(M_{t_n} -M_{t_0})^2} \leq 12C^2 \cdot 4C^2 = 48C^4.
\eeast
\end{proof}
...
\be
X_t = \E\brb{\wt{X}_t|\sF_t}\text{ a.s. for all }t \geq 0.
\ee
...
\be
\lm \pro\brb{\max_{j\in J} \abs{X_j} \geq \lm} \leq \E\abs{X_{j_n}} \leq \E\abs{X_K}.
\ee
...
\be
\lm \pro\brb{\sup_{t\in I} \abs{X_t} \geq \lm} = \lm \E\brb{\ind_{\bra{\sup_{t\in I} \abs{X_t} \geq \lm}}} = \lm \lim_{n\to \infty}  \E \brb{\ind_{\bra{\max_{j\in J} \abs{X_j} \geq \lm}}}= \lm \lim_{n\to \infty} \pro\brb{\max_{j\in J} \abs{X_j} \geq \lm} \leq \E\abs{X_K} < \infty.
\ee
since $X$ is a martingale with integrability. Therefore by letting $\lm  \to \infty$ this shows that
\be
\pro\brb{\sup_{t\in I} \abs{X_t} < \infty} = 1.
\ee
...
\be
(b - a)\E\brb{N(X,J,[a, b])} \leq \E\brb{\brb{X_{a_n} - a}^-} \leq \E\brb{\brb{X_K - a}^-}\quad \quad (*)
\ee
...
\be
(b - a)\E\brb{N(X,I_M,[a, b])} = (b - a)\lim_{n\to \infty} \E\brb{N(X,J,[a, b])} \leq \E\brb{\brb{X_K - a}^-} \leq \E\abs{X_K -a} \leq \E\abs{X_K} + \E\abs{a} < \infty.
\ee
where $K = \sup I_M$. The last inequality is from the fact that $X$ is a martingale. Thus,
\be
\E\brb{N(X,I_M,[a, b])} < \infty \ \ra \ N(X,I_M,[a, b]) < \infty\text{ a.s.}
\ee
...
Therefore, by conditional dominated convergence theorem (Theorem \ref{thm:dominated_convergence_conditional_expectation}),%\beast \E \brb{\E\brb{X_{t_n}|\sF_t} - \E\brb{\wt{X}_t|\sF_t}} & = & \E \brb{X_t - \E\brb{\wt{X}_t|\sF_t}} \eeast
\be
\E\brb{X_{t_n} |\sF_t} \to \E\brb{\wt{X}_t|\sF_t} \quad \text{ a.s.}
\ee

But $\E\brb{X_{t_n}|\sF_t} = X_t$ since $X$ is a martingale. Therefore,
\be
X_t = \E\brb{\wt{X}_t|\sF_t} \quad \text{ a.s.}.
\ee
...
\be
\wt{X}_s \stackrel{\text{a.s.}}{=} \lim X_{s_n}  \stackrel{\text{a.s.}}{=}  \lim \E\brb{X_t|\sF_{s_n}}.
\ee
...
\be
\wt{X}_s = \E\brb{X_t|\sF_{s^+}}\quad \text{ a.s.}\quad\quad (\dag)
\ee

If s < t, then by the tower property and ($\dag$) we get that for any $r\geq t$,
\be
\E\brb{\wt{X}_t|\sF_{s^+}} = \E\brb{\E\brb{X_r|\sF_{t^+}}|\sF_{s^+}} \stackrel{\text{a.s.}}{=}  \E \brb{X_r| \sF_{s^+}} \stackrel{\text{a.s.}}{=}  \wt{X}_s.
\ee

Then by Proposition \ref{pro:conditional_expectation_tower_independence}.(vi), we get that
\be
\E\brb{\left.\wt{X}_t\right|\wt{\sF}_s} = \E\brb{\left.\wt{X}_t\right|\sigma(\sF_{s^+},\sN)} = \E\brb{\left.\wt{X}_t\right|\sF_{t^+}} \ \text{ a.s..}
\ee

Then with $(\dag)$, we have
\be
\E\brb{\left.\wt{X}_t\right|\wt{\sF}_s} = \wt{X}_s\ \text{ a.s.}
\ee
...
\be
\E\brb{N(\wt{X}(\omega),I_M,[a, b])} \leq \E\brb{N(X,I_M,[a, b])} < \infty \ \ra \ N(\wt{X},I_M,[a, b]) < \infty\text{ a.s.}
\ee

Also, if $t\in \Q^+$, we have $\wt{X}_t(\omega) = \lim_{s\da t,s\in \Q^+}X_s(\omega) = X_t(\omega)$.
\be
\pro\brb{\sup_{t\in I} \abs{\wt{X}_t} < \infty} = \pro\brb{\sup_{t\in I} \abs{X_t} < \infty} = 1.
\ee
...
\be
\lm \pro\brb{X^*_t \geq \lm} \leq \E X_t.
\ee
\end{theorem}
...
\be
\lm \pro\brb{\sup_{0\leq s\leq t}\abs{X_s} \geq \lm} \leq \E \abs{X_t}.
\ee
...
We observe that
\be
X^*_t = \sup_{s\leq t} \abs{X_s} = \sup_{s\in \bra{t} \cup \brb{[0,t] \cap \Q}} \abs{X_s}
\ee
...
\be
X^*_t = \sup_{\substack{J\subseteq \bra{t}\cup \brb{[0,t]\cap\Q} \\ J\text{ is finite}}} \max_{s\in J} \abs{X_s}.
\ee
...
\be
\lm \pro\brb{\max_{s\in J}\abs{X_s} \geq \lm} \leq \E\abs{X_{t_k}} \leq \E\abs{X_t}.
\ee
...
\be
\ind_{\bra{\max_{s\in J}\abs{X_s} \geq \lm}} \ua \ind_{\bra{X^*_t \geq \lm}} \ \ra \ \pro\brb{\max_{s\in J}\abs{X_s} \geq \lm} \ua \pro\brb{X^*_t \geq \lm}
\ee
which gives the required result.%\be %a \pro\brb{\max_{s\in J} \abs{X_s} \geq a} \leq \E[\abs{X_t}] \leq \E[\abs{X_t}] %\ee
\end{proof}
...
\be
\brb{\max_{s\in J}\abs{X_s}}^p \ua \brb{X^*_t}^p \ \ra \ \E\abs{\max_{s\in J}\abs{X_s}}^p \ua \E\abs{X^*_t}^p \ \ra \ \dabs{\max_{s\in J}\abs{X_s}}_p = \brb{\E\abs{\max_{s\in J}\abs{X_s}}^p}^{1/p} \ua \brb{\E\abs{X^*_t}^p}^{1/p} = \dabs{X^*_t}_p\nonumber
\ee

Hence,
\be
\dabs{X^*_t}_p \leq \frac p{p-1}\dabs{X_t}_p.%= \dabs{\sup_{\substack{J\subseteq \bra{t}\cup \brb{[0,t]\cap\Q} \\ J\text{ is finite}}} \max_{s\in J} \abs{X_s}}_p \not\leq \sup_{\substack{J\subseteq \bra{t}\cup \brb{[0,t]\cap\Q} \\ J\text{ is finite}}} \dabs{\max_{s\in J} \abs{X_s}}_p
\ee%as required.
\end{proof}
...
\be
\pro\brb{X_t^* \geq \lm} \leq \frac{\E X_t^+}{\lm}.
\ee
\end{theorem}
...
\be
\pro\brb{\max_{s\in J} X_s \geq \lm} \leq \frac{\E X_{t_k}\ind_B}{\lm},\qquad B = \bra{\max_{1\leq i\leq k} X_{t_i} \geq \lm}
\ee
and $B$ is $\sF_{t_k}$-measurable. Since $X$ is submartingale, we have
\be
\E\brb{X_t|\sF_t} \geq X_{t_k} \text{ a.s.} \ \ra \ \brb{\E\brb{X_t|\sF_t} - X_{t_k}}\ind_B \geq 0\text{ a.s.} \ \ra \ \E\brb{\brb{\E\brb{X_t|\sF_t} - X_{t_k}}\ind_B} \geq 0
\ee
which implies that $\E\brb{X_t\ind_B} \geq \E\brb{X_{t_k}\ind_B}$. Thus,
\be
\pro\brb{\max_{s\in J} X_s \geq \lm} \leq \frac{\E X_{t_k}\ind_B}{\lm} \leq \frac{\E X_{t}\ind_B}{\lm} \leq \frac{\E X_t^+\ind_B}{\lm} \leq \frac{\E X_t^+}{\lm}.
\ee
...
Therefore, by monotone convergence theorem (Theorem \ref{thm:monotone_convergence_probability})
\be
\ind_{\bra{\max_{s\in J} X_s}} \ua \ind_{\bra{X^*_t}} \ \ra \ \pro\brb{\max_{s\in J} X_s \geq \lm} \ua \pro\brb{X_t^* \geq \lm}.
\ee

Hence,
\be
\pro\brb{\sup_{s\in [0,t]} X_s \geq \lm} \leq \frac{\E X_t^+}{\lm}.%= \dabs{\sup_{\substack{J\subseteq \bra{t}\cup \brb{[0,t]\cap\Q} \\ J\text{ is finite}}} \max_{s\in J} \abs{X_s}}_p \not\leq \sup_{\substack{J\subseteq \bra{t}\cup \brb{[0,t]\cap\Q} \\ J\text{ is finite}}} \dabs{\max_{s\in J} \abs{X_s}}_p
\ee%as required.
\end{proof}
...
\be
\pro\brb{\sup_{s\leq t}\abs{X_s -m} \geq \lm} \leq \frac{\var\brb{X_t}}{\lm^2}.
\ee
\end{corollary}
...
\be
(b - a)\E\brb{N(X,I_M,[a, b])} \leq \E\brb{\brb{X_{a_n} -a}^-} \leq \E\brb{\abs{X_K} + a} \leq a+ \sup_{t\geq 0} \E\abs{X_t} < \infty,
\ee
...
\be \dabs{\sup_{0\leq s \leq t}\abs{X_s}}_p \leq 2\dabs{X_t}_p \leq \frac p{p - 1}\brb{\dabs{X_\infty}_p + \dabs{X_t - X_\infty}_p } \to \frac p{p - 1}\dabs{X_\infty}_p. \ee
...
\item [(ii)] $X^T = \brb{X_{T\land t}}_{t\geq 0}$ is a (\cadlag) martingale with respect to $((\sF_t)_{t\geq 0},\pro)$ for all stopping times $T$.%\footnote{maybe for all stopping times}.
...
\be
\E\brb{\left.X^{T_n}_{t_n}\right|\sF_{s_n}} = \E\brb{\left.Y^{2^nT_n}_{2^n t_n}\right|\sG_{2^n s_n}} \stackrel{\text{a.s.}}{=} Y^{2^nT_n}_{2^n s_n} = X^{T_n}_{s_n}.
\ee
...
\be
E\brb{X_{T_n\land t_n}\ind_{A_n}} = \E\brb{X_{T_n\land s_n}\ind_{A_n}} \text{ for any }A_n\in \sF_{s_n}.
\ee
...
\be
\left\{\ba{l}
\E\brb{X_{T_n\land t_n}\ind_{A_n}} \to \E\brb{X_{T \land t}\ind_A} \\
\E\brb{X_{s_n}\ind_{A_n}} \to \E\brb{X_{T\land s}\ind_A}
\ea\right. \ \ra \ \E\brb{X_{T \land t}\ind_A} = \E\brb{X_{T \land s}\ind_A} \ \ra \ \E\brb{X_{T\land t}|\sF_s} = X_{T\land s}\ \text{ a.s..}
\ee
%\be
%\E\brb{X^T_t|\sF_s} = \E \brb{X_t\ind_{\bra{T>t}} + X_T\ind_{\bra{T\leq t}}|\sF_s} %\E \brb{X_{T\land t}\ind_{\bra{T>s}} + X_{T\land t}\ind_{\bra{T\leq s}}|\sF_s} = \E \brb{X_{T\land t}|\sF_s} \underbrace{\ind_{\bra{T>s}}}_{\bra{T>s} \in \sF_s} \stackrel{\text{a.s.}}{=}   %\E \brb{X_t\ind_{\bra{T>t}} + X_T\ind_{\bra{T\leq t}}|\sF_s} = \E \brb{X_t + (X_T-X_t)\ind_{\bra{T\leq t}}|\sF_s}
%\ee
...
\be
\E\brb{X_{T_n}|\sF_{S_n}} = X_{S_n}\ \text{ a.s.} \ \ra \ \E\brb{X_{T_n}\ind_{A_n}} = \E \brb{X_{S_n}\ind_{A_n}}\quad \text{for }A_n \in \sF_{S_n}.
\ee
...
\be
\left\{\ba{l}
\E\brb{X_{T_n}\ind_{A_n}} \to \E\brb{X_{T}\ind_A} \\
\E\brb{X_{S_n}\ind_{A_n}} \to \E\brb{X_{S}\ind_A}
\ea\right. \ \ra \ \E\brb{X_{T}\ind_A} = \E\brb{X_{S}\ind_A} \ \ra \ \E\brb{X_{T}|\sF_s} = X_{S}\ \text{ a.s..}
\ee
...
\be
\E\brb{X_T\ind_A} = \E\brb{X_S\ind_A}.
\ee
...
\be
\E\brb{X_{T_n}|\sF_{S_n}} = X_{S_n}\ \text{ a.s.} \quad \quad (*)
\ee
...
\be
\E\brb{X_{T_n} \ind_A} = \E\brb{X_{S_n}\ind_A}
\ee
...
\be
X_{T_n}\ind_A \to X_T\ind_A \ \ra \ \E\brb{X_{T_n}\ind_A} \to \E\brb{X_T\ind_A}.
\ee
...
\be
\E\brb{X_t|\sF_s} = X_s \text{ a.s.}.\qquad (*)
\ee
...
\be
\abs{\E\brb{X^n_t\ind_A} - \E\brb{X_t\ind_A}} = \abs{\E\brb{(X^n_t - X_t)\ind_A}} \leq \E\brb{\abs{X^n_t - X_t}\ind_A} \leq \E\abs{X^n_t - X_t} \to 0 \ \ra \ \E\brb{X^n_t\ind_A} \to \E\brb{X_t\ind_A}.
\ee
...
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s - X^m_s} \geq \ve} \leq \frac 1{\ve} \E\abs{X^n_t - X^m_t} \leq \frac 1{\ve} \brb{\E\abs{X^n_t - X_t} + \E\abs{X^m_t - X_t}} \to 0
\ee
...
\be
\pro\brb{\abs{X^n_t - Y_t} > \ve} \leq \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s - Y_s} > \ve} \to 0\quad \text{as }n\to \infty \ \ra \ X^n_t \xrightarrow{p} Y_t.
\ee
...
%That is,
%\be
%\pro\brb{\abs{X^n_t - X_t} > \ve} \to 0\quad\text{as }n\to \infty.
%\ee
%\footnote{then so what?}
%Thus, since $X^n$ and $Y$ are \cadlag,
%\beast
%\pro\brb{\abs{X_t - Y_t} > \ve} & \leq & \pro\brb{\abs{X^n_t - X_t} + \abs{X^n_t - Y_t} > \ve} \\
%& = & \pro\brb{\bigcup_{q\in \Q^{++},q \leq \ve} \bra{\abs{X^n_t - Y_t} > q} \cap \bra{\abs{X^n_t -X_t} > \ve-q}}\\
%& \leq & \sum_{q\in \Q^{++},q \leq \ve} \pro\brb{\abs{X^n_t - Y_t} > q} \to 0. %\leq \pro\brb{\abs{X^n_t - X_t} > \ve} + \pro\brb{\abs{X^n_t - Y_t} > \ve/2}
%\eeast
%Thus, $\pro\brb{\abs{X_t - Y_t} > \ve} = 0$
\end{proof}
...
\be
\pro\brb{X_t^{T_n}(\omega) = \wt{X}_t^{T_n}(\omega)} = 1 \ \ra \ X_t^{T_n} = \wt{X}_t^{T_n} \text{ a.s.}
\ee
...
\be
\E\brb{X_{m+1} \ind_A} = \E\brb{X_{m+1}^{T_k} \ind_{\bra{T_k > m}} \ind_A} = \E\brb{X_m^{T_k} \ind_{\bra{T_k > m}} \ind_A} = \E\brb{X_m \ind_A}.
\ee
...
\be
(M+N)^{T_n} = \brb{(M+N)^{S_n}}^{S_n'} \in \sM_c,\quad (M-N)^{T_n} = \brb{(M-N)^{S'_n}}^{S_n} \in \sM_c.
\ee
...
\be
\sup_{T\leq t}\E\brb{\abs{X_T}\ind_{\abs{X_T}\geq K}} = \sup_{T\leq t}\E\brb{\abs{\E(X_t|\sF_T)}\ind_{\abs{\E(X_t|\sF_T)}\geq K}} \to 0 \quad\text{ as }K\to \infty.
\ee
...
\be
\sup_{T\leq t} \E\brb{\abs{X_T}\ind_{\bra{\abs{X_T} > K}}} \leq \sup_{T\leq t} M \E\brb{\ind_{\bra{\abs{X_T}\geq K}}} = M \sup_{T\leq t} \pro\brb{\abs{X_T}\geq K} \to 0 \ \text{ as }K\to \infty.
\ee
...
%\be
%\sup_{T\leq t} \E\brb{\abs{X_T}\ind_{\bra{\abs{X_T} > K}}} \leq \sup_{t\geq 0} \E\brb{\abs{X_t}\ind_{\bra{\abs{X_t} > K}}}  \to 0 \ \text{ as }K\to \infty.
%\ee
%
%Thus, $\sX_t = \bra{X_T : T \text{ is a stopping time, }T \leq t}$ is UI. By Proposition \ref{pro:martingale_local_martingale_equivalent}, we have $X$ is a martingale.
%\end{proof}
...
Then By Proposition \ref{pro:martingale_local_martingale_equivalent}, we have $X$ is a martingale. %recall definition of UI (Theorem \ref{thm:ui_equivalent_probability}) %\be \sup_{T\leq t} \E\brb{\abs{X_T}\ind_{\bra{\abs{X_T} > K}}} \leq \sup_{T\leq t} \E\brb{Y\ind_{\bra{Y > K}}}  = \E\brb{Y\ind_{\bra{Y > K}}} \to 0 \ \text{ as }K\to \infty \ee as we know that single integrable  Thus,
\end{proof}
...
\beast
0 & \leq & \E\abs{\E\brb{X_t|\sF_s} - X_s} = \E\abs{\E\brb{X_t|\sF_s} - X_s - \E\brb{\left.X_t^{T_n}\right|\sF_s} + X^{T_n}_s}\\
& \leq & \E\abs{\E\brb{\left.X_t^{T_n} - X_t\right|\sF_s}} + \E\abs{X^{T_n}_s - X_s} \leq \E\brb{\E\brb{\abs{X_t^{T_n} - X_t}\sF_s}} + \E\abs{X^{T_n}_s - X_s} \\
...
\be
\E\abs{\E\brb{X_t|\sF_s} - X_s} = 0 \ \ra \ \E\brb{X_t|\sF_s} - X_s = 0 \text{ a.s.}
\ee
as required.
\end{proof}
...
\be
\E\brb{ \sup_{n\geq 0}\abs{ X_t^{T_n} }} < \infty \quad \text{for every }t,
\ee
...
\be
\E\brb{\sup_{s\in[0,t]} \abs{X_s}} < \infty\quad\text{ for every }t,
\ee
...
\be
\abs{X^{T_n}_t} = \abs{X^{T_n\land t}} \leq \sup_{s\in[0,t]} |X_s|  \ \ra\ \sup_{n\geq 0}\abs{ X_t^{T_n} } \leq \sup_{s\in[0,t]} |X_s| \ \ra\  \E\brb{\sup_{n\geq 0}\abs{ X_t^{T_n} }} \leq  \E\brb{\sup_{s\in[0,t]} |X_s|} < \infty
\ee
...
\be
\sup_{t\geq 0}\E\brb{\abs{X_t} \ind_{\abs{X_t} > K}} \leq \E\brb{\sup_{t\geq 0} \abs{X_t} \ind_{\sup_{t\geq 0} \abs{X_t} > K}} \to 0 \quad  \text{as }K\to \infty.
\ee
%Therefore, for all $t$, by dominated convergence theorem (Theorem \ref{thm:dominated_convergence_probability})
%\be
%\E\brb{\abs{X_t} \ind_{\abs{X_t} > K}} \to 0
%\ee       % $\sup_{t\geq 0}\E\brb{\abs{X_t} \ind_{\abs{X_t} > K}} \to 0$ as $K \to \infty$ and this implies that
...
\be
\E\brb{\left.X_t^{T_n}\right|\sF_s} = X^{T_n}_s \ \text{ a.s.}.\quad \quad (*)
\ee

We know that $\liminf_{n\to \infty} X^{T_n}_t = X_t$ a.s., $\liminf_{n\to \infty} X^{T_n}_s = X_s$ a.s.. Thus, for any $A\in \sF_s$, we have%\E \brb{\E\brb{\left.\liminf_{n\to \infty} X^{T_n}_t\right| \sF_s}\ind_A}
\be
\E \brb{\liminf_{n\to \infty} X^{T_n}_t \ind_A} = \E \brb{X_t \ind_A} \quad (\text{by Theorem \ref{thm:non_negative_measurable_property} as $\liminf_{n\to \infty} X^{T_n}_t \ind_A = X_t \ind_A$ a.s.})
\ee

Thus, by Theorem \ref{thm:conditional_expectation_existence_uniqueness},
\be
\E\brb{\left.\liminf_{n\to \infty} X^{T_n}_t\right| \sF_s} = \E\brb{X_t|\sF_s} \ \text{ a.s..}\quad\quad (**)
\ee

Also, by Fatou's lemma for conditional expectations (Theorem \ref{thm:fatou_conditional_expectation}, $X^{T_n}_t\geq 0$),
\be
\E\brb{\left.\liminf_{n\to \infty} X^{T_n}_t\right| \sF_s} \leq \liminf_{n\to \infty} \E\brb{\left.X^{T_n}_t \right|\sF_s}\ \text{ a.s.}\quad (\dag)
\ee

Then substituting ($*$) and ($**$) into the equation ($\dag$),
\beast
\E\brb{\left.\liminf_{n\to \infty} X^{T_n}_t\right| \sF_s} & \leq & \liminf_{n\to \infty} X^{T_n}_s\ \text{ a.s.}\\
\E\brb{\left. X_t\right| \sF_s} & \leq & X_s\ \text{ a.s.}
\eeast
...
\be
\E\brb{M^2_t} = \E\brb{\sum^{N-1}_{k=0} \brb{M^2_{t_{k+1}} -M^2_{t_k}}} = \E\brb{\sum^{N-1}_{k=0}\brb{M_{t_{k+1}} -M_{t_k}}^2} \leq  \E\brb{\underbrace{\sup_{k<N} \abs{M_{t_{k+1}} -M_{t_k}}}_{\leq 2 n} \underbrace{\sum_{0\leq k <N} \abs{M_{t_{k+1}} -M_{t_k}}}_{\leq \abs{V^{S_n}} \leq n}
}.\nonumber
\ee
...
\be
\E\brb{\sup_{k<N}\abs{M_{t_{k+1}} -M_{t_k}} \sum^{N-1}_{k=0} \abs{M_{t_{k+1}} -M_{t_k}}} \to 0 \quad\quad \text{as }N \to \infty.
\ee
...
\be
\pro\brb{M^{(n)}(\omega) = 0,\forall t} = 1.
\ee

Thus, since $S_n \ua \infty$ a.s.,
\beast
\pro\brb{X_t(\omega) = 0,\forall t} & = & \pro\brb{\bigcap_{n\in \N} X_t^{S_n}(\omega) = 0,\forall t} = 1 - \pro\brb{\bigcup_{n\in \N} X_t^{S_n}(\omega) \neq 0,\exists t}\\
& \geq & 1 - \sum_{n\in \N} \pro\brb{X_t^{S_n}(\omega) \neq 0,\exists t} = 1 - \sum_{n\in \N} \pro\brb{M^{(n)}(\omega) \neq 0,\exists t} \\
& = & 1 -\brb{1 - \sum_{n\in \N}\brb{1- \pro\brb{M^{(n)}(\omega) = 0,\forall t} }}=  1- \sum_{n\in \N} 0 = 1- 0 = 1.
\eeast
...
%$\sF_n=\sigma(X_0,X_1,\dots,X_n)$. \beast \E \brb{\left.X_{n+1}\right|\mathcal{F}_n} & = & \E \brb{\left.\frac{Y_{n+1}}{n+2}\right|\mathcal{F}_n} = \frac{1}{n+2} \E \brb{\left.\left(Y_n+1\right)\frac{Y_n}{n+1}+Y_n\left(1-\frac{Y_n}{n+1}\right)\right|\mathcal{F}_n} \\
%& = & \frac{1}{n+2} \E \brb{\left.\frac{n+2}{n+1}Y_n\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=} \frac{1}{n+1} Y_n = X_n . \eeast
%
...
%Actually, since $\brb{X_n}_{n\geq 0}$ is bounded (and thus $\sL^p$-bounded), $X_n\to X_\infty$ a.s. and in $\sL^p$ for any $p\geq 1$.
...
%first. \beast \E \brb{\left.X_{n+1}^2\right|\sF_n} & = & \E \brb{\left.\frac{Y_{n+1}^2}{(n+2)^2}\right|\sF_n} = \frac{1}{(n+2)^2} \E \brb{\left.\left(Y_n+1\right)^2\frac{Y_n}{n+1}+Y_n^2\left(1-\frac{Y_n}{n+1}\right)\right|\mathcal{F}_n}\\
%& = & \frac{1}{(n+2)^2} \E \brb{\left.\frac{(n+3)Y_n^2+Y_n}{n+1}\right|\mathcal{F}_n } \stackrel{\text{a.s.}}{=}  \frac{(n+1)(n+3)}{(n+2)^2} X_n^2 + \frac{1}{(n+2)^2}X_n .\eeast
%
%Rebalancing the equation \be \E \brb{\left.\frac{n+2}{n+3}X_{n+1}^2\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{n+1}{n+2} X_n^2 + \frac{1}{(n+2)(n+3)}X_n. \ee
%
%With respect to the previous result, we also have \be \E \brb{\left.\frac{1}{n+3}X_{n+1}\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{1}{n+3}X_n \ee
%
%The sum of these two equations gives us
%\be
%\E \brb{\left.\frac{n+2}{n+3}X_{n+1}^2+\frac{1}{n+3}X_{n+1}\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{n+1}{n+2} X_n^2 + \frac{1}{n+2}X_n
%\ee
...
%\be
%\E X_\infty^2 = \lim_{n\to\infty}\E \brb{\frac{n+2}{n+3}X_{n+1}^2+\frac{1}{n+3}X_{n+1}} = \frac{1+1}{1+2} \E X_0^2 + \frac{1}{1+2} \E X_0 =\frac{2}{3}\frac{1}{4} + \frac{1}{3}\frac{1}{2} = \frac{1}{3}.
%\ee
%
...
\beast
\E\brb{M_{n+1}|\sF_n} = \E\brb{\left.\frac{B_{n+1}+1}{n+3}\right|\sF_n} & \stackrel{ \text{ a.s.}}{=} & M_n \frac{B_n+2}{n+3} +(1-M_n)\frac{B_n+1}{n+3}\\
...
\beast
\E \brb{\left.M_{n+1}^2\right|\sF_n} & = & \E \brb{\left.\frac{\brb{B_{n+1}+1}^2}{(n+3)^2}\right|\sF_n} \stackrel{ \text{ a.s.}}{=} \frac{1}{(n+3)^2} \brb{M_n \brb{B_n+2}^2+ (1-M_n)(B_n+1)^2}\\
& = & \frac{1}{(n+2)(n+3)^2} \brb{(B_n+1)(B_n+2)^2 + (n+1-B_n)(B_n + 1)^2} \\
& = & \frac 1{(n+3)^2}M_n\brb{(n+4)(B_n+1) + 1} = \frac 1{(n+3)^2}M_n\brb{(n+2)(n+4)M_n + 1} .\eeast

Rebalancing the equation \be \E \brb{\left.\frac{n+3}{n+4}M_{n+1}^2\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{n+2}{n+3} M_n^2 + \frac{1}{(n+3)(n+4)}M_n. \ee

With respect to the previous result, we also have
\be
\E \brb{\left.\frac{1}{n+4}M_{n+1}\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{1}{n+4}M_n.
\ee

The sum of these two equations gives us
\be
\E \brb{\left.\frac{n+3}{n+4}M_{n+1}^2+\frac{1}{n+4}M_{n+1}\right|\mathcal{F}_n} \stackrel{\text{a.s.}}{=}  \frac{n+2}{n+3} M_n^2 + \frac{1}{n+3}M_n
\ee
...
\be
\E X_\infty^2 = \lim_{n\to\infty}\E \brb{\frac{n+2}{n+3}M_{n}^2+\frac{1}{n+3}M_n} = \frac 2{3} \E M_0^2 + \frac{1}{3} \E M_0 =\frac{2}{3}\frac{1}{4} + \frac{1}{3}\frac{1}{2} = \frac{1}{3}.
\ee
...
\E[M^{(k)}_n+1|\sF_n] & = & \E\brb{\left.\prod^k_{i=1} \frac{B_n+1+i}{n+i+2}\right|\sF_n} \stackrel{\text{a.s.}}{=}  M_n \prod^k_{i=1} \frac{B_n+i+1}{n+i+2} +(1-M_n) \prod^k_{ i=1}\frac{B_n+i}{n+i+2} \\
...
manner. Each factor can be rewritten as follows: \be \frac{B_n+i}{n+i+1} = \frac{B_n+i}{n+2} \frac{n+2}{n+i+1} = \frac{B_n+1+i-1}{n+2} \frac{n+2}{n+i+1} = \brb{M_n+ \frac{i-1}{n+2}} \frac{n+2}{n+i+2}
\ee
...
\be
M_{X_\infty}(t) := \E\brb{e^{tX_\infty}} = \E\brb{\sum^\infty_{n=0} \frac{(tX_\infty)^n}{n!}} = \sum^\infty_{n=0} \frac {t^n\E X^n_\infty}{n!} = \sum^\infty_{ n=0} \frac{t^n}{(n+1)!} = \frac{e^t -1}t
\ee
...
1$,
\be
\pro\brb{\ol{\Delta}_n = \bba_n} = \frac{K_n!(n-K_n)!}{(n+1)!}
\ee
...
\beast
\pro\brb{\ol{\Delta}_{N+1} = \bba_{N+1}} & = & \pro\brb{\Delta_{N+1} = a_{N+1}|\ol{\Delta}_N = \bba_N} \frac{K_N!(N -K_N)!}{(N +1)!}\\
...
\be
\pro\brb{\ol{\Delta}_{N+1} = \bba_{N+1}} = \frac{B_{N+1}K_N!(N -K_N)!}{(N +2)!} = \frac{K_{N+1}!(N +1-K_{N+1})!}{(N +2)!}
\ee
as required, so induction completes the proof of the claim. From this we see that, if $K \in \bra{0,1, \dots ,n}$,
\be
\pro\brb{B_n = K} = \pro\brb{\sum^n_{i=1}\Delta_i = K} = \sum_{K_n=K} \pro\brb{\ol{\Delta}_n = a_n} = \binom{n}{K} \frac{K_n!(n-K_n)!}{(n+1)!} = \frac 1{n+1}
\ee
...
0$. Then \beast
\E\brb{N_{n+1}(\theta)|\sF_n} & = & \E\brb{\left.\frac{(n+2)!}{B_{n+1}!(n+1-B_{n+1})!} \theta^{B_{n+1}}(1-\theta)^{n+1-B_{n+1}} \right|\sF_n}\\
...
\be
\pro\brb{\ol{\Delta}_n = \bba_n|\Theta} = \Theta^{K_n}(1-\Theta)^{n-K_n}
\ee
for all $n \geq 1$ and $\bba_n \in \bra{0,1}^n$. Hence
\be
\pro\brb{\ol{\Delta}_n = \bba_n} = \E\brb{\pro\brb{\ol{\Delta}_n = \bba_n|\Theta}} = \E\brb{\Theta^{K_n}(1-\Theta)^{n-K_n}} = \int^1_0 x^{K_n}(1-x)^{n-K_n} dx
\ee

By the definition of the beta function, $B$, we have that
\be
\pro\brb{\ol{\Delta}_n = \bba_n} = B(K_n+1,n+1-K_n) = \frac{\Gamma(K_n+1)\Gamma(n+1-K_n)}{\Gamma(n+2)} = \frac{K_n!(n-K_n)!}{(n+1)!}
\ee
...
\beast
\pro\brb{\ol{\Delta}_n = \bba_n,\Theta\in A} & = & \E\brb{\ind_{\bba_n}(\ol{\Delta}_n)\ind_A(\Theta)} = \E\brb{\E\brb{\ind_{\bba_n}(\ol{\Delta}_n)\ind_A(\Theta)|\Theta}}\\
& = & \E\brb{\ind_A(\Theta)\E\brb{\ind_{\bba_n}(\ol{\Delta}_n)|\Theta}} = \E\brb{\ind_A(\Theta)\Theta^{K_n}(1-\Theta)^{n-K_n }}\\
