...
We define
\be
\sF_\infty = \bigvee_{n\geq 0} \sF_n = \sigma\brb{\bigcup_{n\geq 0} \sF_n}. %= \sigma (\sF_n:n\geq 0)
\ee
\end{definition}
...
Let $(X_n)_{n\geq 0}$ be a stochastic process, and let $\sF^X_n = \sigma\brb{X_k, k \leq n}$. Then $\sF^X_n$ is the natural filtration\index{natural filtration!discrete} of $X$. It is the smallest filtration with respect to which $X$ is adapted.
\end{definition}
...
\be
\bra{\inf_n T_n < k} = \bra{\inf_n T_n \geq k}^c = \brb{\bigcap_n \bra{T_n \geq k}}^c = \bigcup_n \brb{ \brb{\bra{T_n < k}}^c}^c =\bigcup_n \underbrace{\bra{T_n < k}}_{\in \sF_k} \in \sF_k.
\ee

\item [(vi)] By (iv) and (v),
\be
\bra{\limsup_n T_n \leq k}= \bra{\inf_{n}\brb{\sup_{m\geq n}T_m} \leq k} \in \sF_k.
\ee

\item [(viii)]
\be
\bra{\liminf_n T_n \leq k}= \bra{\sup_{n}\brb{\inf_{m\geq n}T_m} \leq k} \in \sF_k.
...
\be
A^c \cap \bra{T\leq n} = \bra{T\leq n} \bs \brb{A \cap \bra{T\leq n}} \in \sF_n.
\ee
\item [(iii)] For a sequence $A_m \in \sF_T$, we have $A_m \cap \bra{T\leq n}\in \sF_n$ for all $n$. Thus, since $\sF_n$ is $\sigma$-algebra
\be
\brb{\bigcup_m A_m} \cap \bra{T\leq n} = \bigcup_m \brb{A_m \cap \bra{T\leq n}} \in \sF_n  \ \ra \ \bigcup_m A_m \in \sF_T.
\ee
\een
...
\be
A\cap \bra{T<n} = \brb{A\cap \bra{T\leq n}}\cap \bra{T<n} \in \sF_n.
\ee

\item [(ii)] Similarly, $\bra{T=n} \subseteq \bra{T\leq n} \in \sF_n$. Hence
\be
A\cap \bra{T=n} = \brb{A\cap \bra{T\leq n}} \cap \bra{T=n} = \brb{A\cap \bra{T\leq n}} \cap \brb{\underbrace{\bra{T\leq n}}_{\in \sF_n} \bs\underbrace{\bra{T<n}}_{\in \sF_n}} \in \sF_n.
\ee
\een
\end{proof}
...
\be
A \cap \bra{T \leq n} = A \cap \brb{\bra{S \leq n} \cap \bra{T \leq n}} = \underbrace{\brb{A \cap \bra{S \leq n}}}_{\in \sF_n} \cap \underbrace{\bra{T \leq n}}_{\in \sF_n} \in \sF_n.
\ee
...
\be
\bra{S<T} = \bigcup_{k\in \N} \brb{\underbrace{\bra{S\leq k}}_{\in \sF}\cap \underbrace{\bra{T>k}}_{\in \sF}}.
\ee
...
\ee
\be
\bra{S<T} \cap \bra{S\leq n} = \bra{S<T\leq n}  \cup \bra{S\leq n<T} = \brb{\bigcup_{k\in \N,k< n} \bra{S\leq k}\cap \bra{k<T\leq n}} \cup \brb{\bra{S\leq n}\cap \bra{T\leq n}^c} \in \sF_n \nonumber
\ee
...
%\be
%\E \abs{X_{T\land n}} =  \E\brb{\sum^{n-1}_{m=0} \abs{X_m} \ind_{\bra{T = m}} } + \E\brb{\sum^\infty_{m=n} \abs{X_n} \ind_{\bra{T = m}}} \leq \sum^n_{m=0} \E\abs{X_n} < \infty.
%\ee
...
Now let
\be
G(z) = \sum^\infty_{k=0} p_kz^k = \E \brb{z^{X_1}}\,\qquad G_n(z) = \E \brb{z^{X_n}},\qquad \text{for }n \geq 1,
\ee
so that $G_1 = G$.
\end{definition}
...
\beast G_{n+1}(z) & = & \E\brb{z^{X_{n+1}}} = \sum^\infty_{k=0} \E \brb{z^{X_{n+1}} |X_n = k} \pro (X_n = k) = \sum^\infty_{k=0} \E\brb{z^{Y^n_1 + \dots+Y^n_k}|X_n = k}\pro (X_n = k) \\
& = & \sum^\infty_{k=0} \E\brb{z^{Y^n_1 + \dots+Y^n_k}}\pro (X_n = k) = \sum^\infty_{k=0} (G(z))^k p_k  =  \E\brb{(G(z))^{X_n}} = G_n (G(z)). \eeast
...
\be \E (X_n (X_n - 1)) = m^2 \E (X_{n-1} (X_{n-1} - 1)) + \brb{\sigma^2 + m^2 - m} \E (X_{n-1}). \ee

We then have, using the fact that $\E X_n = m^n$, \beast
\var (X_n) & = & \E (X_n(X_n - 1)) + \E (X_n) - (\E X_n)^2\\
& = & m^2\E (X_{n-1}(X_{n-1} - 1)) + \brb{\sigma^2 + m^2 - m}\E (X_{n-1}) + m^n - m^{2n} \\
& = & m^2\brb{\var (X_{n-1}) - \E (X_{n-1}) + (\E X_{n-1})^2} + \brb{\sigma^2 + m^2} m^{n-1} - m^{2n}\\
& = & m^2\var (X_{n-1}) + \sigma^2 m^{n-1}. \eeast Iterating this, we see that \beast
\var (X_n) & = & m^2\var (X_{n-1}) + \sigma^2 m^{n-1} = m^4\var (X_{n-2}) + \sigma^2 \brb{m^{n-1} + m^n} = \dots \\
& = & m^{2n}\var (X_0) + \sigma^2 \brb{m^{n-1} + \dots + m^{2n-2}}\\
& = & \sigma^2 \brb{m^{n-1} + \dots + m^{2n-2}},\quad\quad \text{since $\var (X_0) = 0$ because $X_0 = 1$}, \eeast and then the result may be obtained immediately.
\end{proof}
...
\ref{lem:measure_increasing_sequence}) we see that the extinction probability is \be q = \pro(A) = \lim_{n\to\infty} \pro (A_n) = \lim_{n\to\infty} G_n(0) = \lim_{n\to\infty} \pro \brb{X_n = 0}. \ee
...
\be G\brb{\lim_{n\to\infty} G_n(0)} = \lim_{n\to\infty}G_{n+1}(0) \ee,
...
\be S_n = S_0 + \sum^n_{k=1}X_k, \quad n \geq 1 \ee where $S_0$ is a constant then $\brb{S_n}_{n \geq 0}$ is known as a (one-dimensional) random walk.
...
\be
\pro\brb{(S_k)^n_{k=0}\in \Pi} = \frac{\text{\# of paths of length $n$ that start from (0,0) and are in $\Pi$}}{2^n}.
\ee
...
\be
\pro\brb{\max_{0\leq k\leq n}S_k \geq a} = 2\pro\brb{S_n \geq a} = \pro\brb{\abs{S_n}\geq a}.
\ee
\end{corollary}
...
\be
\pro\brb{T_0 >n|S_n=x} = \frac xn,\qquad \forall x=1,\dots,n.
\ee
\end{remark}
...
& = & \frac {N_{n,x}}n \brb{\frac{n+x}2 - \frac{n-x}2} = \frac xn N_{n,x}.
\eeast
\end{proof}
...
\be
\pro\brb{S_{2n} = 0} = \frac{\binom{2n}{n}}{2^{2n}}.
\ee
...
\be
\pro\brb{T_0 = 2n} = \frac{2\binom{2n-2}{n-1}}{n} \frac 1{2^{2n}} = \frac{\binom{2n-2}{n-1}}{2^{2n-1}n}.
 \ee


\item [(iii)] By Proposition \ref{pro:random_walk_number_paths_greater_equal_than_zero},
\beast
\pro\brb{S_k\geq 0,\forall k\leq 2n-3,S_{2n-2}=0,S_{2n-1}=-1} & = & \frac 12 \pro\brb{S_k\geq 0,\forall k\leq 2n-3,S_{2n-2}=0} \\
...
\be
\pro\brb{T_0 > 2n} = 1 - \sum^{n}_{k=1} \pro\brb{T_0 = 2k} = 1 - \sum^n_{k=1} \frac{\binom{2k-2}{k-1}}{2^{2k-1}k}
\ee
...
Therefore, we have
\be
\pro\brb{T_0 > 2n} = 1 - \sum^n_{k=1} \brb{\frac{\binom{2k-2}{k-1}}{2^{2k-2}} - \frac{\binom{2k}{k}}{2^{2k}}}  = \frac{1}{2^{2n}}\binom{2n}{n}.
\ee


\item [(v)] $S_{2n}$ can take value $0,2,\dots,2n$. Then by ballot problem (Theorem \ref{thm:ballot_problem})
\beast
\pro\brb{S_k \geq 0, \forall k \leq 2n} & = & 2\sum^n_{k=0} \pro\brb{S_{-1} = -1, S_0 = 0,S_m \geq 0, m = 1,\dots,2n-1,S_{2n}=2k}\\
 & = & 2\sum^n_{k=0} \pro\brb{S_{0} = 0, S_m > 0, m = 1,\dots,2n,S_{2n+1}=2k+1}\\
 & = & 2\sum^{n}_{k=0} \frac {2k+1}{2n+1}N_{2n+1,2k+1} \frac 1{2^{2n+1}}= \frac 1{2^{2n}}\sum^{n}_{k=0} \frac {2k+1}{2n+1}\binom{2n+1}{n+k+1} \\ %1 - \pro\brb{\exists k\leq 2n, S_k = -1}
 & = & \frac 1{2^{2n}}\sum^{n}_{k=0} \brb{\binom{2n}{n+k} - \binom{2n}{n+k+1}} = \frac 1{2^{2n}}\binom{2n}{n}.
\eeast
...
Thus,
\beast
\pro\brb{S_k \geq 0, \forall k \leq 2n} & = & \frac 1{2^{2n}} \#\bra{S_k \geq 0, \forall k \leq 2n} = \frac{2}{2^{2n+1}}\#\bra{S_{0}=0, S_k \geq 1, k =1,\dots, 2n+1}\\
& = & \frac{1}{2^{2n+1}}\brb{\#\bra{S_{0}=0, S_k \geq 1, k =1,\dots, 2n+1} + \#\bra{S_{0}=0, S_k \leq -1, k =1,\dots, 2n+1}} \\
& = & \frac{1}{2^{2n+1}}\#\bra{S_{0}=0, S_k \neq 0, k =1,\dots, 2n+1} = \pro\brb{T_0 > 2n} = \frac 1{2^{2n}}\binom{2n}{n}
\eeast
by (iv).
\een
\end{proof}
...
\beast
& &\pro\brb{\max_{0\leq i\leq n}S_i - S_n = n-2k} = \left.\binom{n}{k}\right/ 2^n,\\
& &\pro\brb{\max_{0\leq i\leq n}S_i - S_n = n-2k-1} = \left.\binom{n}{k}\right/ 2^n = \left.\binom{n}{n-k}\right/ 2^n.
\eeast

If $n$ is even, $k = 0,1\dots \frac n2-1$,
\beast
& & \pro\brb{\max_{0\leq i\leq n}S_i - S_n = 0} = \left.\binom{n}{n/2}\right/ 2^n,\\
& &\pro\brb{\max_{0\leq i\leq n}S_i - S_n = n-2k} = \left.\binom{n}{k}\right/ 2^n,\\
& &\pro\brb{\max_{0\leq i\leq n}S_i - S_n = n-2k-1} = \left.\binom{n}{k}\right/ 2^n = \left.\binom{n}{n-k}\right/ 2^n.
\eeast
...
\be
\pro\brb{S_n = \max_{0\leq i\leq n}S_i} = \left\{\ba{ll}\left.\binom{n}{n/2}\right/2^n\quad \quad & n\text{ is even} \\
...
\be
\pro\brb{\max_{0\leq i\leq 1} S_i - S_1 =0} = \pro\brb{\max_{0\leq i\leq 1} S_i - S_1 = 1} = \frac 12 = \left.\binom{1}{1}\right/2^1= \left.\binom{1}{0}\right/2^1.
\ee

For $n=2$, we have
\beast
& & \pro\brb{\max_{0\leq i\leq 2} S_i - S_2 =0} = \frac 12 = \left.\binom{2}{1}\right/2^2,\\
& & \pro\brb{\max_{0\leq i\leq 2} S_i - S_2 =1} = \frac 14 = \left.\binom{2}{0}\right/2^2,\\
& & \pro\brb{\max_{0\leq i\leq 2} S_i - S_2 =2} = \frac 14 = \left.\binom{2}{0}\right/2^2.
\eeast
...
\be
h_{n-m,n+1} = \frac 12 \frac{\binom{n}{k}}{2^n} + \frac 12 \frac{\binom{n}{k-1}}{2^n} = 2^{-(n+1)}\brb{\binom{n}{k} +\binom{n}{k-1}} = 2^{-(n+1)}\binom{n+1}{k}
\ee
...
\be
\pro\brb{S_n = \max_{0\leq i\leq n}S_i} = 2^{-n}\binom{n}{k} = 2^{-(n+1)} 2\binom{2k+1}{k} = 2^{-(n+1)} \binom{2k+2}{k+1} = 2^{-(n+1)} \binom{n+1}{(n+1)/2}.
\ee
\end{proof}
...
\be
\pro\brb{\omega:X_t(\omega)\text{ is continuous for all }t} = 1.
\ee
\end{definition}
...
We define \be
\sF_\infty = \bigvee_{t\geq 0} \sF_t = \sigma\brb{\bigcup_{t\geq 0} \sF_t}. %= \sigma (\sF_t:t\geq 0)
\ee
\end{definition}
...
\begin{definition}[natural filtration, discrete]
Let $(X_t)_{t\geq 0}$ be a stochastic process, and let $\sF^X_t = \sigma\brb{X_s: s \leq t}$. Then $\sF^X_t$ is the natural filtration\index{natural filtration!continuous} of $X$. It is the smallest filtration with respect to which $X$ is adapted.
\end{definition}
...
\een%If $\sF_t$ is complete (see Definition \ref{def:complete_measure_space}) for all $t\geq 0$, we say that the filtration $\brb{\sF_t}_{t\geq 0}$ is complete.
\end{definition}
...
\be
\bra{T=t} = \bra{T\leq t}\bs \bra{T<t} = \bra{T\leq t} \left\bs \brb{\bigcup_n \bra{T\leq t-\frac 1n}} \right.\in \sF_t.
\ee
\end{proof}
...
\be
\bra{\inf_n T_n < t} = \bra{\inf_n T_n \geq t}^c = \brb{\bigcap_n \bra{T_n \geq t}}^c = \bigcup_n\brb{ \brb{\bra{T_n < t}}^c}^c =\bigcup_n \underbrace{\bra{T_n < t}}_{\in \sF_t} \in \sF_t.
\ee

\item [(vii)]
\be
\bra{\limsup_n T_n \leq t}= \bra{\inf_{n}\brb{\sup_{m\geq n}T_m} \leq t} \in \sF_t.
\ee

\item [(viii)]
\be
\bra{\liminf_n T_n \leq t}= \bra{\sup_{n}\brb{\inf_{m\geq n}T_m} \leq t} \in \sF_t.
\ee%where $\sup_{m\geq n}T_m$ and $\inf_{m\geq n}T_m$ are stopping time sequences.
\een
\end{proof}
...
\be
A^c \cap \bra{T\leq t} = \bra{T\leq t} \bs \brb{A \cap \bra{T\leq t}} \in \sF_t.
\ee
\item [(iii)] For a sequence $A_m \in \sF_T$, we have $A_m \cap \bra{T\leq t}\in \sF_t$ for all $n$. Thus, since $\sF_t$ is $\sigma$-algebra
\be
\brb{\bigcup_m A_m} \cap \bra{T\leq t} = \bigcup_m \brb{A_m \cap \bra{T\leq t}} \in \sF_t  \ \ra \ \bigcup_m A_m \in \sF_T.
\ee
\een
...
\be
A\cap \bra{T<t} = \brb{A\cap \bra{T\leq t}}\cap \bra{T<t} \in \sF_t.
\ee

\item [(ii)] Similarly, $\bra{T=t} \subseteq \bra{T\leq t} \in \sF_t$. Hence
\be
A\cap \bra{T=t} = \brb{A\cap \bra{T\leq t}} \cap \bra{T=t} = \brb{A\cap \bra{T\leq t}} \cap \brb{\underbrace{\bra{T\leq t}}_{\in \sF_t} \bs\underbrace{\bra{T<t}}_{\in \sF_t}} \in \sF_t.
\ee
\een
\end{proof}
...
\be
A \cap \bra{T \leq t} = A \cap \brb{\bra{S \leq t} \cap \bra{T \leq t}} = \underbrace{\brb{A \cap \bra{S \leq t}}}_{\in \sF_t} \cap \underbrace{\bra{T \leq t}}_{\in \sF_t} \in \sF_t \ \ra \ A \in \sF_T.
\ee
\end{proof}
...
\item [(i)] First, $\bra{S<T}\in \sF$ since it can be expressed by
\be
\bra{S<T} = \bigcup_{r\in \Q} \brb{\underbrace{\bra{S\leq r}}_{\in \sF}\cap \underbrace{\bra{T>r}}_{\in \sF}}.
\ee
...
\ee
\be
\bra{S<T} \cap \bra{S\leq t} = \bra{S<T\leq t}  \cup \bra{S\leq t<T} = \brb{\bigcup_{r\in \Q,r< t} \bra{S\leq r}\cap \bra{r<T\leq t}} \cup \brb{\bra{S\leq t}\cap \bra{T\leq t}^c} \in \sF_t \nonumber
\ee
which implies $\bra{S<T}  \in \sF_S$.
...
& \lra & \brb{\bigcup_{i=1}^n \bra{A_i:c_i \leq c} }\cap \bra{T\leq t}  \in \sF_t.\\ %\bigcup_{i=1}^n \bra{A_i\cap \bra{T\leq t} :c_i = c}\\
& \lra & \bra{X\ind_{\bra{T\leq t}}\leq c} = \bra{\sum^n_{i=1} c_i\ind_{A_i \cap \bra{T\leq t}} \leq c}= \bra{\brb{\bigcup_{i=1}^n \bra{A_i:c_i \leq c} }\cap \bra{T\leq t} } \in \sF_t \\
...
\be
X_t\ind_{\bra{T = t}} = \underbrace{X_t}_{\sF_t\text{-measurable}} \brb{\ind\bra{\underbrace{T\leq t}_{\in \sF_t}} - \ind\bra{\bigcup_n \underbrace{\bra{T\leq t-\frac 1n}}_{\in \sF_{t-\frac 1n}\subseteq \sF_t}}} \ \ra \ X_t \ind_{\bra{T=t}} \text{ is $\sF_t$-measurable}
\ee
...
\be
\mu_X((-\infty, x]) = \pro(X \leq x) = \pro\brb{\{\omega:X(\omega) \leq x\}} \ \ra \ \mu_X(A) = \pro\brb{\bra{\omega:X(\omega) \in A}},\quad A\in \sB(\sS).
\ee
...
\be
\mu_J = \pro\brb{\brb{X_{t_1},\dots,X_{t_n} }\in A},\quad A \in \sB(\sS^n)
\ee
...
\be
\pro\brb{\brb{X_{t_1},\dots,X_{t_n}}\in A} = \wt{\pro}\brb{\brb{Y_{t_1},\dots,Y_{t_n}}\in A}.
\ee
\end{definition}
...
\be
\pro\brb{X'_t = 0,\ \forall t \in [0, 1]} = 0.
\ee
\end{example}
...
X_t(\omega)} = 0$. Then by Proposition \ref{pro:probability_property}, \be \pro(X_t(\omega) = X_t(\omega),\forall t\in \Q) = 1 - \pro\brb{X_t(\omega) \neq X_t(\omega),\exists t\in \Q} \geq 1 - \underbrace{\sum_{t\in \Q}
...
\beast
\pro\brb{\abs{X_{k2^{-n}} - X_{(k+1)2^{-n}}} \geq 2^{-n\alpha}} & = & \pro\brb{\abs{X_{k2^{-n}} - X_{(k+1)2^{-n}}}^p \geq 2^{-np\alpha }} \leq  \frac{\E\abs{X_{k2^{-n}} - X_{(k+1)2^{-n}}}^p}{2^{-np\alpha}}\\
& = & \E\bsb{\abs{X_{k2^{-n}} - X_{(k+1)2^{-n}}}^p}2^{np\alpha} \leq c 2^{-n(1+\ve)} 2^{np\alpha}  \leq c2^{-n}2^{-(\ve-p\alpha)n}.
\eeast

Summing over $\sD_n$ we obtain
\be
\pro\brb{\max_{0\leq k<2^n} \abs{X_{k2^{-n}} - X_{(k+1)2^{-n}} } \geq 2^{-n\alpha}} \leq c 2^{-n(\ve-p\alpha)},
\ee
...
\be
\E\abs{X_t - \wt{X}_t}^p = \E\abs{X_t - \liminf_n X_{t_n}}^p = \E\brb{\liminf_n \abs{X_t - X_{t_n}}^p} \leq \liminf_n \E[\abs{X_t - X_{t_n}}^p],
\ee
...
\beast
\pro\brb{X_{t_1}\in A_1,\dots X_{t_n}\in A_n} & = & \pro\brb{\omega:X_{t_1}(\omega)\in A_1,\dots, X_{t_n}(\omega)\in A_n} \\
& = & \pro\brb{\omega:\phi(\omega)\brb{t_1}\in A_1,\dots, \phi(\omega)\brb{t_n}\in A_n} \\
& = & \pro\brb{\omega:Y_{t_1}\circ \phi (\omega)\in A_1,\dots, Y_{t_n}\circ \phi(\omega)\in A_n} \\
& = & \pro\circ \phi^{-1} \brb{Y_{t_1}\in A_1,\dots, Y_{t_n}\in A_n}.\quad\quad (*)
\eeast
...
\be
\pro\brb{\omega:Y_t(\omega)\ind_{\bra{\omega\in \Omega'^c}}\leq c} = 0\text{ or }1 \ \ra \bra{\omega:Y_t(\omega)\ind_{\bra{\omega\in \Omega'^c}}\leq c}\text{ is $\sF_t$-measurable}).
\ee
...
\beast
\sigma \brb{\text{all bounded adapted (sample) left-continuous processes}} \subseteq \sigma \brb{\text{all adapted (sample) left-continuous processes}} \subseteq \sP.
\eeast
...
\be
\lim_{t_m \ua t}\ind_C(t_m) = \lim_{t_m \ua t}\lim_{n\to \infty} \ind_{C_n}(t_m) = \lim_{t_m \ua t} \sum^\infty_{n=1} \brb{\ind_{C_n}- \ind_{C_{n-1}}}(t_m)\quad \text{a.s..}
\ee
...
\be
\lim_{t_m \ua t} \sum^\infty_{n=1} \brb{\ind_{C_n}- \ind_{C_{n-1}}}(t_m) = \sum^\infty_{n=1} \lim_{t_m \ua t} \brb{\ind_{C_n}- \ind_{C_{n-1}}}(t_m) = \sum^\infty_{n=1} \brb{\ind_{C_n}- \ind_{C_{n-1}}}(t) = \ind_C(t)\quad\text{a.s.}.
\ee
...
\beast
\abs{(H \cdot A)_t(\omega)} & \leq & \abs{\int_{(0,t]} H_s^+(\omega) dA^+_s(\omega) - \int_{(0,t]} H_s^-(\omega) dA^+_s(\omega) - \brb{\int_{(0,t]} H_s^+(\omega) dA^-_s(\omega) -  \int_{(0,t]} H_s^-(\omega) dA^-_s(\omega)}}\quad (*)\\
...
\be
\brb{\ind_{D\bs C}\ind_{\bra{\omega \in \Omega'}} \cdot A}_t = \brb{(\ind_D -\ind_C) \cdot A}_t\ind_{\bra{\omega \in \Omega'}} = \underbrace{\brb{\brb{\ind_D \cdot A}_t - \brb{\ind_C\cdot A}_t}}_{\text{by Theorem \ref{thm:non_negative_measurable_property}.(i)}}\ind_{\bra{\omega \in \Omega'}} \quad \text{is $\sF_t$-measurable.}
\ee
...
& = & \sum^\infty_{k=1} \underbrace{\brb{\ind_{\bra{H \in \left[2^{-n}k, 2^{-n}(k + 1)\right)}}\ind_{\bra{\omega \in \Omega'}} \cdot A}_t}_{\text{$\sF_t$-measurable}}\quad \text{is $\sF_t$-measurable as a sum of $\sF_t$-measurable functions.}
\eeast
% is $\sF_t$-measurable.
...
\be
\brb{H^n \cdot A}_t = \sum^\infty_{k =0} H_{k2^{-n}}\brb{A_{(k+1)2^{-n}\land t} - A_{k2^{-n}\land t}}
\ee
...
\be
\abs{\brb{H^n \cdot A}_t- \brb{H \cdot A}_t }  = \abs{\brb{\brb{H^n - H} \cdot A}_t} \leq \brb{\abs{H^n-H}\cdot (A^+ + A^-)}_t \leq \brb{\abs{H^n-H}\cdot (A^+ + A^-)}_T
\ee
...
\be
\abs{\brb{H^n \cdot A}_t- \brb{H \cdot A}_t } \to 0\ \text{ a.s.}.
\ee
as required.
\end{proof}
...
\be
\int_{(0,t]} H_s d\brb{(K\cdot A)_s} = \int_{(0,t]} H_s K_s d A_s \ \ra \ d\brb{\int_{(0,t]}K_s dA_s} = K_t dA_t.
\ee
%\ben
%\item [(i)]
%\item [(ii)] %Since both processes are non-decreasing, we have $\brb{H\cdot (K\cdot A)}_\infty = ((HK)\cdot A)_\infty$. Then monotone convergence theorem (Theorem \ref{thm:monotone_convergence_probability}) implies that
%\be
%\E\brb{\brb{H\cdot (K\cdot A)}_\infty} = \E\brb{((HK)\cdot A)_\infty}.
%\ee
%\een
\end{remark}
...
\be
\brb{H\cdot (K\cdot A)}_t = \brb{(HK)\cdot A}_t = 0.
\ee
...
\ea\right.  =  \ind_{B_H}\brb{A_{t\land v_H} - A_{t\land u_H}}
\ee

Similarly, we have $(K \cdot A)_t = \ind_{B_K}\brb{A_{t\land v_K} - A_{t\land u_K}} $, Thus,
\beast
(H \cdot (K\cdot A))_t & = & \ind_{B_H} \brb{(K\cdot A)_{t\land v_K} - (K\cdot A)_{t\land u_K}}\\
& = & \ind_{B_H}\ind_{B_K} \brb{A_{t\land v_H\land v_K} + A_{t\land u_H\land u_K} - A_{t\land u_H \land v_K} - A_{t\land u_K\land v_H}}
%& = & \ind_{B_H}\ind_{B_K} \brb{A_{t\land v_H\land v_K} + A_{t\land v_H\land v_K} - A_{t\land \brb{\brb{u_H\land u_K}\vee\brb{v_H\land v_K}}} - A_{t\land u_H \land v_K} - A_{t\land u_K\land v_H}} \\
%& = & \ind_{B_H}\ind_{B_K} \brb{A_{(u_H\vee u_K)\vee (v_H\land v_K) \land t} - A_{(u_H\vee u_K)\land t}}\\%& = & \ind_{B_H}\ind_{B_K}\ind_{\bra{u_H<t}}\ind_{\bra{u_K<t}} \brb{A_{\brb{(u_H\vee u_K)\vee (v_H\land v_K)}\land t} - A_{(u_H\vee u_K)\land t}}\\
%& = & \brb{\ind_{B_H}\ind_{B_K} \ind_{\left(u_H\vee u_K, (u_H\vee u_K)\vee (v_H\land v_K)\right]} \cdot A}_t = ((HK)\cdot A)_t
\eeast
...
%%%A_{t\land v_H\land v_K} + A_{u_K} -A_{u_H\land v_K} - A_{u_K}  \quad \quad & u_H\vee u_K \leq t \quad \quad \brb{ u_K\leq u_H \leq t } %\\%< \brb{u_H\vee u_K} \vee \brb{v_H\land v_K}\\
%%%\qquad\qquad = A_{t\land v_H\land v_K} -A_{u_H\land v_K}  & \qquad%< u_H \vee \brb{v_H\land v_K}}
%%%A_{t\land v_H\land v_K} + A_{u_K} -A_{u_H\land v_K} - A_{u_K} \quad \quad & t \geq \brb{ u_H\vee u_K} \vee \brb{v_H\land v_K}\\
%%% & \qquad\qquad \brb{ t > u_H\geq u_K, t > v_H\land v_K}\\
%\ea\right.
%\eeast
...
& = & A_{t\land v_H\land v_K} + A_{t\land \brb{u_H\vee u_K}} - A_{t\land u_H \land v_K} - A_{t\land u_K\land v_H}  + A'\\
& = & A_{\brb{t\land v_H\land v_K} \land \brb{t\land \brb{u_H\vee u_K}} } + A_{\brb{t\land v_H\land v_K} \vee \brb{t\land \brb{u_H\vee u_K}} } - A_{\brb{t\land u_H \land v_K} \land \brb{t\land u_K\land v_H}} - A_{\brb{t\land u_H \land v_K} \vee \brb{t\land u_K\land v_H}} + A'\\
& = & A_{\brb{\brb{v_H\land v_K} \land \brb{u_H\vee u_K} }\land t} + A_{\brb{\brb{v_H\land v_K} \vee \brb{u_H\vee u_K}}\land t} - A_{\brb{\brb{ u_H \land v_K} \land \brb{ u_K\land v_H}}\land t} - A_{\brb{\brb{ u_H \land v_K} \vee \brb{ u_K\land v_H}}\land t} + A'\\
& = & A_{\brb{\brb{u_H\land v_K}\vee \brb{v_H\land u_K}}\land t} + A_{\brb{\brb{v_H\land v_K} \vee \brb{u_H\vee u_K}}\land t} - A_{\brb{u_H \land u_K}\land t} - A_{\brb{\brb{ u_H \land v_K} \vee \brb{ u_K\land v_H}}\land t} + A'\\
& = & A_{\brb{\brb{v_H\land v_K} \vee \brb{u_H\vee u_K}}\land t} - A_{\brb{u_H \land u_K}\land t}  + A' =  A_{\brb{\brb{v_H\land v_K} \vee \brb{u_H\vee u_K}}\land t} - A_{\brb{u_H \vee u_K}\land t}.\quad\quad (*)
\eeast

Thus,
\beast
(H \cdot (K\cdot A))_t & = & \ind_{B_H}\ind_{B_K} \brb{A_{\brb{\brb{v_H\land v_K} \vee \brb{u_H\vee u_K}}\land t} - A_{\brb{u_H \vee u_K}\land t}} \\
& = & \brb{\ind_{B_H}\ind_{B_K} \ind_{\left(u_H \vee u_K, \brb{v_H\land v_K} \vee \brb{u_H\vee u_K}\right]} \cdot A}_t = \brb{(HK)\cdot A}_t.
\eeast
...
\be
\brb{H^m \cdot \brb{K^n\cdot A}}_t = \brb{\brb{H^m K^n}\cdot A}_t
\ee
...
\be
(K^n \cdot A)_t \ua (K\cdot A)_t, \quad \brb{H^m\cdot (K\cdot A)}_t \ua \brb{H\cdot (K\cdot A)}_t,\quad\brb{\brb{H^m K^n}\cdot A}_t \ua \brb{\brb{H K}\cdot A}_t
\ee
...
\be
H^m \cdot (K^n \cdot A)_t = (K^n \cdot A)_t(H^m) = (K \cdot A)_t\brb{\ind_{C_n}H^m}.
\ee
...
\be
\brb{H\cdot (K\cdot A)}_t =  \brb{(HK)\cdot A}_t.
\ee
...
\be
\brb{H\ind_{(0,T_n]}\cdot (K\ind_{(0,T_n]}\cdot A)}_t = \brb{H\ind_{(0,T_n]}\cdot (K\cdot A)^{T_n}}_t = \brb{H\cdot (K\cdot A)}_t^{T_n} \to \brb{H\cdot (K\cdot A)}_t\quad \text{a.s.}
\ee
...
Thus, we have
\be
\brb{H\cdot (K\cdot A)}_t = \brb{(HK)\cdot A}_t\quad \text{a.s.}.
\ee
...
\be
\forall \ve > 0, \forall t \geq 0,\quad \pro\brb{\sup_{s\leq t} \abs{X^n_s - X_s} \to 0} =1 \quad \text{ as }n \to\infty.
\ee
...
\be
\forall \ve > 0, \forall t \geq 0,\quad \pro\brb{\sup_{s\leq t} \abs{X^n_s - X_s} > \ve} \to 0 \quad \text{ as }n \to\infty.
\ee
...
\be
\pro\brb{\sup_{0\leq s\leq t}|X^n_s| \to 0 } = 1\quad \text{as }n\to \infty.
\ee

Then for each $\ve > 0$ and $t\geq 0$,
\be
\pro\brb{\sup_{0\leq s\leq t}|X^n_s| \leq \ve} \geq \underbrace{\pro \brb{\bigcap_{m\geq n} \bra{\sup_{0\leq s\leq t}|X^n_s| \leq \ve}} \ua \pro\brb{\sup_{0\leq s\leq t}|X^n_s| \leq \ve \text{ ev.}}}_{\text{Lemma }\ref{lem:measure_increasing_sequence}} \geq \pro\brb{\sup_{0\leq s\leq t}|X^n_s| \to 0},
\ee

\be
\pro\brb{\sup_{0\leq s\leq t}|X^n_s| > \ve} = 1 - \pro\brb{\sup_{0\leq s\leq t}|X^n_s| \leq \ve} \leq 1 - \pro\brb{\sup_{0\leq s\leq t}|X^n_s| \to 0} = 0.
\ee
...
\be
\forall \ve>0,\ \delta >0, \ \exists N,\ \forall n\geq N,\quad \pro\brb{\sup_{0\leq s\leq t}|X^n_s| > \ve} < \delta.
\ee
so we can find a subsequence ($n_k$) such that $\ve_k = 1/k$ and $\delta_k = 2^{-k}$,
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X^{n_k}_s} > 1/k} < 2^{-k} \ \ra \ \sum_{k=1}^\infty \pro\brb{\sup_{0\leq s\leq t}\abs{X^{n_k}_s} > 1/k} < \sum_k 2^{-k} = 1 < \infty.
\ee
So, by the first Borel-Cantelli lemma (Lemma \ref{lem:borel_cantelli_1_probability}),
\be
0 = \pro\brb{\sup_{0\leq s\leq t}\abs{X^{n_k}_s} > 1/k \text{ i.o.}} = \pro\brb{\bra{\sup_{0\leq s\leq t}\abs{X^{n_k}_s} \leq 1/k \text{ ev.}}^c} = \pro\brb{\bra{\sup_{0\leq s\leq t}\abs{X^{n_k}_s} \to 0}^c} \ \ra\ \sup_{0\leq s\leq t}\abs{X^{n_k}_s} \to 0\text{ a.s.}\nonumber
\ee
...
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s} > \ve} \to 0,\qquad \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -Y_s} > \ve} \to 0.
\ee

Then for any $\ve >0$,
\beast
& & \pro\brb{\sup_{0\leq s\leq t}\abs{X_s -Y_s} > \ve} \\
& \leq & \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s} + \sup_{0\leq s\leq t}\abs{X^n_s -Y_s} > \ve} \\
& \leq & \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s}>\ve/2, \sup_{0\leq s\leq t}\abs{X^n_s -Y_s} > \ve/2} + \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s}>\ve/2, \sup_{0\leq s\leq t}\abs{X^n_s -Y_s} \leq \ve/2} \\
& & \qquad + \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s}\leq \ve/2, \sup_{0\leq s\leq t}\abs{X^n_s -Y_s} > \ve/2} \\
& \leq & \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -X_s}>\ve/2} + \pro\brb{\sup_{0\leq s\leq t}\abs{X^n_s -Y_s} > \ve/2} \to 0 + 0 = 0.
\eeast

Therefore,
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X_s -Y_s} > \ve} = 0 \ \ra\ \pro\brb{\sup_{0\leq s\leq t}\abs{X_s -Y_s} = 0} = 1\ \ra\ \pro\brb{X_t = Y_t, \forall t} = 1,
\ee
as required.
\end{proof}
...
%\be
%d_t\brb{f,g} = \sup_{0\leq s\leq t}\abs{f_s - g_s}.
%\ee
...
%\be
%d\brb{X,Y} = \sum^\infty_{k=1} 2^{-k}\land d_k\brb{X,Y}.
%\ee
...
%\be
%d(X^n,X) \xrightarrow{p} 0 \ \ra \ \E\brb{d(X^n,X)} \to 0%\pro\brb{d(X^n,X)\geq \ve} \to 0
%\ee
%as $d(X^n,X) \leq 1$. Hence, for any $k\in \Z^+$, $\E\brb{d_k(X^n,X)} \to 0$. Then by Markov's inequality (Theorem \ref{thm:markov_inequality_probability}), for any $\ve >0$,
%\be
%\pro\brb{\sup_{0\leq s\leq k} \abs{X^n_s - X_s}\geq \ve} \leq \frac 1{\ve}\E\brb{d_k(X^n,X)} \to 0
%\ee
...
%\be
%d^{\text{ucp}}(X,Y) = \E\brb{d(X,Y)} = \sum^\infty_{k=1} \E\brb{2^{-k}\land d_k\brb{X,Y}}.
%\ee

%Note that we can exchange the expectation and summation by Fubini theorem (Theorem \ref{thm:fubini}) as $2^{-k}\land d_k\brb{X,Y}$ are non-negative.
...
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X^m_s - X^n_s} > \ve} \to 0\quad \text{as }m,n\to \infty. %d^{\text{ucp}}\brb{X^m,X^n} \to 0 \quad \text{as }m,n\to \infty.
\ee

This is equal to the statement
\be
\forall \ve>0,\ \delta>0, \ \exists N, \ \forall n\geq N,\ \forall m\geq 0,\quad \pro\brb{\sup_{0\leq s\leq t}\abs{X^{m+n}_s - X^n_s} > \ve} < \delta
\ee
...
\be
\pro\brb{\sup_{0\leq s\leq t}\abs{X^{n_{k+1}}_s - X^{n_k}_s} \to 0} = 1
\ee
which means $X^{n_k}$ is also a Cauchy sequence under uniform convergence on compacts a.s.. %
%$d^{\text{ucp}}\brb{X^{n_k},X^{m}} \leq 2^{-k}$ for all $m\geq n_k$. In this case, we have
%\be
%\sum^\infty_{k=1}\E\brb{d\brb{X^{n_k},X^{n_{k+1}}}} = \sum^\infty_{k=1}d^{\text{ucp}}\brb{X^{n_k},X^{n_{k+1}}} \leq 1. %= \E\brb{\sum^\infty_{k=1} d\brb{X^{n_k},X^{n_{k+1}}}} \leq 1.
%\ee
%Thus, this imples that $d\brb{X^{n_k},X^{n_{k+1}}} \to 0$ a.s.\footnote{need theorem here}. Thus, we will have, for all $n\in \Z^+$ and all $t\in \R^{++}$,
%\be
%\sup_{0\leq s\leq n}\abs{X^{n_k}_s - X^{n_{k+1}}_s} \stackrel{\text{a.s.}}{=} d_n\brb{X^{n_k},X^{n_{k+1}}} \to 0\text{ a.s.} \ \ra \ \sup_{0\leq s\leq t}\abs{X^{n_k}_s - X^{n_{k+1}}_s} \to 0\text{ a.s.}.
%\ee
...
\be
\lim_{k\to\infty} \pro\brb{\sup_{0\leq s\leq t} \abs{X^{n_k}_s - X_s}> \ve} = 0\quad (*)
\ee

%Then since $d^{\text{ucp}}$ is a pseudometric, we have
%\be
%\lim_{n\to\infty}d^{\text{ucp}}(X^n,X) = \limsup_{n\to\infty}d^{\text{ucp}}(X^n,X) \leq \limsup_{k,n\to\infty}\brb{d^{\text{ucp}}(X^n,X^{n_k})+ d^{\text{ucp}}(X^{n_k},X)} = 0,
%\ee
...
\beast
\pro\brb{\sup_{0\leq s\leq t}\abs{X^{n}_s - X_s} > \ve} & \leq & \pro\brb{\sup_{0\leq s\leq t}\abs{X^{n}_s - X^{n_k}_s} + \sup_{0\leq s\leq t}\abs{X^{n_k}_s - X_s} > \ve}\\
& \leq & \pro\brb{\sup_{0\leq s\leq t}\abs{X^{n}_s - X^{n_k}_s} > \ve/2} + \pro\brb{\sup_{0\leq s\leq t}\abs{X^{n_k}_s - X_s} > \ve/2} \to 0 + 0 = 0
\eeast
