...
Let $(\Omega,\sF, (\sF_t)_{t\geq 0},\pro)$ be a filtered probability space. A stochastic process $X = (X_t)_{t\geq 0}$ is adapted to the filtration $(\sF_t)_{t\geq 0}$\index{adapted process!continuous} (or $\brb{\sF_t}_{t\geq 0}$-adapted) if $X_t$ is $\sF_t$-measurable for all $t\in \R^{++}$. $(X_t)_{t\geq 0}$ is called adapted process (or non-anticipating process).
\end{definition}
...
\begin{definition}[complete filtered probability space\index{complete!filtered probability space}]\label{def:complete_filtered_probability_space}%ration $\brb{\sF_t}_{t\geq 0}$
...
\be
A\cap \bra{S\land T \leq t} = A\cap \brb{\bra{S\leq t}\cup \bra{T\leq t}} =  \bra{A\cap \brb{S\leq t}} \cup \bra{A\cap \brb{T\leq t}} \in \sF_t
\ee
...
\be
A\cap \bra{T<t} = A \cap \bra{\bigcup_n\bra{T_n<t}} = \bigcup_n\brb{A\cap \bra{T_n<t}} \in \sF_t
\ee
...
\be
\brb{\Omega,\sF_t} \to \brb{\Omega\times [0,t],\sF_t\otimes \sB([0,t])}:\ \omega \mapsto \brb{\omega,T(\omega)}
\ee
is $\sF_t$-meaurable as $\bra{\omega \in A} \cap \bra{T(\omega) \in \sT} \in \sF_t$.

Also, the map
\be
\brb{\Omega\times [0,t],\sF_t\otimes \sB([0,t])}\to (\R,\sB(\R)):\ (\omega,s) \mapsto X_s(\omega)
\ee
is $\sF_t\otimes \sB([0,t])$-measurable by the fact that $X$ is progressive. Then the map
\be
\brb{\Omega,\sF_t} \to  (\R,\sB(\R)):\ \omega \mapsto X_{S(\omega)}(\omega)
\ee
...
\beast
\sigma \brb{\text{all bounded adapted (sample) left-continuous processes}} \subseteq \sigma \brb{\text{all adapted (sample) left-continuous processes}} \subseteq \Prev.
\eeast
