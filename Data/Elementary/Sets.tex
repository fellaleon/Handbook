...
\be
h\circ (g\circ f) (x) = h\brb{(g\circ f) (x)} = h\brb{g(f(x))} = (h\circ g)(f(x)) = \brb{(h\circ g) \circ f}(x).
\ee
\end{proof}
...
\be
\brb{g\circ f}(x) = g\brb{f(x)} = g\brb{f(x')} = \brb{g\circ f}(x') \ \ra\ x = x'
\ee
...
\be
g\brb{f_1(x)} = \brb{g\circ f_1}(x) =  \brb{g\circ f_2}(x) = g\brb{f_2(x)} \ \ra\ f_1(x) = f_2(x)
\ee
since $g$ is injective. Then $f_1 = f_2$ on $X$.

\item [(ii)] For any $z\in Z$, we can find $y \in Y$ such that $g(y) =z$ since $g$ is surjective. So
\be
h_1(c) = h_1\brb{g(y)} = \brb{h_1\circ g}(y) = \brb{h_2\circ g}(y) = h_2\brb{g(y)} = h_2(c),
\ee
...
\be
f\brb{\bigcup_{i\in I} A_i} = \bigcup_{i\in I} f(A_i),\qquad
f\brb{\bigcap_{i\in I} A_i} \subseteq \bigcap_{i\in I} f(A_i). \ee
\end{proposition}
...
\be
f^{-1} \brb{\bigcup_{i\in I} A_i} = \bigcup_{i\in I} f^{-1}(A_i),\qquad
f^{-1} \brb{\bigcap_{i\in I} A_i} = \bigcap_{i\in I} f^{-1}(A_i).
\ee
\end{proposition}

\begin{proof}[\bf Proof]
\beast
x \in f^{-1}\brb{\bigcup_{i\in I} A_i} \ \lra\  f(x) \in \bigcup_{i\in I} A_i & \lra & f(x) \in A_i \text{ for some }i \\
& \lra & x\in f^{-1}\brb{A_i}\text{ for some }i \ \lra \ x \in \bigcup_{i\in I} f^{-1}\brb{A_i}.
\eeast
\beast
x \in f^{-1}\brb{\bigcap_{i\in I} A_i} \ \lra\  f(x) \in \bigcap_{i\in I} A_i & \lra & f(x) \in A_i \text{ for all }i \\
& \lra & x\in f^{-1}\brb{A_i}\text{ for all }i \ \lra \ x \in \bigcap_{i\in I} f^{-1}\brb{A_i}.
\eeast
\end{proof}

%\beast
%x \in f^{-1}(B \bs A) \ \lra\  f(x) \in B \bs A & \lra & f(x) \in B,\ f(x)\in A^c \\
%& \lra & x\in f^{-1}\brb{B},\ x\in f^{-1}\brb{A^c} \\
%& \lra & x\in f^{-1}\brb{B},\ x\in \brb{f^{-1}\brb{A}}^c \ \lra \ x \in f^{-1}(B) \left\bs f^{-1}(A)\right..
%\eeast
...
\be
A \subseteq f^{-1}\brb{f(A)}.
\ee

For any subset $B\subseteq Y$, we have
\be
f\brb{f^{-1}(B)} = B\cap f(X).
\ee
...
\be
y = f(x) \in f\brb{f^{-1}(B)} \ \ra\ B\cap f(X) \subseteq f\brb{f^{-1}(B)}
\ee
...
\be
f^{-1}\brb{f(A)} = f^{-1}(1) = \bra{a,b} \neq A.
\ee

\item [(ii)] Let $X=Y = \R$ and $f(x) = x^2$ and $A = [0,1]$. Then
\be
f^{-1}\brb{f(A)} = f^{-1}\brb{[0,1]} = [-1,1] \neq A.
\ee
\een
\end{example}
...
\be
\brb{f\circ g}^{-1}(W) =g^{-1} \brb{f^{-1}(W)} = \bra{x\in X:\brb{f\circ g}(x) \in W}.
\ee
\end{proposition}
...
Thus,
\beast
x\in (f\circ g)^{-1}(W) & \ \lra \ &  \brb{f\circ g}(x) \in W \ \lra\ f(g(x)) \in W \\
& \ \lra\ & g(x) \in f^{-1}(W) \ \lra \  x\in g^{-1}\brb{f^{-1}(W)}.
\eeast
\end{proof}
...
\be
\brb{g\circ f }(x) = x,\qquad (g\circ f = \identity_X)
\ee
...
\be
\brb{f\circ g }(y) = y,\qquad (g\circ f = \identity_Y)
\ee
...
\be
\brb{g\circ f}(x) = g\brb{f(x)} = g(y) = x \ \ra\ g\circ f = \identity_X.
\ee
...
\be
\brb{f\circ g}(y) = f\brb{g(y)} = y \ \ra\ f\circ g = \identity_Y.
\ee
...
\be
f\brb{g(y)} = y,\qquad g\brb{f(x)} = x.\qquad\qquad \brb{f\circ g = \identity_Y,\qquad g\circ f = \identity_X}
\ee
...
\be
g_1 = g_1 \circ i_Y = g_1 \circ \brb{f\circ g_2} = \brb{g_1\circ f}\circ g_2 = \identity_X \circ g_2 = g_2,
\ee
...
\beast
\brb{f\circ g} \circ\brb{ g^{-1}\circ f^{-1}} & = & f\circ \brb{g \circ  g^{-1}} \circ f^{-1} = f\circ i_Y \circ f^{-1} = f \circ f^{-1} = \identity_Z,\\
\brb{ g^{-1}\circ f^{-1}}\circ  \brb{f\circ g}  & = & g^{-1} \circ \brb{f^{-1}\circ f} \circ g = g^{-1}\circ i_Y \circ g = g^{-1} \circ g = \identity_X.
\eeast
