...
\item [(ii)] $r_1r_2 = (s_1,t_1)(s_2,t_2) = (s_1s_2,t_1t_2) \in S\times T $ and $1_R := \brb{1_S,1_T}$.%Associative since multiplication in $H$ and $K$ is associative.
...
\be
(f + g)(X) = \sum_i (a_i + b_i)X^i,\quad\quad (fg)(X) = \sum_i \brb{\sum^i_{j=0} a_jb_{i-j} } X^i
\ee
...
\be
f(X) = \sum_{i\in \Z} a_iX^i, \quad\quad (fg)(X) = \sum_i \brb{\sum_j a_jb_{i-j}} X^i
\ee
...
\be
s_1(s_2 + s_3) = \theta(r_1)\brb{\theta(r_2) + \theta(r_3)} = \theta(r_1)\theta(r_2+r_3) = \theta\brb{r_1(r_2 + r_3)}
\ee

Then by distributivity of ring $R$,
\be
\theta\brb{r_1(r_2 + r_3)} = \theta (r_1r_2 + r_1r_3) = \theta (r_1r_2) + \theta(r_1r_3) = \theta (r_1)\theta(r_2) + \theta (r_1)\theta(r_3) = s_1s_2 + s_1 s_3.
\ee
...
\be
\Z/n\Z \to \Z/n_1\Z \times \dots \times \Z/n_k\Z,\quad r \mapsto \brb{r\lmod{n_1},\dots, r\lmod{n_k}}.
\ee
...
\beast
\theta(r_1+r_2) & = & \brb{(r_1+r_2)\lmod{n_1},\dots, (r_1+r_2)\lmod{n_k}} \\
& = & \brb{(r_1\lmod{n_1},\dots, r_1\lmod{n_k}} + \brb{(r_2\lmod{n_1},\dots, r_2\lmod{n_k}} \\
& = & \theta(r_1)+ \theta(r_2).
\eeast

\beast
\theta(r_1r_2) & = & \brb{(r_1r_2)\lmod{n_1},\dots, (r_1r_2)\lmod{n_k}} \\
& = & \brb{(r_1\lmod{n_1},\dots, r_1\lmod{n_k}}\cdot \brb{(r_2\lmod{n_1},\dots, r_2\lmod{n_k}} \\
& = & \theta(r_1)\theta(r_2).
\eeast

For $r = 1_{\Z/n\Z}$, we have that
\be
\theta(r) = \brb{r\lmod{n_1},\dots,r\lmod{n_k}} = \brb{1_{\Z/n_1\Z},\dots,1_{\Z/n_k\Z}}
\ee
...
\be
\brb{\Z/p\Z}^\times = \brb{\Z/p\Z}\bs \bra{0} = \bra{1,2,\dots,p-1}.
\ee

In particular, if $p = 7$,
\be
\brb{\Z/p\Z}^\times = \brb{\Z/7\Z}^\times = \bra{1,2,3,4,5,6}.
\ee

\item [(ii)] The multiplicative group of quotient ring $\Z/8\Z$ is
\be
\brb{\Z/8\Z}^\times = \bra{1,3,5,7}.
\ee
\een
\end{example}
...
\be
\brb{\Z/2\Z}^\times \cong C_1,\qquad \brb{\Z/4\Z}^\times \cong C_2,\qquad \brb{\Z/2^n\Z}^\times \cong C_2\times C_{2^{n-2}},\quad n\geq 2.
\ee
\end{theorem}
...
\be
\brb{\Z/2\Z}^\times = \bra{1} \cong C_1,\qquad \brb{\Z/4\Z}^\times = \bra{1,3} \cong C_2.
\ee

For $n=3$, we have
\be
\brb{\Z/8\Z}^\times = \bra{1,3,5,7} \cong C_2\times C_2
\ee
...
\be
\brb{\Z/16\Z}^\times = \bra{1,3,5,7,9,11,13,15} \cong C_2\times C_4
\ee
...
\be
\brb{\Z/2^n\Z}^\times \cong C_2 \times C_{2^{n-2}}
\ee
...
%\be
%3^{2^{n-3}} = 1 + 2^{n-1}x \ \ra\ \ 3^{2^{n-2}} = 9 \brb{1 + 2^{n-1}x} = (1+8) \brb{1 + 2^{n-1}x} \equiv 1 + 2 + 2^{n-1}x \not\equiv 1 \lmod{2^{n}}.
%\ee
%
%Furthermore,
%\be
...
Then for $k\geq 2$
\be
3^{2^{k}} = (1+2)^{2^{k}} = 1 + \sum^{2^k}_{i=1} 2^i \binom{2^k}{i} =  1 + \sum^{2^k}_{i=1} \frac{2^i \brb{2^k}!}{i!\brb{2^k-i}!}.
\ee

%For any $i\geq 1$, we have that by fundamental theorem of arithmetic $i! = p^{a_i}z$ for some positive integer $z$. Then $a_i < i$ so $p\mid \frac{p^i}{p^{a_i}}$. Also we have $p^{n-1}\left|\frac{\brb{p^{n-1}}!}{\brb{p^{n-1}-i}!}\right.$. Therefore,
%\be
%p^n \left| \frac{p^i \brb{p^{n-1}}!}{i!\brb{p^{n-1}-i}!}\right.
%\ee
%which implies that
...
\be
\frac{2^i \brb{2^k}!}{i!\brb{2^k-i}!} = x 2^{i} 2^{k-j} = x 2^k 2^{2^j y -j}.
\ee

But we know that $2^j y - j \geq 2$ for all $j$ except $i=1,2$. Hence for ($\dag$),
\beast
3^{2^{n-2}} & = & (1+2)^{2^{n-2}} \equiv 1 + 2\cdot 2^{n-2} + 2^2 \cdot \frac{2^{n-2}\brb{2^{n-2}-1}}2 \lmod{2^n} \\
& = & 1 + 2\cdot 2^{n-2} + 2 \cdot 2^{n-2}\brb{2^{n-2}-1} = 1 + 2^{2n-3} \equiv 1 \lmod{2^n}
\eeast
since $n\geq 5$. Similarly, for ($*$) we know that $2^j y - j \geq 3$ for all $j$ except $i=1,2,4$,
\beast
3^{2^{n-3}} & = & (1+2)^{2^{n-3}} \equiv 1 + 2\cdot 2^{n-3} + 2^2 \cdot \frac{2^{n-3}\brb{2^{n-3}-1}}2 + 2^4\cdot \frac{2^{n-3}\brb{2^{n-3}-1}\brb{2^{n-3}-2}\brb{2^{n-3}-3}}{24} \lmod{2^n} \\
& = & 1 + 2\cdot 2^{n-3} + 2 \cdot 2^{n-3}\brb{2^{n-3}-1} + \frac{2^{n-1}\brb{2^{n-3}-1}\brb{2^{n-4}-1}\brb{2^{n-3}-3}}{3} \lmod{2^n} \\
...
\be
\brb{\Z/p\Z}^{\times} = \brb{\Z/p\Z}\bs \bra{0} = \bra{1,2,\dots, p-1} \cong C_{p-1}.
\ee%forms $\brb{\Z/p\Z}^{\times}$, the cyclic multiplicative groups of $\Z/p\Z$.%is cyclic.
\end{theorem}

\begin{proof}[\bf Proof]
{\bf Approach 1.} First, it is easy to see that $\brb{\Z/p\Z}\bs \bra{0}$ is a group under multiplication.

%$\forall x\in \brb{\Z/p\Z}^*$. If $x = 1$ or $p-1$, then its inverse is itself since
%\be
...
\be
\brb{\Z/p\Z}^{\times} = \brb{\Z/p\Z}\bs \bra{0} \cong C_{d_1} \times C_{d_2} \times \dots \times C_{d_n}\quad \text{with }d_i\mid d_{i+1}
\ee
...
\be
g^{dp^{n-1}} = (1+pz)^{p^{n-1}} = 1 + \sum^{p^{n-1}}_{i=1} (pz)^i \binom{p^{n-1}}{i} =  1 + \sum^{p^{n-1}}_{i=1} z^i \frac{p^i \brb{p^{n-1}}!}{i!\brb{p^{n-1}-i}!}.
\ee
...
which implies that the $i$th term is
\be
z^i \frac{p^i \brb{p^{n-1}}!}{i!\brb{p^{n-1}-i}!} = z^i x p^{i} p^{n-1-k} = z^i x p^n p^{p^k y -1-k}.
\ee
...
Therefore the order of $g$ in $\brb{\Z/p^n\Z}^\times$ is at most $dp^{n-1}$, i.e., $o(g) \leq dp^{n-1}$. However, $o(g)$ should be $\phi\brb{p^n} = p^n - p^{n-1}$ and then $d\geq p-1$ which is the size of $\brb{\Z/p\Z}^\times$. Therefore, $g$ is also a generator in $\brb{\Z/p\Z}^\times$. %Since  which is a contradiction.
...
\be
\phi\brb{p^n} = p^{n-1}(p-1) \ \ra\ d\mid p^{n-1}(p-1).
\ee
...
\begin{proof}[\bf Proof]%For $a\in \brb{\Z/p\Z}^\times$, we can see that $a\in \brb{\Z/p^2\Z}^\times$ as well.
Consider the natural reduction
\be
\theta: \ \brb{\Z/p^2\Z}^\times \to \brb{\Z/p\Z}^\times,\quad a+ p^2\Z \mapsto a+ p\Z,\qquad p\nmid a.% = 1,\dots p-1.
\ee
...
a^{p-1} = 1+np
\ee
where $\gcd(n,p)=1$. Then we let the preimage of $a$ be $x := a(1+np) = a + anp \in \brb{\Z/p^2\Z}^\times$. % since $\gcd(n,p)=1$.
Then we have for $1\leq k < p-1$
\be
x^{k} = a^k\brb{1+np}^k \equiv a^k \lmod{p} \ \ra\ x^k \not\equiv 1\lmod{p} \ \ra\ x^k \not\equiv 1 \lmod{p^2}.
\ee

However,
\be
x^{p-1} = a^{p-1}\brb{1+np}^{p-1} = (1+np)^p \equiv 1 \lmod{p^2}.%\equiv (1+np)\brb{1+np(p-1)}\equiv (1+np)(1-np) \equiv 1 \lmod{p^2}.
\ee
...
%\footnote{and $\brb{\Z/p^2\Z}^\times/\ker\theta\cong \im\theta = \brb{\Z/p\Z}^\times$ by the first isomorphism theorem (Theorem \ref{thm:isomorphism_1_group}) since $\theta$ is surjective. Therefore the quotient group $\brb{\Z/p^2\Z}^\times/\ker\theta$ (a subgroup of $\brb{\Z/p^2\Z}^\times$) is cyclic since $\brb{\Z/p\Z}^\times$ is cyclic (Theorem \ref{thm:multiplicative_group_pz_cyclic}) Then we can find an element $g$ of order $p-1$ in $\brb{\Z/p^2\Z}^\times$.}%has the same size $p-1$ with $\im\theta$



%Also, we can find a generator $g$ of $\brb{\Z/p\Z}^\times$ with order $p-1$. Clearly, $g\in \brb{\Z/p^2\Z}^\times$. Therefore, $\brb{\Z/p^2\Z}^\times$ have an element $g$ of order $p-1$ and an element of $h$ of order $p$. Then $gh$ must has order $p(p-1)$, which
...
\be
\brb{\Z/9\Z}^{\times} = \bra{1,2,4,5,7,8} \cong C_6.%2 \times C_3.
\ee
...
Then for each $k$
\be
g^{p^k(p-1)} = \brb{1+pz}^{p^k} = 1 + \sum_{i=1}^{p^k} p^iz^i \binom{p^k}{i} = 1 + \sum_{i=1}^{p^k} \frac{p^iz^i \brb{p^k}!}{i!\brb{p^k-i}!}
\ee
...
which implies that the $i$th term is
\be
z^i \frac{p^i \brb{p^{k}}!}{i!\brb{p^{k}-i}!} = z^i x p^{i} p^{k-r} = z^i x p^k p^{p^r y -r}.
\ee
...
\be
\bra{1,3,5,7} = \brb{\Z/2^3\Z}^{\times} \cong \brb{\Z/2\Z}^2.
\ee
\end{remark}
...
\be
\brb{\Z/n\Z}^\times \cong \brb{\Z/n_1\Z}^\times \times \dots \times \brb{\Z/n_k\Z}^\times = \prod^k_{i=1} \brb{\Z/n_i\Z}^\times.
\ee
\end{theorem}
...
\be
\brb{\Z/n_1\Z \times \dots \times \Z/n_k\Z}^\times \cong \brb{\Z/n_1\Z}^\times \times \dots \times \brb{\Z/n_k\Z}^\times.
\ee
...
Consider the mapping $\theta$:
\be
\brb{\Z/n\Z}^\times \to \brb{\Z/n_1\Z}^\times \times \dots \times \brb{\Z/n_k\Z}^\times,\quad a \mapsto \brb{a\lmod{n_1},\dots, a\lmod{n_k}}.
\ee
...
\be
\hcf(a,n_i) = 1 \ \ra\ \hcf(a,n)=1 \ \ra\ a \in \brb{\Z/n\Z}^\times \ \ra\  \theta \text{ is surjective.}
\ee
...
\be
\brb{\Z/360\Z}^\times \cong \brb{\Z/2^3\Z}^\times \times \brb{\Z/3^2\Z}^\times \times \brb{\Z/5\Z}^\times \cong \brb{C_2 \times C_{2}}\times C_6 \times C_4 \cong C_2 \times C_2 \times C_4 \times C_6.
\ee
\end{example}

%\footnote{need group structure of $\brb{Z/n\Z}^\times$, see wiki for multiplicative group of integers modulo $n$.}
...
%\be
%\theta (z_1z_2) = \theta\brb{\underbrace{z_1 + \dots + z_1}_{z_2\text{ times}}} = \theta(z_1) + \dots + \theta(z_1) = g^z_1 \dots g^{z_1} = g^{z_1z_2} =
%\ee
...
\be
\phi:\Z[\sqrt{2}] \to \Z^+,\ a+b\sqrt{2} \mapsto \abs{\brb{a+b\sqrt{2}}\brb{a-b\sqrt{2}}} = \abs{a^2 - 2b^2}.
\ee

Then for $a+b\sqrt{2}, c+ d\sqrt{2} \in \Z[\sqrt{2}]$,
\beast
\phi\brb{\brb{a+b\sqrt{2}}\brb{c+d\sqrt{2}}} & = & \phi \brb{ac + 2bd + (ad + bc)\sqrt{2}} = \abs{(ac + 2bd)^2 - 2(ad + bc)^2} \\
...
\be
1 = \phi(a-b\sqrt{2}) = \abs{\brb{a-b\sqrt{2}}\brb{a+b\sqrt{2}}} > \abs{1} = 1. \text{ (Contradiction.)}
\ee
...
\be
\brb{1 + \sqrt{2}}^n < u < \brb{1+\sqrt{2}}^{n+1} \ \ra \ 1 < u \brb{1+\sqrt{2}}^{-n} < 1+\sqrt{2}.
\ee
...
\be
p_1 \dots p_m = up_1q_2 \dots q_n \ \ra \ p_1 \brb{p_2 \dots p_m - uq_2\dots q_n} = 0 \ \ra \ p_2 \dots p_m = uq_2\dots q_n
\ee
%\be
...
\be
abf(X) = g(X) c(bh(X)) ah_1(X) = u'c(bh(X)) u'^{-1} a g(X)h_1(X) = ab u'^{-1} a g(X)h_1(X) = ab  g(X) \brb{u'^{-1} ah_1(X) }.
\ee
...
\be
c(f(X)) = c\brb{p_1 \dots p_m \cdot q_1(X) \dots q_n(X)} = u p_1 \dots p_m \cdot c(q_1(X)) \dots c(q_n(X)) = u' p_1 \dots p_m\quad (\dag)
\ee
...
\be
R[X_1],\quad R[X_1,X_2] = (R[X_1])[X_2],\quad R[X_1,X_2,X_3] = \brb{(R[X_1])[X_2]}[X_3]\dots.
\ee
...
\item [(b)] If $p \equiv 1 \lmod{4}$, we consider the finite field of $p$ elements $\Z/p\Z = \bra{0, 1,\dots , p - 1}$ (see Example \ref{exa:zpz_field}). Then by Proposition \ref{thm:multiplicative_group_pz_cyclic}, the field $\Z/p\Z$ with $p$ elements has a cyclic multiplicative group $\brb{\Z/p\Z}^{\times} = \brb{\Z/p\Z}\bs\bra{0}$ and thus the order of $\brb{\Z/p\Z}^{\times}$ can be divided by 4. A cyclic group $\bsa{g}$ of order $4n$ has a subgroup of order 4, namely $\bsa{ g^n }$, and a unique element of order 2, namely $g^{2n}$. $p - 1$ is the element of order 2 in $\brb{\Z/p\Z} \bs\bra{0}$, the multiplicative group since %$\F_p^* := \F_p\bs\bra{0}$
...
& \cong & \Z[X]/\bsa{f_\alpha(X)}/\brb{\bsa{p, f_\alpha(X)}/\bsa{f_\alpha(X)}} \quad (\text{the third isomorphism theorem, Theorem \ref{thm:isomorphism_3_ring}})\\
...
\caption{group structure of $\brb{\Z/n\Z}^\times$} \label{tab:group_structure_multiplicative_group_modular_ring}\\ %\vspace{-2mm}
\hline
& $n$ & & $\brb{\Z/n\Z}^\times$ & & $\phi(n)$ & & & $n$ & & $\brb{\Z/n\Z}^\times$ & & $\phi(n)$ & \\ %& & $n$ & & $\brb{\Z/n\Z}^\times$ & & $\phi(n)$ & & & $n$ & & $\brb{\Z/n\Z}^\times$ & & $\phi(n)$ &  \\
\hline
& 1 & & $C_1$ & & 1 & & & 2 & & $C_1$ & & 1 & \\%&
& 3 & & $C_2$ & & 2 & & & 4 & & $C_2$ & & 2 &  \\
& 5 & & $C_4$ & & 4 & & & 6 & & $C_2$ & & 2 & \\%&
...
\be
\omega^2 = \brb{\frac{1+\sqrt{-3}}2}^2 = \frac{1+(-3) + 2\sqrt{-3}}4 = \frac {-1+\sqrt{-3}}2 = \omega -1.
\ee
...
%\be
%(a_1 + b_1\omega)\brb{a_2 + b_2\omega + a_3 + b_3\omega} = (a_2 + b_2\omega)\brb{a_2 + a_3+ (b_2 + b_3)\omega} =
%\ee
...
IF $a\neq 0$, $c = \frac{1+bd}a$,
\be
ad + b  \frac{1+bd}a + bd = 0  \ \ra \ \brb{a+b + \frac {b^2}a }d = - \frac ba
\ee
...
We can define
\be
N\brb{a+b\omega} = N\brb{a+ \frac b2 + \frac{\sqrt{-3}}2b} = (a+\frac b2)^2 + \frac 34 b^2 = a^2 + b^2 + ab \geq 0.
\ee
...
\be
\phi(g_1 + g_2) = (r(g_1+g_2), (1-r)(g_1 + g_2)) = \brb{rg_1,(1-r)g_1} + \brb{rg_2,(1-r)g_2} = \phi(g_1) + \phi(g_2),
\ee
\be
\phi(g_1g_2) = \brb{rg_1g_2,(1-r)g_1g_2} = \brb{r^2 g_1g_2, (1-r)^2 g_1g_2} = (rg_1,(1-r)g_1)(rg_2,(1-r)g_2) = \phi(g_1)\phi(g_2).
\ee
...
\beast
\ind_{A\cap (B\triangle C)} & = &  \ind_A \ind_{B\triangle C} = \ind_A \brb{\ind_B + \ind_C \lmod{2}} \\
& = & \ind_A \ind_B \lmod{2} + \ind_A \ind_C \lmod{2} = \ind_{A\cap B} + \ind_{A\cap C} \lmod{2} = \ind_{(A\cap B) \triangle (A\cap C)}.
\eeast

%\beast
%A \cap (B\triangle C) & = & A \cap ((B\cup C) \bs (B \cap C)) = A \cap \brb{(B\cup C) \cap (B \cap C)^c} \\
...
We have
\beast
\brb{\frac {-1 + \sqrt{-3}}2}^2 & = & \frac {-2 - 2\sqrt{-3}}4 = \frac {-1 - \sqrt{3}}2 = (-1) \frac {1 + \sqrt{3}}2 = 1 \cdot \frac {1 + \sqrt{3}}2 \lmod{2\omega}\\
\brb{\frac {1 + \sqrt{-3}}2}^2 & = & \frac {-2 + 2\sqrt{-3}}4 = \frac {-1 + \sqrt{3}}2 \\
\brb{\frac {-1 + \sqrt{-3}}2}\brb{\frac {1 + \sqrt{-3}}2} & = & \frac {-3 - 1}4 = -1 = 1 \lmod{2}
\eeast
